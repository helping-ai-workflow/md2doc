...
async function createEditorServer({
  ...
  function bumpIdle(server) {
    ...
    idleTimer = setTimeout(() => {
      // F13: name the reason before closing, so cli.js's listener (the
      // process's stdout writer) can print a line distinct from the
      // SIGINT and never-opened exits.
      server.emit('md2doc-auto-close', 'idle');
      server.close();
    }, idleTimeoutMs);
  ...
  neverOpenedTimer = setTimeout(() => {
    ...
    if (started) return;
    server.emit('md2doc-auto-close', 'never-opened');
    server.close();
  }, neverOpenedGraceMs);
