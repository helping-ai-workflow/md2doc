...
async function createEditorServer({
  ...
  //
  // Capped, because every re-registration is one heartbeat away from costing
  // a headless-Chromium bake: a buggy or malicious loopback script minting a
  // fresh id per ping would otherwise grow this map AND the render load
  // without bound. At the cap the route degrades to the old bare 204.
  ...
  function ensureDrawioClient(fileId, clientId, now) {
    ...
    if (drawioClients.size >= DRAWIO_CLIENT_MAX) return null;
  ...
  function checkDrawioStale(fileId, clientId) {
    ...
    // malformed id, or the DRAWIO_CLIENT_MAX cap. It gets the old bare 204
