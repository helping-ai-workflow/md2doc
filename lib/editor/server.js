...
async function createEditorServer({
  ...
  //
  //   drawioWatch   fileId -> { refs, stamps }.  WHAT THE LAST BAKE READ.
  //                 `refs` is the list of absolute paths that render actually
  //                 resolved; `stamps` is what each one stamped at read time
  //                 (lib/md2doc.js takes the stamp BEFORE the bake — review
  //                 F4). Used to seed a NEW tab and to know which paths to
  //                 stat at all.
  //
  ...
  function trackDrawioRefs(fileId, refs) {
    ...
    drawioWatch.set(fileId, { refs: paths, stamps });
    return stamps;
  }
  ...
  function checkDrawioStale(fileId, clientId) {
    ...
    // An unknown client is a page from before this feature (or one whose
    // entry was swept after 10 idle minutes). It gets the old bare 204
    // rather than a signal it has no baseline to interpret.
    if (!c) return null;
    const now = new Map();
    let changed = false;
  ...
  // Keeps a live tab out of the idle sweep above, and is the one place the
  // sweep is actually driven from — the heartbeat is the only thing that
  // fires often enough to be worth hanging it on, and it is throttled to at
  // most once a minute inside sweepDrawioClients().
