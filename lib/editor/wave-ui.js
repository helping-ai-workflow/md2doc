...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
     ...
     *
     * `opts.fileLabel` falls back to the page title because md2doc sets that
     * to `path.basename(src, '.md')` — the markdown file's own name — so the
     * fallback is the same answer by another road rather than a placeholder.
     * The sanitiser is not cosmetic: a heading with a `/` in it would
