...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function onCanvasDown(ev) {
      ...
      // v3.5.0 Task 11 fix round 2: a press that lands on the label TEXT
      // itself — not merely somewhere inside the box it sits in — opens the
      // data-label field instead of arming a paint drag. Round 1 gated this
      // on the CELL (`codec.dataSlotOf`/`resolveBusTarget` alone, asked off
      ...
      // the text means to change, so the text is the target; a press
      ...
      // on the text hit rather than on cell membership, so a press on the
      // label of a merged run still resolves to the run's owning explicit
      // cycle regardless of which cycle's pixels the text happens to
      // straddle (see `resolveBusTarget`'s own comment — its answer is the
      // same whichever cycle a press on that shared label geometrically
      // lands on). `.ed-wave-buslabel`'s `pointer-events: visiblePainted`
      // (`lib/md2doc.js`) plus paint order — the `<text>` is appended AFTER
      // its box's `<polygon>` in `drawLane`, so it sits on top and wins the
      // hit test in its own footprint — is what makes the two targets
      // disjoint instead of the text being invisibly swallowed by the
