...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    // readability reason.
    const tickInput = metaField('ed-wave-head-tick', 'head.tick', '起始刻度，例如 0');
    const headTockInput = metaField('ed-wave-head-tock', 'head.tock', '半格刻度起始值，例如 0');
    const everyInput = metaField('ed-wave-head-every', 'head.every', '每幾格顯示一次刻度');
    const footTickInput = metaField('ed-wave-foot-tick', 'foot.tick', '下方起始刻度，例如 0');
    const tockInput = metaField('ed-wave-foot-tock', 'foot.tock', '下方半格刻度起始值，例如 0');
