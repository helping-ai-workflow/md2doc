...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function onCanvasDown(ev) {
      ...
      // v3.5.0 Task 11: a press on a cell inside a bus box — a `=`/`2`-`9`
      // value cycle, or a `.`/`|` continuation inside that same run (fix
      // round 1 — see `resolveBusTarget`'s own comment for the corruption a
      // press on a bare continuation used to cause) — opens the label field
      // over it instead of arming a paint drag. Asked BEFORE the
      ...
      // current. A press `resolveBusTarget` cannot resolve (`'blocked'`)
      // refuses outright rather than falling through — silence is exactly
      // what made the round-1 defect invisible.
