...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
     ...
     *
     * v3.6.0 final fix (M2): every handle is measured against the WRAP's own
     * visible rect before it is allowed to move anything. `.ed-wave-canvas-
     * wrap` is `overflow: auto`, and `getBoundingClientRect()` on a child it
     * has scrolled or clipped out of sight still returns a full viewport
     * rect for a box that is painted NOWHERE — this repo's own documented
     * trap, which the test helper `pointInCanvas` already applies and this
     * function did not. Two things follow, and both are load-bearing:
     *
     *   - A handle with no intersection with the wrap's rect is SKIPPED
     *     outright. It cannot be overlapped by the rail, because it is not
     *     on screen to overlap.
     *   - A handle that is only PARTLY visible contributes the right edge of
     *     its visible part (`min(r.right, wrapRect.right)`), not its full
     *     rect's right. Clamping alone would not do: a fully clipped handle
     *     clamps to `wrapRect.right`, which is the largest value in the
     *     whole calculation and would narrow the rail harder than the real
     *     defect did. The skip is what makes the clamp safe.
     *
     * MEASURED on the fixture `test/wave-rail-clearance.test.js` now carries
     * (24 cycles, `edge: ['a~>b']` from cycle 1 to cycle 20, a real press on
     * the drawn `.ed-wave-edge-hit` stroke), at 1000x900 — the viewport this
     * whole version exists for, since `.ed-wave-side[data-expanded]` is
     * `position: absolute` below 1100px:
     *
     *     to-handle right 1332.2   wrap right 970   visible false
     *     before: side.style.width '160px'  rendered 160px  label input 23px
     *     after:  side.style.width ''       rendered 340px  label input 203px
     *
     * The rail was being collapsed to its floor by a handle drawn 362px
     * past the clip. The edge-shape `<select>` measures 50px in BOTH states
     * and is deliberately NOT quoted as a symptom — it is a fixed-width
     * control, so it survives the squeeze that guts the `<input>` beside it,
     * and quoting it would read as damage that was never there.
     *
     * The wrap's border box is used for that visibility test rather than its
     * padding box, matching `pointInCanvas`'s own visibility check (which
     * reserves the padding box for scroll ARITHMETIC). The wrap carries
     * padding and no border, and content is visible inside padding, so the
     * two boxes are the same rect here anyway.
     *
     * Where this genuinely does nothing, stated as the code behaves rather
     * than as round 4 hoped: an inline width IS still applied at a wide
     * viewport — MEASURED 160px at 1280x900 and 198px at 1600x900 on that
     * same 24-cycle document before this fix, i.e. the old "no inline width
     * is ever applied at a wide viewport" claim was simply false. It was
     * harmless there for a DIFFERENT reason than the one claimed:
     * `.ed-wave-side`'s `flex: 0 0 340px` beats an inline `width` for a flex
     * item's main size, so the rendered column stayed 340px regardless. The
     * same mechanism covers the collapsed 28px rail (`flex: 0 0 28px`), so a
     * leftover inline style from a PRIOR expansion cannot keep it
     * artificially wide. Only the `position: absolute` expanded state below
     * 1100px takes the inline width for real — which is exactly why that is
     * the state the defect showed up in.
     *
     * Residual, accepted limit — and it IS reached, including by this
     * function's own test fixture. When `bodyRight - rightmost - MARGIN`
     * falls below `MIN_WIDTH`, this clamps to `MIN_WIDTH` rather than
     * shrinking further, and the selected handle stays covered by the rail.
     *
     * MEASURED at HEAD on `MEDIUM_MD` — the 12-cycle fixture in
     * `test/wave-rail-clearance.test.js`'s own scenario 2 — at 1000x900:
     *
     *     to-handle 888.2–900.2, painted and not clipped at all
     *     bodyRight 970 -> fits = 970 - 900.2 - 4 = 65.8  (< MIN_WIDTH)
     *     side.style.width '160px', so the rail's left edge lands at 810
     *     => 90.2px of the handle is still under the rail, and
     *        `elementFromPoint` at the handle's own centre returns a
     *        `<span>` INSIDE `.ed-wave-side`, not the handle
     *
     * It needs no contrived fixture: 16 cycles with the `to` end at cycle
     * 12, at 970x900, leaves 166.4px of overlap the same way.
     *
     * The sentence that stood here — "a small residual overlap in a
     * genuinely too-narrow-to-satisfy-both-at-once case … which no fixture
     * here produces" — was therefore wrong three times: the overlap is most
     * of the handle rather than small, the case is ordinary rather than
     * genuinely-too-narrow, and the fixture that produces it shipped in the
     * SAME commit as the sentence denying it. It is recorded rather than
     * quietly replaced because it is this batch's signature defect (a true
     * piece of code with a false sentence beside it) landing inside the
     * delivery that exists to fix that defect — one clause after the
     * warning that "no fixture here reaches it" is a claim about fixtures,
     * never about what a user can reach.
     *
     * Why it is accepted rather than fixed here: below 1100px the rail is
     * `position: absolute` and `bodyRight === wrapRight` (MEASURED: both
     * 970 above), so a handle drawn near the right-hand end of a wide
     * diagram cannot be cleared by narrowing the rail AT ALL — no width
     * satisfies both, and `MIN_WIDTH` is the floor precisely so the panel
     * does not become unusable while trying. Clearing it needs a different
     * mechanism (scroll the wrap, or anchor the overlay to the other edge),
     * which is a design change and not a bound to re-tune.
     *
     * One more thing this does not do, in the opposite direction, and it
     * behaves identically before and after the fix above so it is not a
     * regression: the wrap has no scroll listener, so scrolling AFTER an
     * edge is selected does not re-run the verdict. MEASURED on the
     * 24-cycle fixture at 1000x900 — `scrollLeft = 356` brings the `to`
     * handle back to 964.2–976.2, i.e. painted and squarely under the rail,
     * while `side.style.width` stays `''` from the reading taken at
     * `scrollLeft 0`. That is the only measurable way the new
     * skip-what-the-wrap-clipped rule can be wrong, and it is a STALE
    ...
    function clearSelectedEdgeOfSide() {
      ...
      for (const end of ['from', 'to']) {
        ...
        const painted = Math.min(r.right, wrapRect.right);
        if (rightmost === null || painted > rightmost) rightmost = painted;
      }
