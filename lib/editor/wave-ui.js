...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function drawLane(svg, layout, i) {
      ...
      const lane = row.lane;
      const wave = typeof lane.wave === 'string' ? lane.wave : '';
      const cells = codec.expandWave(wave);
      // An EMPTY lane is one `x` cycle, not nothing.
      //
      // `levelsOf('')` is `[]`, and a drawing that took that literally painted
      // a blank row while the engine painted an x — MEASURED: `renderAny` on
      // `{signal:[{name:'a',wave:''}]}` gives `["xxx","xxx"]`, i.e. exactly one
      // cycle, and it is still exactly one when the lane sits beside a 4-cycle
      // lane (`["xxx","xxx","000",…]`). `wave-geometry` says the same thing
      // from the other side: that lane's own `cycles` is 0 while the diagram's
      // is floored at 1.
      //
      // This is not a corner: `deleteCycles` covering every cycle sets every
      // lane's wave to `''`, which is two clicks away (select all, 刪除 cycle).
      const levels = wave === '' ? ['x'] : codec.levelsOf(cells);
    ...
    function renderPreview(doc) {
      ...
      try {
        ...
        const skinOnPage = d.getElementById('socket') !== null;
