...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function carryMarks(nextDoc) {
      ...
      // Fix round 1: `pendingFrom` is a mark of the exact same shape as
      // `cursor` (a row number plus a cycle) and is carried the same way —
      // by lane IDENTITY through this same trace, not by raw row number —
      // for the same reason `cursor`/`selection` already are: a structural
      // undo/redo can renumber every row, and a mark left pointing at
      // whichever lane inherited the old number is the defect class this
      // function exists to close. A content operation traces as the
      // identity, so an ordinary paint made while armed leaves it alone.
      if (pendingFrom !== null) {
        const at = rowOf(pendingFrom.at);
        if (at !== null) pendingFrom = { at: at, cell: pendingFrom.cell };
      }
    }
