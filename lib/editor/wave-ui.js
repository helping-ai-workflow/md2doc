...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function carryMarks(nextDoc) {
      ...
      marksDoc = nextDoc;
      if (before === null || (cursor === null && selection === null)) return;
      const trace = typeof opts.laneTrace === 'function'
        ? opts.laneTrace(before, nextDoc) : null;
      if (trace === null) { cursor = null; selection = null; return; }
     ...
     * drift apart.
     *
     * A second, invisible, wide-stroke `<path>` is layered over each visible
     * one purely so it can be clicked: the visible path is 1px wide, and
     * `document.elementFromPoint` (what `onCanvasDown` reads to answer "did
     * this press land on a line") would otherwise miss any press that is not
     * on that exact pixel. `pointer-events="stroke"` is what makes a
     * `stroke="transparent"` path hit-testable at all — without it a
     * transparent stroke paints nothing, and "nothing painted" is what
     ...
     * markers, to `url(#tee)` — the five plain shapes (`-`, `~`, `-|`, `|-`,
     * `-|-`) set no `style` at all and draw no arrowhead. `edgeArrowMarker`
     * below reproduces the head/tail split with ONE shared marker definition
     * rather than two: `orient="auto-start-reverse"` is what flips a
     * `marker-start` 180° from a `marker-end` automatically, so the single
     * triangle already points the right way at both ends.
     */
    function renderEdges(svg, doc, layout) {
      ...
      for (const e of edges) {
        ...
        path.setAttribute('stroke', isSelected ? '#1a73e8' : '#0041c4');
    ...
    function edgeTeeMarker() {
      ...
      line.setAttribute('stroke', '#0041c4');
      m.appendChild(line);
      return m;
    }
    ...
    function onCanvasDown(ev) {
      ...
      // returns without arming `drag`: this task draws and selects edges, it
      // does not yet let a press on one paint through to the cell behind it,
      // and it does not drag the endpoint either (`moveEdgeEnd` is wired to a
      // later task's drag, not this one's plain click).
