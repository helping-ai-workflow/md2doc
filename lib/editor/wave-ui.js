...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    // A shared title for every button whose behaviour a LATER task wires up
    // (Task 15: 匯出) — created disabled here so the section's shape and
    // button count are real without pretending to work. The 訊號 four and
    // 群組's 解散群組 left this list in Task 14.
    const NOT_YET_WIRED = '尚未接上，之後的版本會補上這個按鈕的行為';
     ...
     *
     * v3.6.0 Task 14. Called from `render()` AND from `repaintDrawing()`,
     * which is not a duplicate call: `repaintDrawing` is the fast path every
     * arrow key and every pointer drag takes, it changes `selection`, and it
     * deliberately does not rebuild the toolbar. Wired only into `render()`,
     * these buttons kept whatever state the LAST full repaint left them in —
     * MEASURED: open the dialog (no selection, 刪除 disabled), walk to a lane
     * with the arrow keys, and 刪除 was still disabled although a lane was
     * selected and highlighted on both the rail and the canvas. The gesture
     * the user could see was the one the toolbar refused.
     *
     * 新增 and 空白列 stay enabled with nothing selected — they append to
     * the end, which is a real answer — and they publish WHERE that append
     * lands before they are pressed, which is the property the rail's
     * per-row ＋ used to carry in `data-lands-in` and its tooltip. The other
     * three need a lane named, and 解散群組 needs that lane to be inside a
     * group; a button that looks pressable and then answers「先選一條 lane」
     * is the affordance this batch is removing, not reproducing.
     */
    ...
    // and the tooltip, both refreshed by `render()` — and the status line
    // says it again after. That is the property the rail's per-row ＋ used
    // to carry, moved rather than dropped.
    ...
    function addLaneFromToolbar(name, lane) {
      ...
      if (added) panels.say((lane.wave === undefined ? '空白列' : '新增的 lane') + '落在' + where);
    }
    ...
    const signalDeleteBtn = panels.toolButton('ed-wave-signal-delete', '刪除',
      '刪掉選取的那幾條 lane（如果群組因此空掉，群組也會一起走）', function () {
        ...
        // the highest index first leaves the ones still to go untouched. A
        // top-down loop deletes `lo` n times, which is a different set of
        // lanes whenever the range is longer than one.
    ...
    //
    // v3.6.0 Task 14: the range is `selection`'s, not a second mark's.
    // Shift+click on the rail and Shift+arrow on the canvas produce the same
    // one, so「選起來的那幾條」means one thing on screen.
    //
    ...
    const groupBtn = panels.toolButton('ed-wave-group-make', '建立群組',
      '把選起來的那幾條連續 lane 包成一個新群組（在左邊 Shift+點，或在畫布上 Shift+↑↓）',
      function () {
    ...
    // ── 匯出 ──────────────────────────────────────────────────────────────
    //
    // v3.6.0 Task 12: shape-only, wired in Task 15.
    const exportGroup = panels.toolGroup('匯出');
    toolbar.appendChild(exportGroup);
    const exportSvgBtn = panels.toolButton('ed-wave-export-svg', 'SVG',
      NOT_YET_WIRED, function () {}, exportGroup);
    exportSvgBtn.disabled = true;
    const exportPngBtn = panels.toolButton('ed-wave-export-png', 'PNG',
      NOT_YET_WIRED, function () {}, exportGroup);
    exportPngBtn.disabled = true;
    const exportWaveJsonBtn = panels.toolButton('ed-wave-export-wavejson', '複製 WaveJSON',
      NOT_YET_WIRED, function () {}, exportGroup);
    exportWaveJsonBtn.disabled = true;
    ...
    function carryMarks(nextDoc) {
      ...
      if (selection !== null) {
        ...
        if (at !== null) {
          // v3.6.0 Task 14: the FAR end of the lane range is retraced too,
          // and it is DROPPED rather than left on a stale number when its
          // lane cannot be traced — the same split `pendingFrom` documents
          // below, for the same reason. `laneIndex` gets away with "leave
          // the number alone" because it is also a screen position
          // `renderCanvas` re-clamps on every repaint; `laneTo` is the far
          // end of a range a STRUCTURAL op (建立群組, and 訊號's 刪除)
          // reads the instant a button is pressed, and a stale far end
          // silently swallows lanes the user never selected. Collapsed to
          // `at`, the range is exactly the one lane still vouched for.
          const far = rowOf(selection.laneTo);
          const to = far === null ? at : far;
          selection = {
            laneIndex: Math.min(at, to), laneTo: Math.max(at, to),
