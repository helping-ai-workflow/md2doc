...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
     ...
     * not land on any lane at all. Nothing below special-cases the three
     * no-op drops rule 3 lists — dropping back on the start cell, dropping on
     * a row with no waveform (a group header or a spacer), dropping on the
     * cell that already holds this edge's OTHER end:
     *
     *   - a row with no waveform has zero cells in the codec's own internal
     *     reading of that lane, so `codec.ensureNode`'s bounds check already
     *     answers `letter: null` for it — and so does the "missed the
     *     diagram entirely" case (`at === null`), for the same reason:
     *     neither is an integer in range, and `ensureNode` bounds-checks
     *     before it ever looks at the letter that is there;
     ...
     *
     * So there is one test of "did anything change", not three: `commit`'s
