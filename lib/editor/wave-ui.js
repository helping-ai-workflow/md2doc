...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    /**
     * Carry the cell cursor and the selection across a ▲ / ▼.
     *
     * A move is the one gesture that changes which LANE a row index names, and
     * the marks are indexes. The keyboard already follows the lane — ▲ hands
     * the cursor to the button the moved lane arrives at, so pressing it twice
     * moves the same lane twice — and before this the cell cursor and the
     * rail's `is-selected` did not: measured, cursor on `3,0` (`ack`), press ▲,
     * and both marks stayed on row 3, which is now `dat`. Two answers to
     * "where am I", naming different lanes.
     *
     * `from` and `to` are always adjacent here (the buttons move one row), and
     * in the codec's flattened display order an adjacent move exchanges
     * exactly those two rows — so the remap is a swap of the two indexes and
     * every other row keeps its number. Run BEFORE `commit()`, because
     * `commit` repaints and the repaint is what draws the marks.
     */
    function carryMarksAcrossMove(from, to) {
      ...
      if (cursor !== null) {
        cursor = { laneIndex: swap(cursor.laneIndex), cycle: cursor.cycle };
      }
      if (selection !== null) {
        selection = { laneIndex: swap(selection.laneIndex),
    ...
    function renameGroup(span) {
      ...
      // The field is going off screen without committing — either because the
      // user said so (Escape, below) or because SOMETHING ELSE repainted the
      // rail underneath it (an undo, a redo, any gesture that reaches
      // `render()` while it is open). Both need the same two things done
      // before the rebuild: drop the latch, and name where the keyboard goes,
      // because the field's own `data-focus-key` will not exist afterwards and
      // `restoreFocus` would leave focus on a detached node.
      //
      // `focusOverride` is only claimed when nobody else has claimed it: a
      // caller that already knows where the keyboard belongs (a lane ✕ naming
      // its ＋, say) is making a more specific promise than this one.
      const retire = function () {
        if (groupRename === null) return;
        groupRename = null;
        if (focusOverride === null) focusOverride = 'group-' + span.from;
      };
      ...
      if (tag !== null) {
        ...
        // standing (this being non-null), how to take it down on purpose, and
        // how to let go of it when a repaint takes it down for us. A copy of
        // the span or of the field itself would be a second record of
        // something the closure above already holds.
    ...
    function renderCanvas(doc, layout) {
      ...
      // catches the undo/redo of that, and every other route.)
      if (selection !== null) {
        if (layout.lanes.length === 0 || layout.cycles === 0 ||
