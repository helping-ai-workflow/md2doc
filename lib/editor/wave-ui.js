...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    // v3.5.0 Task 13: the ruler fields. `tick`/`every` are the pinned engine's
    // OWN names for `head`'s two ruler controls (`lib/parse-config.js` and
    // `lib/render-marks.js` in `node_modules/wavedrom` read `source.head.tick`
    // and `cxt.head.every` — measured, not recalled), so the labels below are
    // the literal WaveJSON keys, the same convention `head`/`foot` already use
    // above rather than a translated gloss. `tock` is the same ruler control
    // for `foot`; the engine supports `foot.tick` too but this task's brief
    // names only `head.tick` / `head.every` / `foot.tock`, so that is what
    // ships — a fourth field for `foot.tick` is not this task's to add.
    const tickInput = metaField('ed-wave-head-tick', 'tick', '起始刻度，例如 0');
    const everyInput = metaField('ed-wave-head-every', 'every', '每幾格顯示一次刻度');
    const tockInput = metaField('ed-wave-foot-tock', 'tock', '下方起始刻度，例如 0');
    ...
    tickInput.addEventListener('change', function () { commitBanner('head', 'tick', tickInput.value); });
    everyInput.addEventListener('change', function () { commitBanner('head', 'every', everyInput.value); });
    tockInput.addEventListener('change', function () { commitBanner('foot', 'tock', tockInput.value); });
    ...
    // — to `(which, key, value)` so the same function also carries
    // `head.tick` / `head.every` / `foot.tock`. The no-op guard moves with
    // it: it used to read `doc[which]` as a whole because `text` was the
    // only key that object ever held, so "nothing there" and "the text key
    // is empty" were the same question. With more than one key possible on
    // the same object that stops being true — clearing an empty `tick` field
    // must not manufacture `head.text = ''` from a doc that already has a
    // real `head.text` — so the guard now reads the specific key rather than
    // the whole object.
    function commitBanner(which, key, value) {
      commit(which + '.' + key, function (doc) {
        const had = doc[which];
        const hadObj = (had !== null && typeof had === 'object') ? had : {};
        if (value === '' && (hadObj[key] === undefined || hadObj[key] === null)) return doc;
        const next = Object.assign({}, doc);
        next[which] = Object.assign({}, hadObj);
        next[which][key] = value;
        return next;
      });
    }
    ...
    function renderMeta(doc) {
      ...
      footInput.value = (f !== null && typeof f === 'object' && typeof f.text === 'string')
        ? f.text : '';
      tickInput.value = (h !== null && typeof h === 'object') ? tickTockText(h.tick) : '';
      everyInput.value = (h !== null && typeof h === 'object') ? tickTockText(h.every) : '';
      tockInput.value = (f !== null && typeof f === 'object') ? tickTockText(f.tock) : '';
    }
    ...
    function laneRow(doc, layout, i, ungroupSpan) {
      ...
      // v3.5.0 Task 13: duplicate this lane — a full deep copy (`data` /
      // `node` / any other key, via `codec.duplicateLane`) landing right
      // after it at `i + 1`. Same landing-awareness `insertButton` below
      // already gives the user for an ordinary insert: `duplicateLane` is
      // built on the very `addLane`/`laneInsertPath` door that button uses,
      // so it inherits the identical group-tail asymmetry, and the status
      // line says so the same way.
      const dup = d.createElement('button');
      dup.type = 'button';
      dup.className = 'ed-wave-lane-duplicate';
      dup.setAttribute('data-focus-key', 'lane-duplicate-' + i);
      dup.textContent = '複製';
      dup.title = '複製這條 lane，接在它後面（data/node 等所有欄位一起複製）';
    ...
    function onCanvasDown(ev) {
      ...
      if (ev.target !== null && ev.target !== undefined &&
          typeof ev.target.classList === 'object' && ev.target.classList !== null &&
          ev.target.classList.contains('ed-wave-buslabel')) {
