...
(function (root, factory) {
  ...
  const waveDraw = require('./wave-draw.js');
  // `brickOf` is exported below as part of this module's own public shape
  // (unrelated to any one editor instance), but its definition moved to
  // wave-draw.js with the rest of the drawing layer. `brickOf`/`isClock`/
  // `isBus` take no dependencies of their own (pure functions of their one
  // argument), so an empty-deps drawer is enough to keep that export working
  // without a second copy of the logic.
  const pureDraw = waveDraw.createDrawer({});
  ...
  function createWaveEditor(opts) {
    ...
    const drawer = waveDraw.createDrawer({
      d: d,
      geometry: geometry,
      codec: codec,
      SIZES: SIZES,
     ...
     *      to subtract). final review finding 4: trusted when it names the
