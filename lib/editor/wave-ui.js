...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function handleDrawingKey(ev) {
      ...
      // v3.5.0 Task 11: Enter on a cell that owns a `data` slot opens that
      // slot's label field instead of painting over it with the brush — the
      // keyboard's own version of the pointer's click below in
      // `onCanvasDown`. `codec.dataSlotOf` is asked fresh off the lane's own
      // wave string, the same ground truth `onCanvasDown` uses, so the two
      // input devices can never disagree about which cells are labellable.
      // Placed AFTER the armed-edge Enter above (arming an edge outranks
      // opening a label field — the same press cannot mean both) and BEFORE
      // the generic Enter-paints case below, which it must pre-empt.
      if (key === 'Enter' && cursor !== null) {
        ...
        if (dlLane !== null && codec.dataSlotOf(dlLane, cursor.cycle) !== null) {
          ev.preventDefault();
          ev.stopPropagation();
          openDataEdit(cursor.laneIndex, cursor.cycle);
          return true;
        }
      }
    ...
    function onCanvasDown(ev) {
      ...
      // v3.5.0 Task 11: a press on a cell that consumes a `data` slot — a
      // bus value, `=`/`2`-`9` written out explicitly — opens the label
      // field over it instead of arming a paint drag. Asked BEFORE the
      // deselect/paint dispatch below, exactly where the edge checks above
      // claim their own press, so a bus press never reaches `drag` at all;
      // otherwise the ordinary paint gesture's `mouseup` would overwrite the
      // very cell the user is about to type into with whatever brush is
      // current. `codec.dataSlotOf` is asked fresh off the lane's own wave
      // string — the same one `drawLane` answers with `isBus(brick)` for the
      // PICTURE — so a cycle a merged run's polygon visually covers but that
      // the codec says consumes no slot (a `.` continuation inside the run)
      // still falls through to an ordinary paint, exactly as clicking
      // anywhere else on the lane does; the field only ever opens on a cycle
      // that genuinely owns an entry of `data`.
      const busAt = cellFromEvent(ev, layout);
      if (busAt !== null) {
        const busLane = layout.lanes[busAt.laneIndex].lane;
        if (codec.dataSlotOf(busLane, busAt.cycle) !== null) {
          ev.preventDefault();
          const busHadSelectedEdge = selectedEdge !== null;
          selectedEdge = null;
          cursor = { laneIndex: busAt.laneIndex, cycle: busAt.cycle };
          selection = { laneIndex: busAt.laneIndex, from: busAt.cycle, to: busAt.cycle };
          canvas.focus();
          if (busHadSelectedEdge) render();
          else renderCanvas(store.doc, layout);
          openDataEdit(busAt.laneIndex, busAt.cycle);
          return;
        }
      }
