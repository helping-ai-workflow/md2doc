...
(function (root, factory) {
  ...
  function createWaveEditor(opts) {
    ...
    function onKeyDown(ev) {
      ...
      /**
       * v3.6.0 Task 16: Alt+↑ / Alt+↓ move the selected lane up or down.
       *
       ...
       *
       * Claimed HERE rather than inside `handleDrawingKey`, for two reasons
       * that are both load-bearing. `handleDrawingKey` accepts exactly two
       * targets (the canvas and the dialog) and returns false on anything
       * else, while the place a person reordering lanes actually sits is the
       * lane's own NAME FIELD — which is where `moveSelectedLane`'s
       * `focusOverride` puts them and keeps them, so Alt+↓ twice moves the
       * same lane twice instead of two different ones. And the call to it
       * just below is gated on `!ev.altKey`, so an Alt-modified key cannot
       * reach it at all.
       *
    ...
    function moveSelectedLane(step) {
      ...
      //
      // The whole prefix per branch rather than one shared 落在, the same
      // rule 新增/空白列 above record: this phrase ends in a Latin word and
      // therefore carries the space.
      if (didMove) {
    ...
    //
    // What is still true, and still why `panels.renderUnmodelled()` fires
    // for all three: that per-lane geometry keeps Task 3's own R14
    // invariants self-consistent (so hit-testing and painting still invert
    // each other), not reproduce what the ENGINE paints. The shared cycle
    // grid this editor draws once for the whole canvas, and the CYCLE COUNT
    // any one lane shows, do not move with `period`/`phase`/`hscale` the way
    // the engine's own render does — only that lane's own cycle WIDTH does.
    // Measured against the pinned engine (a wider drawn cycle is not the
    // same picture as more or fewer half-bricks):
    //
    //     {wave:'0101'}                 engine 8 half-bricks   drawing 4 cycles, width 48
    //     {wave:'0101', period:2}       engine 16              drawing 4 cycles, width 96
    //     {wave:'0101', phase:0.5}      engine 7               drawing 4 cycles, shifted -24px
    //     {wave:'0101'} config.hscale:2 engine 16              drawing 4 cycles, width 96
    //
    ...
    // by itself; `panels.renderUnmodelled()` still firing for all three,
    // unconditionally, whenever a document carries any of them, is what
    // actually closes it — this editor now lets a user SET a value it
    // cannot draw, but it never lets that go unsaid. Modelling the three
    // properly on the canvas is still not a change this file may make on
    // its own: cycle width lives in `wave-geometry`, which is closed and
    // whose arithmetic Task 3 pinned, and a half-model here would be a
    // SECOND belief about cycle geometry in the same tree.
     ...
     *
     * v3.6.0 Task 16 added the keymap's ONE mode-dependent exception, and it
     * is stated here because the sentence above would otherwise be false for
     * two keys at a state this function itself decides: while the 關聯線
     * gesture is ARMED, ← / → stop walking one cycle and jump between the
     * lane's transitions (`jumpTransition`), and Shift does not extend,
     * because an edge endpoint is one cell. ↑/↓ and Home/End are untouched
     * in every state. Alt+↑/↓ is NOT part of this keymap and never reaches
     * this function — it moves a LANE, is claimed one level up in
     * `onKeyDown`, and could not arrive here anyway (the call site is gated
     * on `!ev.altKey`, and the two targets below would refuse a press made
     * from a lane's name field).
     *
    ...
    function handleDrawingKey(ev) {
      ...
      // v3.6.0 Task 16: while the edge gesture is ARMED, ← / → walk between
      // this lane's transitions instead of one cycle at a time — see
      // `jumpTransition` for what a transition is here and what the two ends
      // do. Ahead of the generic `nav` branch below, which it pre-empts for
      // those two keys only: ↑/↓ and Home/End keep meaning exactly what they
      // meant, because "which lane" and "the ends of the lane" are not
      // questions arming changes the answer to.
      //
      // The gate is `edgeMode === 'armed'`, NOT `!== 'idle'`, and the
      // difference is not cosmetic: `hoverBoundary` — which this writes —
      // only ever means anything while 'armed' (`setEdgeMode` clears it on
      // the way to either neighbour state, and `onCanvasMove` computes its
      // own hot cell fresh on every move while 'dragging'). A keyboard press
      // that arrived mid-drag and wrote it would resurrect exactly the stale
      // hot dot that invariant exists to prevent.
      if (edgeMode === 'armed' && (key === 'ArrowLeft' || key === 'ArrowRight')) {
        ev.preventDefault();
        ev.stopPropagation();
        jumpTransition(key === 'ArrowRight' ? 1 : -1);
        return true;
      }
      if (nav) {
        ev.preventDefault();
     ...
     * TRANSITIONS — `dir === 1` rightwards, `dir === -1` leftwards.
     *
     * What an armed ← / → is for is picking the cell an edge will anchor to,
     ...
     * land on seven cells they cannot usefully aim at. The POINTER can still
     * drop an endpoint anywhere along a flat run — hanging a label in the
     * middle of a bus needs exactly that — so this narrows the keyboard, not
     * the feature.
     *
     ...
     * the cursor is no longer anywhere near. Shift does NOT extend here —
     * an edge endpoint is one cell, so there is no range for an armed ← / →
     * to grow.
     *
    ...
    // The armed exception is named too, because it changes what two keys
    // this same sentence has just promised actually do.
    panels.say('選一個電位，然後在波形上按住拖過去；或直接按方向鍵進波形，' +
      'Shift+左右鍵選一段，再按電位字元塗上去。' +
      'Alt+↑／Alt+↓ 把選取的 lane 往上下搬；' +
      '武裝關聯線之後，左右鍵改成在轉態點之間跳。');
