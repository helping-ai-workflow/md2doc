 ...
 *
 * Contract: this never throws. Every malformed source — including one nested
 * past `MAX_DEPTH` — comes back as `{ok: false, message, offset}`, so callers
 * in the layers above are entitled to call it without a `try`.
 *
...
function parseSource(text) {
  ...
  try {
    ...
    if (err instanceof RangeError) {
      // MAX_DEPTH normally gets here first. This only fires when the caller's
      // own stack was already near its limit, and it exists so the "never
      // throws" contract holds even then.
