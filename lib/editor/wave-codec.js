...
function parseSource(text) {
  ...
  try {
    ...
    return { ok: true, doc: doc, spans: st.spans, members: st.members, dups: st.dups };
  } catch (err) {
...
function resolveSet(ctx, edit) {
  ...
  const ser = serialize(edit.value, quote);
...
function resolveInsert(ctx, edit) {
  ...
  const ser = serialize(edit.value, '"');
  if (ser.ok !== true) return refuse(ser.why, rangeOf(path, parentPath, parentSpan));

  if (parent.value.length === 0) {
    // Nothing to sit beside, so it goes just inside the opening bracket.
    return { ok: true, path: path, start: parentSpan[0] + 1, end: parentSpan[0] + 1, text: ser.text };
  }
  if (index < parent.value.length) {
    const at = ctx.parsed.members.get(JSON.stringify(parentPath.concat([index])));
    if (!at) {
      return refuse('no member span for ' + JSON.stringify(path), rangeOf(path, parentPath, parentSpan));
    }
    const indent = lineIndentBefore(ctx.text, at[0]);
    const gap = indent === null ? ' ' : '\n' + indent;
    return { ok: true, path: path, start: at[0], end: at[0], text: ser.text + ',' + gap };
  }
  ...
  // behind it.
  const at = endOfOwnLineTrivia(ctx.text, last[1]);
...
function resolveRemove(ctx, edit) {
  ...
  const range = widenToWholeLine(ctx.text, member[0], member[1]);
...
/**
 * Walk forward from `from` over what is left of that one physical line, as long
 * as it is only horizontal whitespace and comments that stay on the line, and
 * answer where the line break is. Anything else on the line — a closing bracket
 * on a one-line array, a block comment that runs on to the next line — answers
...
function hasLineBreak(text, from, to) {
...
function endOfOwnLineTrivia(text, from) {
  let j = from;
  for (;;) {
    const c = text[j];
    if (c === ' ' || c === '\t') { j++; continue; }
    ...
    if (c === '/' && text[j + 1] === '*') {
      ...
      if (hasLineBreak(text, j, close)) return from;
      j = close + 2;
      continue;
    }
    return from;
...
/**
 * Widen a removal to swallow the line it would otherwise leave empty. Only
 * whitespace and the line break are taken, and only when the member had the
 * line to itself — a trailing comment after it, or anything else in front of
 * it, leaves the range exactly as the member span drew it.
 */
function widenToWholeLine(text, start, end) {
  let s = start;
  while (s > 0 && (text[s - 1] === ' ' || text[s - 1] === '\t')) s--;
  if (s > 0 && text[s - 1] !== '\n') return [start, end];
  let e = end;
 ...
 */
function serialize(value, quote) {
  ...
  if (kind === 'array') {
    ...
    for (const item of value) {
      const ser = serialize(item, quote);
      if (ser.ok !== true) return ser;
      parts.push(ser.text);
    }
  ...
  if (kind === 'object') {
    ...
    for (const key of Object.keys(value)) {
      const ser = serialize(value[key], quote);
      if (ser.ok !== true) return ser;
