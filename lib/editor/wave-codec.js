 ...
 * half addresses container paths, because that is how spans are keyed. On a
 * document with a group the two do not coincide — display position 3 is
 * `['signal', 2]` in the fixture above — so a caller that builds
 * `['signal', index]` for itself renames the wrong lane, and nothing refuses,
 * because that path names a real lane too. Every structural edit this module can
 * produce has exactly one supported way to be written back, and it starts here:
 * `lanePath` for a value or a removal source, `laneRemovePath` for what a
 * removal actually has to take out, `laneInsertPath` for where a new lane goes.
 */
...
function emptiedMemberPath(doc, path) {
  ...
  for (;;) {
    ...
    for (let i = 0; i < parent.value.length; i++) {
      if (i !== last && holdsLane(parent.value[i])) {
        survives = true;
        break;
      }
    }
...
/** Whether this is a lane, or a group with a lane anywhere inside it. */
function holdsLane(node) {
  if (isLane(node)) return true;
  if (!Array.isArray(node)) return false;
  for (const item of node) {
    if (holdsLane(item)) return true;
  }
  return false;
}
...
module.exports = {
  ...
  lanePath, laneInsertPath, laneRemovePath,
};
