 ...
 * it, at display position `at + 1` — a full deep copy, `data` / `node` / any
 * other key included, not a name-and-wave reconstruction that would silently
 * drop the rest. Built on `addLane`, which already does that deep copy
 * (`copyValue(lane, MAX_DEPTH)`) for a caller-owned object; this just hands it
 * the SAME lane object the document already holds instead of one the caller
 * built, so nothing about that guarantee is re-decided here.
 *
...
function duplicateLane(doc, at) {
  ...
  return addLane(doc, at + 1, flat[at].lane);
}
