...
function createDrawer(deps) {
  ...
  function drawBand(svg, doc, layout, which) {
    ...
    const textY = isHead
      ...
      : layout.height - layout.footHeight / 2;
    const scaleY = isHead
    ...
    const row = layout.lanes.length > 0 ? layout.lanes[0] : null;
    if (row === null) return;

    if (b.tick !== undefined) {
      const base = Number(b.tick);
      for (let c = 0; c <= layout.cycles; c += 1) {
        if ((c + base) % every !== 0) continue;
        const t = d.createElementNS(SVGNS, 'text');
        t.setAttribute('class', 'ed-wave-ruler-tick');
        t.setAttribute('x', row.originX + c * row.cycleWidth);
    ...
    if (b.tock !== undefined) {
      ...
      for (let c = 0; c < layout.cycles; c += 1) {
        ...
        t.setAttribute('x', row.originX + (c + 0.5) * row.cycleWidth);
        t.setAttribute('y', scaleY);
        t.setAttribute('text-anchor', 'middle');
        t.appendChild(d.createTextNode(String(base + c)));
        svg.appendChild(t);
