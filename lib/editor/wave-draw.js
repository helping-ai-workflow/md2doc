...
function createDrawer(deps) {
  ...
  function renderCanvas(canvas, doc, layout, state) {
    ...
    // v3.6.0 Task 16 fix round 1: the armed exception belongs HERE, not only
    // in the code. This label is the one keyboard manual a screen-reader user
    // ever gets for this surface, and Task 16 made two of its claims
    // mode-dependent: while the 關聯線 gesture is armed, ← / → stop walking
    // one cell and jump between transitions instead, and Shift stops
    // extending (`wave-ui.js`'s `jumpTransition`). A label that still
    // promised the old two would be the only description a user has, and
    // wrong. Alt+↑/↓ is deliberately NOT here — it is a dialog-wide gesture
    // that moves a LANE, not a drawing key, and it is named in the opening
    // status line instead.
    canvas.setAttribute('aria-label',
      '波形繪圖區：方向鍵移動游標，Shift+左右鍵選一段 cycle，' +
      '按電位字元（' + BRUSHES.join(' ') + '）塗上去；' +
      '武裝關聯線之後，左右鍵改成在轉態點之間跳，Shift 不再延伸選取');
