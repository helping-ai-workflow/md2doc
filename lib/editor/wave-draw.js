...
function createDrawer(deps) {
   ...
   * `wave-geometry.js`'s own `SKIN_METRICS` comment measures `slewStartRatio`
   * from:
   *
   ...
   * — `slewStartRatio` (0.075) to 0.225, NOT to `SKIN_METRICS.slewEndRatio`
   * (0.15). Cross-checked against a real node/edge render
   * (`{signal:[{wave:'0.1.0...',node:'a.b.c...'}],edge:['a-b']}`): the
   * engine's own `gmark_a_b` sits at `cycle*40 + 6`, i.e. exactly the ramp's
   * MIDPOINT ((3+9)/2 = 6, and also the midpoint of its height), not its
   * top. `slewEndRatio` is a correct, separately-pinned value for where the
   * anchor/edge sits (Task 5 depends on it, and its own test re-derives it
   * against the engine) — it is a different quantity from "where the visual
   * ramp finishes" that happens to share a name. So the ramp drawn here ends
   * at the MEASURED 0.225, derived as `2*anchorRatio - slewStartRatio`
   * rather than hardcoded, so the relationship to the two already-pinned
   * ratios stays visible if either is ever re-measured. See
   * `task-7-report.md` for the full derivation and the render dump it came
   * from.
   */
  function brickPath(level, prevLevel, width, height) {
    const s = geometry.SKIN_METRICS;
    const rampStart = s.slewStartRatio * width;
    const rampEnd = (2 * s.anchorRatio - s.slewStartRatio) * width;
  ...
  /**
   * The set of bricks `brickPath`'s ramp applies to: the plain "just a
   * line" levels `bandOf` already treats as a single y. Bus (hexagon) and
   * xxx (box) already carry their own slanted/boxed edges and are out of
   * this task's scope; clock bricks draw a whole period (including its own
   * sharp vertical edges — MEASURED: the engine's own `pclk`/`nclk` paths
   * are plain verticals, `'M0,20 0,0 20,0'`/`'m0,0 0,20 20,0'`, no ramp at
   * all) and never reach here (`drawLane`'s `!isClock` guard on both sides).
   */
  function isLineBrick(brick) {
    return !isBus(brick) && brick !== 'xxx' && !isClock(brick);
  }

  /**
   * `brickPath`'s own `yFor` only recognises the three positions `bandOf`
   * assigns a line brick to (top/bottom/mid) — this maps a BRICK ID (not
   * the raw level char, which can be any of `'0'`/`'l'`/`'L'`/`'1'`/`'h'`/
   * `'H'`/`'z'`/`'u'`/`'d'`) onto the one char `yFor` maps to that SAME
   * height, so a `uuu`/`ddd` ramp lands on the exact y `bandOf` draws its
   * flat run at.
   */
  function lineLevelOf(brick) {
    if (brick === '111' || brick === 'uuu') return '1';
    if (brick === '000' || brick === 'ddd') return '0';
    return 'z'; // 'zzz' — brickPath's `yFor` default is already mid-height.
  }
  ...
  function drawLane(svg, layout, i) {
    ...
    for (let r = 0; r < runs.length; r++) {
      ...
      if (r > 0 && !isClock(brick) && !isClock(runs[r - 1].brick)) {
        ...
        if (isLineBrick(brick) && isLineBrick(prevBrick)) {
