...
function createDrawer(deps) {
   ...
   * out of `drawLane` (v3.6.0 Task 10) so it can also drive `levelIcon`'s
  ...
  /** The bus hexagon's point list — lifted out of `drawLane`'s bus branch
   *  (v3.6.0 Task 10) so `levelIcon`'s bus glyph is the exact same hexagon,
   *  not a second hand-drawn approximation of it. */
  ...
  /** The clock brick's stepped edge — lifted out of `drawLane`'s `isClock`
   *  branch (v3.6.0 Task 10), same reasoning as `busPolygonPoints`. */
  ...
  /** The `P`/`N` explicit-edge arrowhead — lifted out of `drawLane`'s
   *  `isClock` branch (v3.6.0 Task 10). `null` for any other level char,
   *  same as the inline `if` it replaces. */
  ...
  /** The gap `|` marker's double chevron — lifted out of `drawLane`'s gap
   *  loop (v3.6.0 Task 10), same reasoning as `busPolygonPoints`. */
   ...
   *
   * Every shape below is built from the SAME functions `drawLane` calls to
   * paint the real canvas (`brickOf`, `bandOf`, `brickPath`, `isBus`,
   * `isClock`, `busPolygonPoints`, `clockStepPath`, `clockArrowPath`,
   * `gapMarkPath`) — never a second, hand-drawn set. That is the one rule
   * this task exists to enforce: change a brush's on-canvas appearance and
   * every toolbar icon updates with it, because there is only one place
   * that appearance is decided.
   *
  ...
  function levelIcon(level) {
    ...
    if (level === '|') {
      ...
      // erases `|` before `brickOf` ever sees it, per `drawLane`'s own
      // `gaps` comment above). So this has to be checked ahead of the
  ...
  function renderCanvas(canvas, doc, layout, state) {
    ...
    if (state.selection !== null) {
      if (layout.lanes.length === 0 || layout.cycles === 0 ||
        ...
        // v3.6.0 Task 14: `laneTo` — the far end of the lane range — is
        // rebuilt here, not merely copied. This function REPLACES the
        // caller's selection object (`wave-ui.js` reads it back out of the
        // state bag afterwards), so a key left out here is a key deleted
        // from the live selection, and `laneRow`'s `i <= laneTo` test would
        // then be comparing against `undefined` for every row — no row
        // highlighted, and 建立群組 disabled, after any repaint at all.
        // Clamped into the shortened document the same way `laneIndex` is
        // checked above, and never allowed below `laneIndex`, so the
        // normalised invariant the whole file relies on survives a delete.
        const lastLane = layout.lanes.length - 1;
        const far = typeof state.selection.laneTo === 'number'
          ? Math.min(state.selection.laneTo, lastLane) : state.selection.laneIndex;
        state.selection = {
          laneIndex: state.selection.laneIndex,
          laneTo: Math.max(state.selection.laneIndex, far),
    ...
    // This label is the one keyboard manual a screen-reader user ever gets
    // for this surface, so every claim in it has to be true in every state.
    //
    // v3.6.0 Task 16 fix round 3 REPLACES what round 1 put here. Round 1 had
    // to say "while the 關聯線 gesture is armed, ← / → stop walking one cell
    // and Shift stops extending", because that is what round 0's binding did.
    // Round 3 took the jump off the unmodified arrows (see
    // `handleDrawingKey`'s own comment and T9b), so the first two clauses of
    // this label are unconditionally true again and the mode-dependent
    // sentence is gone with the mode-dependence. What remains is an EXTRA
    // key, Alt+←/→, which is named because a key nobody can discover is a
    // key that does not exist for a keyboard user. Alt+↑/↓ is still
    // deliberately absent: it moves a LANE, is dialog-wide rather than a
    // drawing key, and is named in the opening status line instead.
    ...
    // v3.6.0 Task 11: 轉態標記——只在 `edgeMode !== 'idle'` 時畫。平常畫面
    // 不該多這一排點，而且它同時是「你現在在連線模式」這個狀態的唯一視覺
    // 證據——在這之前只有工具列那顆按鈕變亮，畫布上看不出來。
    //
    // 落在 `geometry.anchorOfCell`，不是格子中心：這批標記的整個存在理由
    // 是讓使用者瞄準跟畫線用的同一個點，一個畫在格中心的標記會教使用者瞄
    // 準錯的地方，比完全不畫還糟。
    //
    // 資料來自 `geometry.transitionsOf`，不是「這條 lane 的每一格」：它只
    // 問 `codec.expandWave` 已展開的真實資料格，`reachOf` 允許一條 lane 的
    // 可觸及範圍遠超過它自己的 `cycles`（畫過去是延伸 lane 的手段，見
    // `reachOf`/`cellAt` 自己的註解），但那些延伸格沒有資料、也就沒有轉
    // 態——`transitionsOf` 天生不會把它們算進來，這裡不需要另外擋。
    drawTransitions(canvas, doc, layout, state);
    ...
    // v3.6.0 Task 11: hover, unlike the document-level drag pair
    // `wave-ui.js` adds/removes around `onCanvasDown`/`onCanvasUp`, has to
    // be live from the moment the canvas exists — a plain hover with no
    // button down never reaches a mousedown handler at all. Optional
    // (`state.onCanvasHover` may be absent, e.g. `test/wave-draw.test.js`'s
    // `fakeCanvasState()`, which never exercises pointer movement): guarded
    // the same way `state.dataEdit`/`state.cursor` above tolerate a minimal
    // test fixture, rather than requiring every caller to supply a no-op.
    if (state.onCanvasHover) canvas.addEventListener('mousemove', state.onCanvasHover);
  ...
  /** 轉態標記的半徑：一般 / 游標懸停在上面那一顆。 */
  const TRANSITION_R = 3;
  const TRANSITION_R_HOT = 5;

  /**
   * 轉態標記——使用者瞄準 edge 端點的那些點，畫在 `geometry.anchorOfCell`
   * 上，不是格中心（見上方 `renderCanvas` 呼叫處的註解）。
   *
   * `state.hoverBoundary` 是 `wave-ui.js` 的 `boundaryAt` 原生形狀
   * `{laneIndex, cell}`（不是 `cellFromEvent` 對外一致改叫的 `cycle`——
   * 這裡直接比對 `cell`，兩處欄位名故意不同，混用會在這裡悄悄比對失敗）。
   */
  function drawTransitions(svg, doc, layout, state) {
    if (!state || state.edgeMode === undefined || state.edgeMode === 'idle') return;
    for (let i = 0; i < layout.lanes.length; i += 1) {
      const cells = geometry.transitionsOf(doc, i);
      for (const c of cells) {
        const a = geometry.anchorOfCell(layout, i, c);
        if (a === null) continue;
        const hot = state.hoverBoundary !== undefined && state.hoverBoundary !== null &&
          state.hoverBoundary.laneIndex === i && state.hoverBoundary.cell === c;
        const dot = d.createElementNS(SVGNS, 'circle');
        dot.setAttribute('class', 'ed-wave-transition');
        dot.setAttribute('cx', a.x);
        dot.setAttribute('cy', a.y);
        dot.setAttribute('r', hot ? TRANSITION_R_HOT : TRANSITION_R);
        if (hot) dot.setAttribute('data-hot', '');
        svg.appendChild(dot);
      }
    }
  }
  ...
  function drawLane(svg, layout, i) {
    ...
    // lane actually agrees with it. `editor-journey.test.js`'s T6i reads
    // exactly this attribute to prove `period`/`config.hscale` reach the
    // canvas at all (spec A5): its `period: 2` lane under a global
    // `hscale: 2` must measure 4× the same lane in the control document,
    // and the `period`-less sibling 2×.
    ...
    // `bandOf` (module scope, v3.6.0 Task 10 — also driving `levelIcon`)
    // takes hi/lo/mid explicitly rather than closing over this function's
    // locals, so it is the one belief this file has about which height a
    // brick paints at, not a copy of it.
    ...
    for (let r = 0; r < runs.length; r++) {
      ...
      if (isBus(brick)) {
        ...
        poly.setAttribute('class', 'ed-wave-bus ed-wave-bus-' + brick.slice(4));
        poly.setAttribute('points', busPolygonPoints(x0, x1, hi, lo, mid, s));
        svg.appendChild(poly);
        ...
        p.setAttribute('d', clockStepPath(brick, x0, x1, hi, lo));
        svg.appendChild(p);
        ...
        const arrowD = clockArrowPath(edgeLevel, x0, xm, hi, lo);
        if (arrowD !== null) {
          const arrow = d.createElementNS(SVGNS, 'path');
          arrow.setAttribute('class', 'ed-wave-clock-arrow');
          arrow.setAttribute('d', arrowD);
        ...
        const y = bandOf(brick, hi, lo, mid)[0];
        const line = d.createElementNS(SVGNS, 'line');
        line.setAttribute('class', brick === 'uuu' || brick === 'ddd'
      ...
      if (r > 0 && !isClock(brick) && !isClock(runs[r - 1].brick)) {
        ...
        if (isRampPair(brick, prevBrick)) {
          ...
          const a = bandOf(prevBrick, hi, lo, mid);
          const b = bandOf(brick, hi, lo, mid);
    ...
    for (const c of gaps) {
      ...
      mark.setAttribute('d', gapMarkPath(xm, hi, lo, mid));
