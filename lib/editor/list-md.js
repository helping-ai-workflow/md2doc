...
(function (root, factory) {
  ...
  //
  // ONE BLOCK == ONE PHYSICAL LINE. `lines` and `lineMeta` are pushed in
  // lockstep, and a caller that commits a single item's line by index
  // (client.js's per-li degrade path) relies on lineMeta[k] describing
  // md.split('\n')[k]. That only holds while no emitted line can itself contain
  // a newline — see the isBlankText() filter at the .ed-li-text unwrap below.
  //
  ...
  function serializeBlocks(blockEls) {
    ...
    toArray(blockEls).forEach((blockEl) => {
      ...
      // ONE BLOCK == ONE PHYSICAL LINE, enforced rather than assumed.
      //
      // The isBlankText() filter above removes marked's pretty-print artifact,
      // but it cannot remove a newline that is genuinely part of the item's own
      // content: a TIGHT item with a lazy continuation ('- a1' / '    cont')
      // renders as ONE text node holding a literal '\n', which inline-md.js's
      // escapeText() emits verbatim. That item's own content is two physical
      // lines, and a {startLine, endLine} block plus a line-indexed lineMeta
      // simply cannot express it: the extra line shifted every later entry, and
      // the per-li degrade path — which slices md.split('\n') by position —
      // wrote a DIFFERENT item's line into the edited item's range (observed:
      // editing '- a2' after such an item replaced its source line with the
      // continuation text and dropped the edit).
      //
      // So the rule is: an item whose own content is not a single contiguous
      // line is UNSUPPORTED. Truncating to the first physical line keeps
      // `lines` and `lineMeta` parallel — which is what lets a SIBLING's
      // single-line commit still land correctly — and flagging it per-block
      // means canWysiwygForLi() never arms it, listRunSupportsStructuralEdit()
      // refuses structural keys on its run, and the non-empty `unsupported`
      // forces the per-li partial path, so this item's own source bytes are
      // never rewritten from the truncated line. Degrade-never-lose: the `md`
      // is lossy exactly where a caller is contractually required not to use
      // it. 'MULTILINE' is not an element name (every other entry is) because
      // it is not an element — it is a shape the emission contract forbids.
      if (/\n/.test(itemMd)) {
        innerUnsupported.push('MULTILINE');
        itemMd = itemMd.split('\n')[0];
      }
