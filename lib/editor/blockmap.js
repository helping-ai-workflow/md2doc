...
function pushListItemBlocks(listToken, cursor, indent, blocks, nextId) {
  for (const item of listToken.items) {
    ...
    // sits one BEFORE its startLine: its own content really is empty, because
    // the child begins on the very first line. Preserved as-is — it is the
    // shape this file has always produced for that markdown — but it is the one
    // place a block's range is not a well-formed interval, so treat it as a
    // marker of "no own lines", never as an interval to iterate.
