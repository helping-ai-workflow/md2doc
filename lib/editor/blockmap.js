...
// v3.4.0 (backlog #13, closing the "改動前後相同" observation for real):
// MEASURED on marked 14.1.4, a top-level 'paragraph' token's `raw` can carry
// an internal newline that names NO real blank line in the source. Minimal
// case: 'a\n\tb\n---\n- x\n- y\n\ntail\n' — the paragraph's raw comes back as
// 'a\n\n\tb\n' (3 newlines) for what is 2 real source lines ('a', a TAB- or
// >=4-space-over-indented lazy continuation 'b'); every block from the '---'
// onward then names the line ABOVE its real one. This is not "only literal
// '---'" — a table SEPARATOR row immediately after the same over-indented
// continuation ('|---|', ':---') reproduces it too, and the trigger tracks
// the count of over-indented continuation lines, not any single terminator
// (see runs/task11-item13-scope-hunt*.js: 195+ cases across continuation
// width x repeat-count x fifteen terminators — corruption only where the
// line right after the continuation is dash/equals-shaped, matching
// whatever internal check marked runs to decide a following line MIGHT be a
// setext-heading underline; every other terminator tried — the other hr
// markers, headings, blockquotes, fences, header-led tables, lists, a
// genuine setext '===' — left raw uncorrupted in every case). A 'heading'
// token was TRIED as a second covered type (it goes through the same marked
// code path — setext heading construction IS this ambiguity's positive
// case) and REJECTED: test/blockmap.test.js's own first fixture ('# Title\n\n
// Para one.\n\n…') broke immediately, because ATX headings legitimately FOLD
// a trailing blank-line run into their own raw when nothing else does (raw
// '# Title\n\n' for a heading followed by one blank line, with no separate
// 'space' token) — exactly the behaviour this file's own header comment
// documents ("A token's raw may include trailing newlines … folded in").
// `t.text` for a heading is just its own single line of prose ("Title", 0
// newlines) and has no way to reconstruct how many blank lines got folded
// after it, so text-based counting silently DROPS that fold and lands the
// next block one line early. Paragraphs do not fold trailing blanks this way
// (MEASURED: a blank-line run after a paragraph always comes back as its own
// separate 'space' token, at EOF and mid-document alike — see
// runs/task11-item13-parafold.js), which is exactly why paragraphs are safe
// to cover here and headings are not.
//
// This file's own item-level code already named the general family: a lazy
// continuation's `text`-token raw is SYNTHESISED ("a lazy continuation comes
// back as \"x\n\ncont\n\" (three newlines) for two source lines" — see
// childListStartOffsets()'s header comment), which is why that code never
// trusts a `text` token's raw for cursor advancement. This is the same
// defence at buildBlockMap()'s TOP level, where a top-level 'paragraph' is
// marked's equivalent of an in-item lazy continuation — the top level had no
// corresponding defence because the header comment's own stated basis for
// trusting top-level raws ("at top level the raws are faithful source
// slices") is exactly the assumption this case disproves.
//
// The fix does not detect corruption and fall back conditionally; it simply
// never counts newlines out of a 'paragraph' token's `raw` at all. `t.text`
// — marked's own dedented join of the token's real content lines, e.g.
// "a\nb" for the fixture above — was MEASURED to stay correct in every
// corrupted case found in the sweep: its newline count always equals (real
// source lines consumed - 1), carrying none of the synthetic blank lines
// `raw` picks up. And on every UNCORRUPTED case, `text`-based and
// `raw`-based counting agree exactly — MEASURED via a direct before/after
// diff of buildBlockMap()'s own OUTPUT (not just the raw newline count) over
// every fixture already in test/blockmap.test.js plus a 60000-document
// random line-soup fuzz corpus (see runs/task11-item13-fuzz-oracle.js and
// the report) — so there is no accuracy cost paid on the common path.
//
// Every OTHER top-level token type is left on the original raw-based
// computation, unconditionally: the sweep found no corruption outside
// 'paragraph', and 'space' tokens in particular NEED their raw's FULL,
// untrimmed newline count (to skip a multi-blank-line run) — trimmedLineCount()
// or text-based counting would get that case wrong on purpose, not by
// oversight.
function topLevelLineInfo(t) {
  if (t.type === 'paragraph' && typeof t.text === 'string') {
    const contentNewlines = (t.text.match(/\n/g) || []).length;
    const endsWithNewline = t.raw.charAt(t.raw.length - 1) === '\n';
    return { span: contentNewlines + 1, rawNewlines: contentNewlines + (endsWithNewline ? 1 : 0) };
  }
  return { span: trimmedLineCount(t.raw), rawNewlines: (t.raw.match(/\n/g) || []).length };
}
...
// its nested line. Neither of those two strings is itself a fixture — they came
// out of a capture harness and a mutation sweep that are not in the repo. What
// IS pinned, and what to read instead, is the shape FAMILY — grep 'trailing
// space' in test/blockmap.test.js for the range assertions, and 'eats the item
// above' in test/editor-journey.test.js for the gestures that used to do
// exactly that.
//
  ...
  for (const t of tokens) {
    ...
    if (t.type === 'list') {
      ...
      blocks.push({
        ...
        type: t.type,
        startLine: cursor,
        endLine: cursor + info.span - 1,
      });
    }
    // advance by full raw (including folded trailing blank lines) for every
    // type except 'paragraph'/'heading', which use topLevelLineInfo()'s
    // text-based count instead — see its own comment for why.
    // If raw has no trailing newline (EOF), the next token doesn't exist,
    // so the off-by-one is unobservable.
    cursor += info.rawNewlines;
  }
