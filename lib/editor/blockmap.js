...
// range under the old search, and so did the everyday
// '- one item\n- two item\n  - nested item \n- three item\n' the moment a
// trailing space landed on its nested line. (The capture harness is described in
// .superpowers/sdd/2026-09-07-md2doc-v3.3.0/task-4-report.md §4; the shapes are
// pinned in test/blockmap.test.js — grep it for 'trailing space'.)
//
...
// That leaves `pos` ON the newline that ended the consumed token rather than
// after it, so both searches below start one byte earlier than they used to.
// The anchored search cannot match there — indexOfAtLineStart() requires the
// PRECEDING character to be '\n', and at `pos` that newline is the character
// itself, so the earliest line start it will accept is `pos + 1`. The
// un-anchored `content.indexOf(needle, pos)` fallback beneath it has no such
// requirement; what rules it out is the needle, which would have to BEGIN with
// a newline to match at `pos`. MEASURED over the journey capture plus a
// synthetic sweep of nested lists (blank lines, fenced code, blockquotes,
// tab indents, task markers): no nested list token's `raw` began with '\n'.
...
function childListStartOffsets(item) {
  ...
  for (const tk of item.tokens || []) {
    ...
    // `pos` stays where it was. Starting a later search EARLIER can only find
    // the same occurrence or one inside the blank run, and a blank run holds no
    // list marker. MEASURED across the journey capture and a synthetic
    // nested-list sweep: `space` was the only token type whose needle ever
    // collapsed to '' — which is also the only type that CAN, its raw being
    // whitespace by construction — and an ablation that kept advancing `pos` on
    // an empty needle produced block maps identical to these on every document
    // in both corpora.
    if (needle === '') continue;
...
function pushListItemBlocks(listToken, cursor, indent, blocks, nextId, inheritedUnlocatable) {
  for (const item of listToken.items) {
    ...
    // startLine`, and it is what canWysiwygForLi(), openRawEditor(),
    // deleteBlockViaGutter(), the ＋ insert paths and bystanderCarryOver()
    // all consult, so an empty range refuses at every arming and commit
    // boundary instead of writing somebody else's line.
    //
