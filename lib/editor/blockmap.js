...
// markdown the CLIENT serialises mid-session contains the shape. Captured off a
// full test/editor-journey.test.js run — 131 distinct markdown strings reached
// this function, 95 of them serialised by client.js rather than authored —
// '# H\n\n- alpha\n  - beta\n    - \n- gamma\n' arrived from the client and
// mapped to an inverted range under the old search. Appending one trailing
// space to each non-blank line of that captured corpus in turn (1231 variants)
// produced 15 corrupted maps before this change and 0 after, including the
// everyday '- one item\n- two item\n  - nested item \n- three item\n'.
//
// Both sides are put through the same normalisation before the search: trailing
// spaces and tabs are dropped from every line. That CANNOT change a line count,
// because it only ever removes characters that are not '\n' — so a character
// offset found in the normalised copy converts to the same LINE offset as it
// would have in the original, which is the only thing this function returns.
// It also cannot break a search that already worked: two strings that were
// byte-identical stay byte-identical under one shared transform.
//
...
// Shortening the needle only moves `pos` back onto that newline, and the next
// list search requires a line start, so nothing can match there.
