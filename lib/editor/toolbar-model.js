...
function deriveState(ctx) {
  ...
  // v3.2.0: the five inline-format buttons are dead with a collapsed caret —
  // applyMarkToggle() (client.js:9251) and applyLinkToggle() (:9284) both return
  // immediately on range.collapsed, `link` included (it does not even open the
  // prompt). Greying them out matches the runtime semantics exactly.
  if (!ctx.hasSelection) {
