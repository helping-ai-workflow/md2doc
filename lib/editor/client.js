...
(function () {
  ...
  function canWysiwygForLi(blockEl) {
    ...
    return res.unsupported.length === 0 && res.unsupportedByLi.length === 0;
  }
  ...
  function runBlocksOf(blockEl) {
    ...
    if (!self.listStart) {
      for (let k = i - 1; k >= 0; k--) {
        ...
        if (a.listStart) break; // rule (d): nothing before this list's first item
      }
    }
    for (let k = i + 1; k < all.length; k++) {
      const a = liAttrs(all[k]);
      if (!a || a.indent < self.indent || a.listStart) break; // rule (d)
      if (a.indent === self.indent) {
        if (a.listType !== self.listType) break;
        out.push(all[k]);
      }
  ...
  function listRunOf(blockEl) {
    ...
    for (let k = endIdx + 1; k < all.length; k++) {
      const a = liAttrs(all[k]);
      if (!a || a.indent <= anchorIndent || a.listStart) break; // rule (d)
      endIdx = k;
    }
  ...
  function refreshRunStarts() {
    ...
    all.forEach((el) => {
      ...
      // Rule (d): a new list token always opens a new run, and closes every run
      // the previous list had open. data-list-start is renderer-owned and never
      // rewritten here — it is the only carrier of the token boundary between
      // two adjacent same-type lists.
      if (a.listStart) { prev = null; types.length = 0; }
  ...
  function armEditables(root) {
    ...
    blockEls.forEach((blockEl) => {
      ...
      if (editEl && (blockType === 'paragraph' || blockType === 'heading') &&
        ...
        // MUST return here — a list block gets NO ⠿/＋ gutter chrome (overlay
        // chrome is P4). The unconditional appendChild calls below would inject
        // <button> children into the block; list-md.js's serializeBlocks() now
        // allowlists 'ed-handle'/'ed-insert' as chrome, but the buttons would
        // still be unstyled and mispositioned until Task 4 lands the flat
        // gutter. Do NOT remove this return before then.
        return;
  ...
  async function resolveBurst() {
    ...
    if (burst.blockType === 'li') {
      ...
      if (unsupported.length > 0) {
        ...
        // among the run's emitted LINES, NOT the source-line delta
        // (editedBlock.startLine - range.startLine). The tight runMd emits
        // exactly ONE line per block in document order and has NO blank lines,
        // so a loose item present anywhere earlier in the run makes a later
        // supported li's SOURCE startLine overshoot the tight runMd's line
        // count — the old delta slice then returned '' and commitRangeRemoval
        // DELETED the li's line.
        //
        // S1: `lineMeta` is read instead of re-deriving the position from the
        // DOM. Controller note T2-C — serializeBlocks() emits no line at all
        // for a block it refuses — means "k-th element == k-th line" is no
        // longer an identity in general, and lineMeta is the serializer's own
        // authoritative line→blockId mapping. Its blockId is the raw
        // getAttribute() string, hence the String() comparison (same
        // convention as the unsupportedByLi gate above).
        const offset = lineMeta.findIndex((m) => m.blockId === editedIdStr);
  ...
  async function commitListStructure(runEls, focusStartLine, caretToEnd, presetRange) {
    const span = runEls || [];
    const { md } = listMd.serializeBlocks(span);
  ...
  // The line `targetBlock` will occupy once the run span it belongs to is
  // committed by commitListStructure() above. list-md.js's one-block==one-line
  // write invariant means the span's serialized markdown has exactly one line
  // per block in document order, so the target's line is the span's own
  // startLine plus its index in the span. Holds even when the PRE-commit source
  // had multi-line items, because the commit replaces the whole range with the
  // canonical one-line-per-item form.
  //
  // (Controller note T2-C: serializeBlocks() emits NO line for a non-li block,
  // so index==line only holds for an all-li span. listRunOf() never returns a
  // non-li block — its scan stops at one — so the identity is safe here, but
  // do not carry it over to a span assembled some other way.)
  // Returns null when the run (or the block) cannot be located.
  function runLineOfBlock(runEls, targetBlock) {
    const range = runRangeOfBlocks({ lines, blocks, stack }, runEls);
    if (!range) return null;
    const idx = runEls.indexOf(targetBlock);
  ...
  function listRunSupportsStructuralEdit(runEls) {
    return !!(runEls && runEls.length) &&
      listMd.serializeBlocks(runEls).unsupported.length === 0;
  }
  ...
  // Tab (spec §11 row 5): the item's indent goes up by one, clamped by spec
  // §3.4 rule 1 — "the previous block's indent + 1", with an upper bound of 0
  // when the previous block is not a list item — and its OWN SUBTREE moves with
  // it. Later same-level siblings are untouched. Returns true iff something
  // actually moved, so the caller only commits on a real change.
  //
  // The subtree moving with the item is the SHIPPED, test-pinned behaviour
  // (test/editor-client-runtime.test.js row 5: '- a / - b / (2sp)- b1 /
  // (2sp)- b2 / - c' + Tab on b commits '- a / (2sp)- b / (4sp)- b1 /
  // (4sp)- b2 / - c'), and it came for free pre-S1 because Tab re-parented the
  // <li> and its nested <ul> travelled inside it. Spec §3.5's table says the
  // opposite ("子項不跟隨"); that is a spec-vs-shipped divergence for whoever
  // owns §3.4/§3.5 clamping to resolve, NOT something to change here — a Global
  // Constraint of this plan is that no existing test's expected markdown may
  // move without the spec moving first.
  //
  ...
  async function convertEmptyTopLevelLiToParagraph(runEls, li) {
    ...
    const ok = await commitListStructure(survivors, null, false, range);
    if (!ok) return;
  ...
  function handleLiKeydown(e, editEl) {
    ...
    if (e.key === 'Tab') {
      if (!listRunSupportsStructuralEdit(run)) { refuseStructuralListEdit(); return true; }
      ...
      run = listRunOf(li);
      commitListStructure(run, runLineOfBlock(run, li), true);
      return true;
    }
    ...
    run = listRunOf(newLi);
    commitListStructure(run, runLineOfBlock(run, newLi), false);
    return true;
  }
  ...
  function wireBlockSelection() {
    ...
    document.addEventListener('click', async (e) => {
      ...
      if (checkEl) {
        ...
        if (!run.length) return;
        if (!listRunSupportsStructuralEdit(run)) { refuseStructuralListEdit(); return; }
        ...
        if (!listRunSupportsStructuralEdit(targetRun)) { refuseStructuralListEdit(); return; }
