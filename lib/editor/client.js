...
(function () {
  ...
  function toggleGutterMenu(blockEl) {
    ...
    // them).
    gutterMenuConvert.hidden = (blockType === 'table');
  ...
  async function convertBlockViaMenu(blockEl, target) {
    ...
    // S2 Task 3 routes list SOURCES here and Task 5 routes list TARGETS here,
    // each with its own run re-serialization / blank-line policy (§4.3 rules
    // 1 and 2). Until then both REFUSE rather than commit a shape this task
    // cannot produce correctly: a li converted by the plain path below would
    // leave the file mid-list with no blank line and lazy continuation would
    // swallow the result into the item above it (measured, §4.3 rule 1).
    if (kind === 'li' || convertMd.targetIsList(target)) {
      refuseStructuralListEdit('清單的轉換尚未實作');
      return;
    }
  ...
  // Real implementation lands in S2 Task 6 (§4.3: the copy goes after the
  // block's ENTIRE subtree, not after its own line, or the original loses its
  // children). The menu item is visible from this task on because §3.7 fixes
  // the four-item list and test/editor-client-runtime.test.js asserts it, so
  // this refusal is reachable — it is a banner, never a silent no-op.
  async function duplicateBlockViaMenu(blockEl) {
    if (!blockEl) return;
    refuseStructuralListEdit('複製尚未實作');
  }
