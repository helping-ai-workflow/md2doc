...
(function () {
  ...
  // 隱藏它們就夠；.ed-seltb 沒有（它只由 selectionchange 與 applyMarkToggle
  // 驅動，而捲動不觸發 selectionchange），純隱藏會讓它永遠回不來，所以它要
  // 重新定位、只在選取整個離開視窗時才隱藏。
  // capture 階段是必要的：.content 與側欄可以自己捲而 document 不捲，且
  // scroll 事件不冒泡。
  //
  ...
  function onAnyScroll() {
    ...
    hideTableGrips();
    hideTableInsertBubbles();
    hideTableEdgeMenu();      // ← 真正會刪資料的是選單，不是 grip
    closeToolbarMenu();
