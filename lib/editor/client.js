...
(function () {
  ...
  // whose empty-Enter REMOVES the item — which is why the sublist refusal is
  // still correct there. The per-li path uses liOwnTextIsBlank() below; see
  // RULING F-Q on its own comment for why the two must differ.
  ...
  function handleLiKeydown(e, editEl) {
    ...
    if (liOwnTextIsBlank(li)) {
      ...
      const outdented = mutateListRun(() => {
        ...
        // placeholder <br> behind. liOwnTextIsBlank() sees through it
        // (textContent === ''), but inline-md.js serializes a real <br> as a
        // literal '<br>', so committing the outdented item as-is would write
        // '- <br>' where the user sees an empty item. Clear the surface (inside
        // the suppression span: an innerHTML assignment on the focused node
        // triggers the very same Chromium unfocus quirk), and only once the
        // outdent above has actually happened — a refused press must leave the
        // DOM byte-identical.
        const textEl = liTextEl(li);
