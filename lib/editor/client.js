...
(function () {
  ...
  function updateTableEdgeGrips(x, y, target) {
    ...
    // the zone; the column-insert bubble never overlaps the row grip's zone
    // at all — it sits at each header cell's RIGHT edge, never at the
    // table's LEFT edge the row grip is anchored to (measured x=[884.6,
    // 902.6] against the grip's padded x=[398,426] on the same fixture; true
    // for any column boundary in general, since a column bubble can only
    // repeat a cell's right edge, never the table's own left edge).
    //
