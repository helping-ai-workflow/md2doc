...
(function () {
  ...
  let lastParts = null;
  // F10: 上一次【成功套用】的 render 留下來的兩樣東西 —— 當時的整份 `lines`，
  // 以及當時的文件是不是以一道沒閉合的圍欄收尾。
  //
  // fix round 2 (K7): 這裡不再說「跟 `blocks` 一起前進」，那句是錯的。`blocks`
  // 是被 applyPatch()／applyFullRender() 自己換掉的（applyFullRender() 的第一行
  // 就是 `blocks = j.blocks`），時機不受套用成功與否管；下面這兩個是
  // applyRenderResult() 在【呼叫那兩支之前】讀、在【套用成功之後】才寫的，所以
  ...
  // 認字元與長度 —— 見那兩支的註解，K1／K3）。只有開頭圍欄、還沒有本文與收尾
  // 的單行 code 區塊也算沒閉合。
  //
  ...
  // F10 fix round 2 (K2) —— 吞噬與【修剪】的差別是【歸屬】，不是計數。
  //
  // 上一版是「形狀 ∧ 區塊少了」。它擋掉了計數式判別的四種誤報，但放進來一族新
  // 的：使用者把收尾圍欄連同它後面的東西一起【刪掉】（在全文原始碼或 raw 編輯
  // 器裡往後一路 trim），區塊也是 4~5 掉到 3、文件也真的以沒閉合的圍欄收尾 ——
  // 可是圍欄後面本來就沒有東西了，是使用者自己刪的。那時候那句話的兩個子句
  // （「後面的內容全部被吃進去」「補回收尾圍欄就會復原」）都是假的。
  //
  // 真正的差別：吞噬時，本來活在 code block【外面】的行跑進了 code block
  // 【裡面】；修剪時那些行只是不見了。所以判別式問的是內容的歸屬，不是行號的
  // 對齊（對齊在插入／刪除混在一起時很脆，這個做法完全不需要它）：
  //
  //   把新的尾端圍欄的【本文】拿出來，去掉空白行，看有沒有哪一行
  //   （a）在上一份 render 裡活在 code block 外面，而且
  //   （b）在這一份 render 裡已經不在 code block 外面了。
  //
  // （b）那一半是給內容碰巧撞名準備的：使用者把最後一段換成一道沒閉合的圍欄、
  // 而圍欄本文剛好跟文件別處某一段一字不差時，那一段仍然好端端在外面，所以
  // 不算被吃掉。
  //
  // 比對用【整行字串】：行號在兩份 render 之間會被編輯位移，block id 是位置
  // 序號、同樣會被位移，所以這裡拿整行內容當身分。代價寫在下面 `now` 那一半。
  function outsideCodeContent(srcLines, blockList) {
    const inCode = {};
    for (const b of blockList) {
      if (!b || b.type !== 'code') continue;
      for (let n = b.startLine; n <= b.endLine; n++) inCode[n] = true;
    }
  ...
  function absorbedOutsideContent(prevLines, prevBlocks, newLines, newBlocks, tail) {
    const was = outsideCodeContent(prevLines, prevBlocks);
    const now = outsideCodeContent(newLines, newBlocks);
    for (let n = tail.startLine + 1; n <= tail.endLine; n++) {
      const row = newLines[n - 1];
      if (typeof row !== 'string' || row.trim() === '') continue;
      if (was[row] && !now[row]) return true;
    }
    return false;
  }
  ...
  async function applyRenderResult(j, editRange, scrollY, declaredRange) {
    ...
    // 不見了，不是搬進去）。
    //
    // 三個合取項各由一列 journey 控制列釘住，而且各有一個 ablation 證明它有
    // 偵測力：
    //   nowUnclosed              —— 上面那族「清空內容」的誤報靠它擋
    //   !lastRenderEndedUnclosed —— 一開檔就沒閉合的文件靠它不被誣賴
    ...
    // 第 1/2/3 行那三格的機制跟計數無關：新的開頭圍欄跟文件裡【原本就有的】
    ...
    // 在是為了把「render 沒套用成功就不要前進上面那兩個 lastRender* 狀態、也
  ...
  // 進來的原始碼；只有下面 b／b' 那兩種形狀會被動到），`caret` 是 offset。
  //
  ...
  //     所以在兩道圍欄之間種一行空白進 `value`，caret 落在那一行。
  ...
  //  其餘一律維持結尾落點。
  //
  // caret 仍然會落在一行【長得像圍欄】的行上的兩種情形（掃 27 種來源形狀量
  // 到的，兩種都不是破壞）：
  ...
  // 圍欄的 info string 不得再含反引號 —— 少了這兩條，'~~~' 開頭、下一行是
  // '```' 的巢狀圍欄會被誤判成【空圍欄】，於是下面那個 grown 分支往使用者的
  // 檔案裡種進一行空白，而且提交時真的寫到磁碟（40c9569 沒有這個問題，是
  // 588ae30 引進的）。同一個誤判的另一面在 endsWithUnclosedFence()：一份以
  // 光禿禿 '~~~' 收尾的文件會被當成「圍欄有收好」，真正的吞噬因此靜默。
  ...
  function closesFence(line, opener) {
    ...
    const m = /^ {0,3}(`{3,}|~{3,})[ \t]*$/.exec(line);
    if (!m) return false;
    return m[1].charAt(0) === opener.char && m[1].length >= opener.len;
  }
