...
(function () {
  ...
  async function rerenderAll() {
    ...
    // rerenderAll() is still zero-argument this round (Task E adds the
    // parameter to this and safeRerenderAll()'s signatures) — there is no
    // `editRange` binding in scope here yet, so this passes `undefined`
    // explicitly rather than a bare `editRange` reference, which would be
    // a ReferenceError. Task E's signature change should update this call
    // site's second argument to the new `editRange` parameter.
    return applyRenderResult(j, undefined);
  }

  // Task F 會在這裡加 patch 分支；這一輪只有 fallback 一條路。
  async function applyRenderResult(j, editRange) {
    return applyFullRender(j);
  }

  async function applyFullRender(j) {
    blocks = j.blocks;
