...
(function () {
  ...
  //   baseDirtyDepth  `stack.dirtyDepth` at open. NOT a second copy of the
  //              stack's state kept in step by hand — it is the one number that
  //              lets the session DERIVE whether a save landed inside it
  //              (fix 1 / F1): with no save, `dirtyDepth` is exactly
  //              `baseDirtyDepth + ops`, because markSaved() is the only other
  //              thing that moves it and nothing else can push while the modal
  //              owns the screen.
  ...
  async function openWaveEditor(diagram) {
    ...
    waveSeam = { blockId: id, startLine: body.startLine, endLine: body.endLine,
                 ...
                 baseDirtyDepth: stack.dirtyDepth, baseRedoTail: stack.redoTail() };
  ...
  function writeWaveGestureBack(info) {
    ...
    if (result.op === null) {
      // Two different things arrive here, and only one of them is "nothing to
      // do". `commitRangeEdit()` returns `op: null` both for text that is
      // already byte-identical AND — tagged `refused` — for a line range it
      // will not touch at all (an inverted one, i.e. a block that owns no
      // source line). Treating the second as "written" would be exactly the
      // silent drop this whole layer exists to prevent.
      //
   ...
   *
   * Returns false when it was NOT, and that answer is DERIVED rather than
   * tracked: with no save inside the session, `stack.dirtyDepth` is exactly
   * `baseDirtyDepth + ops`, because `markSaved()` is the only other thing that
   * moves it and nothing else can push while the modal owns the screen. When it
   * disagrees, a Ctrl+S landed in the middle — the one gesture the modal
   * deliberately does not swallow — and `discardTop()` must not be used:
   * `UndoStack` keeps `_savedDepth` as an absolute index, popping below it
   * makes `dirtyDepth` NEGATIVE, and the next few ordinary commits then walk it
   * back up THROUGH zero. MEASURED before this: paint, Ctrl+S, Escape, then one
   * ordinary paragraph edit — `documentIsDirty()` answered false, the title
   * dropped its `●`, and the conflict banner's Reload raised no dialog at all
   * and destroyed the typed edit.
   *
  ...
  function unwindWaveOps(seam) {
    ...
    if (stack.dirtyDepth !== seam.baseDirtyDepth + seam.ops) return false;
  ...
  function showWaveRefusal(seam, reason) {
    ...
    el = showBanner(
      ...
      '比檔案裡多一步。受影響的是 ' + range + ' 這個 wavedrom 區塊。原因：' + reason,
      '知道了',
      () => {
        if (el === null) return;
        el.remove();
  ...
  async function finishWaveSession(seam, reason) {
    ...
    if (seam.unwritten) {
      ...
      showBanner(
        ...
        '寫得回去的部分（第 ' + seam.startLine + '–' + seam.endLine + ' 行）。',
        null, null);
      waveUnwrittenEdit = false;
      setDirty();
    }
