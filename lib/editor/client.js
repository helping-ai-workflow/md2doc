...
(function () {
  ...
  //               there is none. A PRE-commit start line, and stable across
  //               the render on BOTH of the body's two commit paths — they are
  //               stable for different reasons, so both are stated:
  //                 * the non-li path commits through commitRangeRemoval(),
  //                   which anchors ops.shiftBlocks() on the removed span's own
  //                   last block, so only ids GREATER than that move — and the
  //                   blank it absorbs sits inside or adjacent to the span
  //                   either way, always below this block's own lines.
  //                 * the li path commits through deleteListItemsViaGutter() ->
  //                   commitListStructure(…, { presetRange }), which calls
  //                   NEITHER of those. It re-serializes the run's survivors
  //                   over runRangeOfBlocks()'s own range, and §4.1 refuses a
  //                   multi-line li outright, so the number of lines above the
  //                   removed item cannot change — whether that neighbour is a
  //                   sibling li inside the run or the block before the run.
  //                   (Driven: middle, first, last and whole-run li deletes.)
