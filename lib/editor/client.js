...
(function () {
  ...
  // text" defect). Policy chosen: opening a second block while one is
  // already open is REFUSED outright — the already-open editor gets a
  // brief visual flash as feedback — rather than auto-cancelling it. An
  // auto-cancel-if-unmodified policy was considered but rejected: it needs
  // its own dirty-diff bookkeeping for one extra convenience and still
  // surprises a user who *did* type something, so "always refuse, always
  // predictable" is the simpler and safer contract for Phase 1.
  let activeEditor = null; // { blockEl } | null
  ...
  function openRawEditor(blockEl) {
    ...
    async function commit() {
      ...
      // (this block included), so there's nothing left to track.
      activeEditor = null;
    }
