...
(function () {
  ...
  // setDirty() is push-driven: something has to call it, and its existing
  // call sites all sit at the tail of work that has already finished — a
  // render, a discard, a save, a source-mode exit. MEASURED by keeping the
  // predicate above but removing the watcher below: typing into a paragraph
  // still left the title at "doc", so nothing repaints it while a burst is
  // open. That is the second half of the gap. The dot therefore needs
  ...
  //
  // Attribute mutations are deliberately NOT watched. MEASURED on the
  // gesture that writes one into a burst surface — 🔗 over an existing link,
  // which reaches applyLinkToggleBody()'s `whole.setAttribute('href', …)` —
  // driven through a REAL modal rather than a stubbed window.prompt: the
  // prompt takes focus off the surface before the write, which resolves and
  // commits the burst, so `stack.dirtyDepth` has already moved by the time
  // the attribute lands. Adding `attributes: true` back changed no assertion
  // in the journey rows below. Table selection chrome is an attribute write
  // into the surface too, and burstBaselineHtml() normalises it away.
  //
  // Cost is a burstBaselineHtml() per delivered batch, and only while
  // `stack.dirtyDepth` still reads clean (`||` short-circuits past it
  // otherwise). MEASURED, innerHTML serialize plus its indexOf scans on a
  // rendered table block: 0.02 ms at 7.4 KB, 0.06 ms at 21 KB, 0.19 ms at
  // 71 KB.
