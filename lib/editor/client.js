...
(function () {
  ...
  // coordinate for a block the refusal did not touch; the two exits ABOVE it
  // (the dropped-gesture re-resolve and the no-source-line refusal) leave it
  // undefined, which is the honest answer — in the first the block is gone,
  // in the second its range is inverted and `focusBlockAtLine()` would resolve
  // some other block's startLine. See restoreAfterStructuralOp().
  ...
  function restoreAfterStructuralOp(focusLine, anchorLine) {
    ...
    if (held && held.closest && held.closest('.ed-wys-armed, .ed-wys-cell, .ed-raw')) return;
