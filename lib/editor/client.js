...
(function () {
  ...
  async function commitListStructure(runEl, focusStartLine, caretToEnd, presetRange) {
    ...
    if (!root) { endBurstWithoutResolve(); return false; }
    ...
    if (!range) { endBurstWithoutResolve(); return false; }
    const result = (md === '')
  ...
  // Degrade-never-lose gate for structural keys. A structural op re-serializes
  // the WHOLE run, and serializeList() strips content it cannot represent from
  // `md` (a loose <p>-wrapped item, a foreign element, stray text directly
  // under the UL, unsupported inline markup) — so committing such a run would
  // silently delete that content. Task 7's resolveBurst() can fall back to a
  // partial, single-li commit for a plain text edit; a structural change has
  // no equivalent narrow range (it moves items between levels), so the only
  // safe answer is to refuse the key and leave the DOM untouched.
  ...
  // True iff `listEl` still has at least one <li>. Used instead of a plain
  // childNodes/childElementCount emptiness test because a server-rendered
  // UL/OL also carries marked's pretty-print "\n" text nodes between items
  // (list-md.js's isBlankText() artifacts): after the last real item is moved
  // out, those remain, so "no children" is never true even though the list is
  // semantically empty. See outdentListItem() below.
  function hasLiChild(listEl) {
    for (let i = 0; i < listEl.childNodes.length; i++) {
      if (listEl.childNodes[i].nodeName === 'LI') return true;
    }
    return false;
  }
  ...
  //   2. The emptied parent list must be removed only when it truly has no
  //      <li> left. Testing childNodes.length (or childElementCount) is wrong:
  //      the leading "\n" text node before the moved item always survives, so
  //      the list is never "empty" by node count even when it holds nothing.
  //      hasLiChild() is the correct predicate.
  function outdentListItem(li) {
    ...
    if (!hasLiChild(parentList)) grandLi.removeChild(parentList); // see hazard 2
    return true;
  }
  ...
  function handleLiKeydown(e, editEl) {
    ...
    if (isEmptyListItem(li)) {
      ...
      // (adoption included). isEmptyListItem() keeps refusing items that own a
      // sublist, so this can never orphan children.
      const outdented = mutateListRun(() => {
        // Deleting the last character of a contenteditable leaves Chromium's
        // placeholder <br> behind. isEmptyListItem() sees through it
        // (textContent === ''), but inline-md.js serializes a real <br> as a
        // literal '<br>', so committing the outdented item as-is would write
        // '- <br>' where the user sees an empty item. Clear the surface first
        // (inside the suppression span: an innerHTML assignment on the focused
        // node triggers the very same Chromium unfocus quirk).
        const textEl = liTextEl(li);
        if (textEl !== li && textEl.textContent === '') textEl.innerHTML = '';
