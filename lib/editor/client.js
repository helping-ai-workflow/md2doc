...
(function () {
  ...
  //     於是不叫。
  //
  // 所以改成算數量。兩個計數都只看【新的尾端圍欄本文裡出現過的那些行內容】：
  //
  //   wasOutCount / nowOutCount —— 這些行在上一份／這一份 render 裡，出現在
  //     fenced code block【外面】的次數。外面的份數變少 ＝ 有東西被吃進去了。
  //   prevTotal / nowTotal —— 同一批行在上一份／這一份【整份文件裡】出現的次數。
  //     沒有變少 ＝ 那些行只是換了位置，不是被刪掉。
  //
  // 「總數」刻意只算這一批行、而且連圍欄裡的份數一起算，不是整份文件的行數：
  ...
  function absorbedOutsideContent(prevLines, prevBlocks, newLines, newBlocks, tail) {
    ...
    for (let n = tail.startLine + 1; n <= tail.endLine; n++) {
      ...
      seen[row] = true;
      wasOutCount += was.out[row] || 0;
      nowOutCount += now.out[row] || 0;
      prevTotal += was.all[row] || 0;
      nowTotal += now.all[row] || 0;
    }
    return wasOutCount > nowOutCount && nowTotal >= prevTotal;
  }
  ...
  async function applyRenderResult(j, editRange, scrollY, declaredRange) {
    ...
    // 不見了，不是搬進去）。
    //
    // 這三項各擋掉一族，而且各有一列 journey 案例釘住它 —— 把那一項單獨拿掉，
    // 那一列就紅（機械掃過：判別式、歸屬測試自己的組成、圍欄配對器的每一條，逐
    // 一 ablate 再跑全部案例，記錄哪一列紅）：
  ...
  //    圍欄 '~~~\n```\nx\n```\n~~~' 的 caret 就落在第二行那個 ``` 的行首（本檔
  //    的巢狀圍欄那一列釘的正是這個落點）。在那裡打字得到 'Z```'，那一行仍然
  //    只是外層圍欄的本文，兩層圍欄的配對都不變。
  //
  ...
  // 圍欄的 info string 不得再含反引號。這兩條各修各的：K1 那個被種進空白行的缺
  ...
  //
  // MEASURED（runs/closer-probe.js：把候選行放進 '```\nx\n<候選>\nAFTER\n'，
  // 看 buildBlockMap 有沒有把 AFTER 留在 code block 外面）—— 上一版跟 marked 在
  // 這些行上不一致，兩邊各有一種：
