...
(function () {
  ...
  function writeWaveGestureBack(info) {
    ...
    if (info.patch.ok !== true) {
      ...
      setDirty();
      showWaveRefusal(seam, info.patch);
      return;
    }
    const result = commitRangeEdit({ lines, blocks, stack },
      seam.startLine, seam.endLine, info.patch.text);
    if (result.op === null) {
      // The patch reproduces the bytes already there (an edit and its inverse
      // inside one session cost zero bytes — see wave-store's own note on why
      // toPatch compares documents and not stack entries). Nothing was pushed,
      // so nothing is owed, and any earlier refusal is now moot: the file and
      // the editor agree again.
    ...
    // nothing reads it while the modal owns the screen. The close settles it:
    // a commit re-renders and the server hands back fresh blocks, an Escape
    // restores `baseBlocks`, and both agree with `lines` again.
    seam.ops++;
   ...
   */
  function showWaveRefusal(seam, patch) {
    const where = waveRefusalWhere(seam, patch);
    const span = where.from === where.to
      ? ('第 ' + where.from + ' 行')
      : ('第 ' + where.from + '–' + where.to + ' 行');
    const at = where.at === null ? ''
      : ('，衝突落在這個區塊自己的第 ' + where.at + ' 行');
    ...
    el = showBanner(
      ...
      '比檔案裡多一步。受影響的是 ' + span + ' 這個 wavedrom 區塊' + at +
      '。原因：' + patch.reason,
      '知道了',
      () => {
        if (el === null) return;
  ...
  /**
   * Two coordinates, each stated in the frame it is actually true in.
   *
   * `from`/`to` are DOCUMENT line numbers and name the block body as it stands
   * right now — exact, and the thing the user can go and look at.
   *
   * `at` is a line number WITHIN the block, and it is not the same frame:
   * `rewroteRange`'s offsets index the source the store was opened over
   * (`baseSource`), which earlier write-backs in this same session may already
   * have lengthened. Mapping the offset through the CURRENT body would be off
   * by however many lines those added — so it is mapped through `baseSource`,
   * where it is exact, and reported as a position inside the block rather than
   * as a document line it no longer is. Null when the refusal carried no range
   * (the parse-level ones), rather than inventing one.
   */
  function waveRefusalWhere(seam, patch) {
    const r = patch && patch.rewroteRange;
    const out = { from: seam.startLine, to: seam.endLine, at: null };
    if (!r || typeof r.start !== 'number') return out;
    const src = seam.baseSource;
    const clamped = Math.max(0, Math.min(src.length, r.start));
    out.at = src.slice(0, clamped).split('\n').length;
    return out;
  }
  ...
  async function finishWaveSession(seam, reason) {
    ...
    if (reason === 'escape') {
      ...
      // been committing all along. `discardTop()` is the one stack operation
      // that pops an op AND reverses it without leaving a redo entry: as far as
      // the history is concerned these commits never happened, which is what
      // makes the dirty depth land back exactly where the session found it
      // rather than N-plus-a-revert away from it.
      //
      // The reversal is taken from `baseLines` rather than from discardTop()'s
      // own returned arrays: the two are the same answer, and only one of them
      // can be off by a line. discardTop() is still CALLED for every op,
      // because popping the stack is the half that `baseLines` cannot do.
      const painted = lines.slice(seam.startLine - 1, seam.endLine).join('\n');
      for (let i = 0; i < seam.ops; i++) {
        if (stack.discardTop(lines) === null) break;
      }
      lines = seam.baseLines;
      blocks = seam.baseBlocks;
      if (seam.ops > 0 && painted !== seam.baseSource) {
        ...
        discardedWaveEdit = {
          ...
          startLine: seam.startLine, endLine: seam.startLine +
            seam.baseSource.split('\n').length - 1,
      ...
      // it is still showing exactly the `baseLines` just restored — rendering
