...
(function () {
  ...
  async function openRawEditor(blockEl, range) {
    ...
    const startLine = range ? range.startLine : block.startLine;
    const endLine = range ? range.endLine : block.endLine;
    ...
    if (endLine < startLine) { refuseStructuralListEdit(NO_SOURCE_LINE_MESSAGE); return; }
    ...
    // the run serializer, and tears down through a real re-render.
  ...
  function toggleGutterMenu(blockEl) {
    ...
    // v3.1.0 修正 4: BOTH conditions removed. RULING F-O's premise — that a
    // textarea inside a li is content the run serializer cannot represent,
    // and that restore() would have to rebuild the marker/check/text chrome
    // out of a string — is answered rather than worked around: openRawEditor()
    // now edits an explicit LINE RANGE for a li (its own source line, marker
    // and leading columns included), commits it through commitRangeEdit()
    // instead of the run re-serializer, and restores through rerenderAll()
    // instead of an innerHTML rewrite, so no serializer ever sees the
    // textarea and no chrome is ever rebuilt by hand. The same line-range
    // shape is what makes the multi-select case honest too: over a set of N
    // the editor now opens on the WHOLE span's lines, so the item no longer
    // appears to act on the selection while quietly answering for one block.
    // The membership question itself moved WITH the decision it feeds, into
    // openRawViaGutter() (it now chooses the range instead of the visibility),
    // so it is asked exactly once and in the place that acts on the answer.
  ...
  let toolbarStateSig = '';
  // 追加 4's three-state document mode: 'edit' | 'source' | 'preview'.
  ...
  const TOOLBAR_NO_BLOCK_ABOVE_MESSAGE = '這已經是文件的第一個區塊，上方沒有可插入的位置';
  ...
  function resetToolbarBlock() {
    toolbarBlockEl = null;
    closeToolbarMenu();
    // A render that lands while the document is in preview mode re-arms every
    // surface as editable (armEditables() has no notion of modes), so the
    // mode's one real effect has to be re-applied on the fresh DOM.
    if (docMode === 'preview') applyPreviewEditability(false);
    updateToolbar();
  }
  ...
  function toolbarContext() {
    ...
    return {
      ...
      // The mark buttons act through applyMarkToggle(), which refuses when
      // `selToolbarEditEl` is null or the selection is collapsed — so that is
      // the same question asked here rather than a looser "is anything
      // selected anywhere on the page".
      hasSelection: !!(selToolbarEditEl && sel && sel.rangeCount > 0 && !sel.isCollapsed),
      mode: docMode,
    };
  }
  ...
  function updateToolbar() {
    ...
    Object.keys(toolbarBtns).forEach((id) => {
      ...
      // Only `headings` carries a label (H / H1..H6); every other button
      // keeps the glyph it was built with.
  ...
  async function insertBlockAbove(blockEl) {
    ...
    const preceding = blocks
      ...
      .pop();
    if (!preceding) { showBanner(TOOLBAR_NO_BLOCK_ABOVE_MESSAGE, null, null); return; }
  ...
  // lib/md2doc.js's setSidebarOpen()). v3.1.0 adds the DESKTOP rule that
  // reads it, so this needs no JS beyond the toggle itself; aria-expanded is
  // mirrored onto the reader's button so the two writers agree.
  function toggleOutlineSidebar() {
    const open = !document.body.hasAttribute('data-sidebar-open');
    if (open) document.body.setAttribute('data-sidebar-open', '');
  ...
  function pickAndInsertImage() {
    ...
    input.multiple = true;
    input.hidden = true;
    const anchorEl = liveToolbarBlock() || lastBlockEl();
    document.body.appendChild(input);
    input.addEventListener('change', () => {
      const files = input.files ? Array.prototype.slice.call(input.files) : [];
      input.remove();
      if (files.length) insertImages(files, anchorEl);
    });
    input.click();
  }
  ...
  function wireImageDrop() {
    // dragover MUST be prevented for a drop to fire at all; gated on the
    // drag actually carrying files so a text drag inside a contenteditable
    // surface keeps the browser's own native behaviour.
  ...
  // done — and that case (copy a word out of a browser, paste it mid
  // sentence) must keep landing at the caret rather than becoming a new
  // block. Anything else is real structure and takes the source path.
  const TOOLBAR_INLINE_SAFE_RE = /^[^\n*_`~\[\]<>#|\\!-]*$/;

  function handleArmedPaste(dt, editEl) {
    const plainOnly = plainPasteArmed;
    plainPasteArmed = false;
    const payload = pasteMd
      ? pasteMd.pickPayload(clipboardItemsOf(dt), plainOnly)
    ...
    if (payload.kind === 'markdown') {
      ...
      if (md.trim() === '') return;
      if (TOOLBAR_INLINE_SAFE_RE.test(md)) {
  ...
  function applyPreviewEditability(editable) {
    const surfaces = contentEl.querySelectorAll('.ed-wys-armed, .ed-wys-cell');
    for (let i = 0; i < surfaces.length; i += 1) {
      surfaces[i].setAttribute('contenteditable', editable ? 'true' : 'false');
    }
  }
  ...
  async function setDocMode(next) {
    ...
    if (docMode === 'preview') applyPreviewEditability(true);
    docMode = next;
  ...
  function wireToolbarTracking() {
    ...
    // hasSelection drives the five inline buttons; selectionchange is the
    // only signal that catches keyboard-driven selections too. updateToolbar()
    // diffs before it writes, so an unchanged selection costs nothing.
  ...
  function mountToolbar() {
    ...
    // ⚠ document.body, NOT contentEl. rerenderAll() does
    // `contentEl.innerHTML = j.bodyHtml`, replacing .content's children
    // wholesale, and only armEditables() rebuilds anything afterwards — it
    // knows nothing about a toolbar. A bar mounted inside .content would
