...
(function () {
  ...
  // Esc-during-drag (a distinct gesture from Esc-reverts-burst — see
  // handleTableCellKeydown() above, which owns Escape for a focused cell;
  // the global keydown listener below intercepts Escape BEFORE that branch
  // whenever a drag is actually in flight): discards the in-progress drag
  // with NO mutation — the dragged row was never actually moved (only the
  // indicator line tracked the pointer), so there's nothing to revert.
