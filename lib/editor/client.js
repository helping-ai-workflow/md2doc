...
(function () {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // would have had it.
    //
    // The modifier exclusions are what that rule needs to stay usable, and
    // each was ablated on its own rather than reasoned about as a set:
    //
    ...
    //   Control / Meta — driven singly, neither changes anything observable.
    //           With an edit pending, Ctrl+S ends the mode and writes the file
    //           with the exclusion in place and with it taken out alike,
    //           because the `s` behind the modifier is unnamed here and hands
    //           back before the save branch below runs. They are kept for the
    //           shape of the set, and they are the entries here with no row of
    //           their own.
    //
