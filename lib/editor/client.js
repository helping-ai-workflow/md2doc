...
(function () {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // ── v3.4.0 batch3 Task 6: the wave editor owns the keyboard ────────
    // Above everything, including the toolbar's keyboard mode. While that
    // overlay is up it is modal over the document, and every branch below this
    // line is about a document the user cannot see or reach — including the
    // `Tab` branch, which used to swallow the key and leave all 44 controls of
    // an `aria-modal` dialog unreachable. The overlay's own capture-phase
    // handler owns Escape, Tab and undo/redo and stops them before they get
    // here; this is the other half of that deal, stated on this side so the two
