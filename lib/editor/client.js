...
(function () {
  ...
  function wireBlockSelection() {
    ...
    document.addEventListener('click', async (e) => {
      ...
      // v3.4.0 batch3 Task 6: the wave editor's overlay and its hover entry
      // button are both on document.body for the same reason `.ed-conflict`
      // is — nothing this editor owns may sit inside `.content`, which is
      // serialised back into the user's markdown. Without these two
      // exclusions a click on either falls through to "clicked outside any
      // block" below and fires switchAwayFrom() on the document BEHIND the
      // modal, which is the same race the `.ed-conflict` line above exists
      // for; it also commits a dirty burst after openWaveEditor() has already
      // read `lines`, so the line range the wave editor was opened on can be
      // stale the moment it is written.
      if (e.target.closest('.ed-wave-overlay')) return;
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // ── v3.4.0 batch3 Task 6: the wave editor owns its own keys ────────
    // Above everything, including the toolbar's keyboard mode. While that
    // overlay is up it is modal over the document, and every branch below
    // this line is about a document the user cannot see or reach. Ctrl+Z was
    // the one that mattered — MEASURED: pressed inside the overlay it undid
    // the MARKDOWN DOCUMENT behind the modal (a committed paragraph edit
    // vanished while the overlay stayed up) and rewrote `lines`, which also
    // invalidates the line range the overlay was opened on. The overlay's own
    // capture-phase handler claims Escape and undo/redo and stops them here;
    // this is the other half of that deal, stated on this side so the two
    // cannot drift into disagreeing about who owns what.
    if (waveEditorOwns(e.target)) return;
  ...
  /**
   * Whether the wave editor owns this event.
   *
   * The one place the rest of this file asks, instead of assuming. `true`
   * whenever the overlay is up AND the event came from inside it — or from
   * nothing at all, which while a modal is on screen is the modal's too.
   *
   * A function declaration on purpose: it is hoisted, so the keydown listener
   ...
   */
  function waveEditorOwns(target) {
    if (waveEditor === null) return false;
    if (target === null || target === undefined || typeof target.closest !== 'function') {
      return true;
    }
    return target.closest('.ed-wave-overlay') !== null;
  }
