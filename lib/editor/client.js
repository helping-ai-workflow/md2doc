...
(function () {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    //
    // Gated on the BLOCK, not on `blockSelection`: the focus holder is the
    // same `.ed-block` wrapper in both of the states that can get here — a
    // standing selection (whose roving `tabindex` focuses it) and the focus
    // this file hands back when a wave session closes — so one branch serves
    // the way in and the way back in. `isTextEntryTarget` keeps it away from
    // every editable, where Enter already means a paragraph break, and the
    // modal gate far above means it cannot fire while the overlay is up.
    if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey &&
        !isTextEntryTarget(e.target) && e.target && typeof e.target.closest === 'function') {
      const blockEl = e.target.closest('.ed-block');
