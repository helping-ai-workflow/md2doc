...
(function () {
  ...
  async function performRowDrop(tableEl, rowEl, dropTarget) {
    // Task 5 adapter: dropTarget is now the nearestRowDropTarget() union
    // ({mode:'above-header'|'before-row'|'append', ...}); this task only
    // adds the ABOVE-HEADER target and its threshold, the actual
    // header-promotion rewrite of this function's body is Task 6. Until
    // then, collapse the union back down to the DOM-node `beforeRow` shape
    // the rest of this function (unchanged below) already expects —
    // `above-header` falls through to the existing append/insert behaviour
    // for now.
    const beforeRow = dropTarget.mode === 'before-row'
      ? allRowsOf(tableEl)[dropTarget.rowIndex] : null;
