...
(function () {
  ...
  // position.
  function applyIndentClamp(spanEls, opBlockEl, opOldIndent) {
    ...
    indentClamp.clampIndents(model, opIndex, opOldIndent, {}).forEach((r) => {
  ...
  function armEditables(root) {
    ...
    blockEls.forEach((blockEl) => {
      ...
      if (editEl && (blockType === 'paragraph' || blockType === 'heading') &&
        ...
        //
        // The ＋ deliberately stays out until S2: insertBlockBelow() writes
        // a bare skeleton line at endLine + 1 with no indent awareness and no
        // run gate, so on a PARENT item it lands between the parent and its
        // children at column 0 and orphans them. The button is not created at
        // all (rather than created-and-hidden) so there is nothing for a
        // stray CSS rule or a keyboard user's tab order to reveal early.
  ...
  let gutterMenuMinus, gutterMenuPlus, gutterMenuMd;
  ...
  function buildGutterMenu() {
    ...
    // Hoisted to a module-level binding (like the heading ± pair above) so
    // toggleGutterMenu() can hide it per block type — see there.
    gutterMenuMd = document.createElement('button');
    const mdBtn = gutterMenuMd;
    mdBtn.type = 'button';
  ...
  // hard-wrapped block in the commit span, mapped to the source lines it owns
  // right now, so listMd.serializeBlocks() can replay them instead of running
  // them back through the (measurably lossy) inline round trip. See its own
  // carryOver comment for the measurement.
  //
  // Two exclusions, both about "whose bytes are authoritative":
  //
  ...
  //    item owns no source lines at all, and a same-line nest's outer item has
  //    endLine === startLine - 1 (blockOwnsNoLine()). Neither has bytes to
  //    replay, and slicing an inverted range would hand back the WRONG line.
  function bystanderCarryOver(span) {
    const probe = listMd.serializeBlocks(span);
    const ids = probe.multiLineBlockIds || [];
    ...
    let any = false;
    ids.forEach((raw) => {
      if (raw === null || raw === undefined || raw === dirtyId) return;
      const rec = blocks.find((b) => b.id === Number(raw));
  ...
  async function commitListStructure(runEls, focusStartLine, caretToEnd, presetRange) {
    const span = runEls || [];
    const { md } = listMd.serializeBlocks(span, { carryOver: bystanderCarryOver(span) });
  ...
  function runLineOfBlock(runEls, targetBlock) {
    ...
    // The SAME carryOver commitListStructure() will use: a replayed bystander
    ...
    const { lineMeta } = listMd.serializeBlocks(runEls, { carryOver: bystanderCarryOver(runEls) });
  ...
  function listRunSupportsStructuralEdit(runEls, targetEl, opts) {
    ...
    return multi.indexOf(targetEl.getAttribute('data-block-id')) === -1;
  }
  ...
  function handleBurstKeydown(e, editEl) {
    ...
    // Task 6 — spec §3.5's other two rows. Tab is CONSUMED on every armed
    // surface, whatever the block type: the alternative is not "nothing
    // happens", it is the browser's own focus traversal walking the caret out
    // of the document body, which is both a surprise and (because it fires
    // focusout) an unasked-for commit.
    //
    ...
    //
    // Table cells are not reachable from here at all — they run through
    // handleTableCellKeydown(), whose Tab keeps its cell-navigation contract.
    if (e.key === 'Tab') {
      e.preventDefault();
  ...
  function wireBlockSelection() {
    ...
    document.addEventListener('click', async (e) => {
      ...
      if (checkEl) {
        ...
        // the item is the operation target (spec §4.1) — a hard-wrapped one
        // refuses, its siblings do not.
        if (!listRunSupportsStructuralEdit(run, li)) { refuseStructuralListEdit(); return; }
        ...
        if (!listRunSupportsStructuralEdit(targetRun, targetLi)) { refuseStructuralListEdit(); return; }
