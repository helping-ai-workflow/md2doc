...
(function () {
  ...
  // `{ prevLine, holeLine }` the body captured BEFORE its commit:
  //
  //   prevLine  the block immediately ABOVE the removed span, or null when the
  //             span started at the document's first block. It is a PRE-commit
  //             start line and it is stable across the render: commitRangeRemoval()
  //             anchors ops.shiftBlocks() on the removed span's own last block,
  //             so only ids GREATER than that move — and the blank the removal
  //             absorbs sits inside or adjacent to the span either way, always
  //             below this block's own lines.
  //   holeLine  the removed span's own first start line, used only for the
  //             null-prevLine case.
  //
  ...
  // block BELOW the hole then moves up too, the first of them lands on
  // `holeLine - 1`, and taking the maximum picks THAT deterministically over
  // the real previous block. Driven on '…Bravo para.\n' immediately followed by
  ...
  //
  // When there is no previous block the deleted span WAS the document's first,
  // so whatever is first now is the block that moved up into it — `blocks[0]`,
  // no search needed. Answers null for a delete that emptied the document.
  ...
  async function deleteBlockViaGutterBody(blockEl) {
    ...
    // element before recs[0] is that neighbour; null when the span starts at
    // the document's first block.
    const at = blocks.findIndex((b) => b.id === recs[0].id);
    ...
    if (shape.allLi) {
      return (await deleteListItemsViaGutter(els))
        ? { prevLine: prevLine, holeLine: recs[0].startLine } : undefined;
    }
    ...
    return { prevLine: prevLine, holeLine: first.startLine };
  }
  ...
  async function applyLinkToggle() {
    ...
    try {
      ...
      if (blurred) {
        await blurred;
        if (switching) await switching;
        // Fix round 1, item 1: `willPrompt` guarantees the modal OPENS, not
        // that the user confirmed it. On Esc / cancel the body does no DOM
