...
(function () {
  ...
  // below seeing `{stale:true}`. Matches ANY part whose baked markup gained
  // OR lost the drawio wrapper div (covers the file being deleted mid-session
  // too — drawioPlaceholderFor() then falls through to an ordinary broken
  // <img>, which is just as much "changed" as a re-bake).
  //
  ...
  // only ever touches blocks whose OWN baked output actually differs, never
  // reassigns `blocks`/`lines` (the source text did not change, only a
  // referenced file's bytes did), and never moves focus.
  const DRAWIO_BLOCK_RE = /class="drawio(?:["\s])/;
  ...
  // session. The comment this replaces promised that retry; the code did not
  // implement it.
  ...
  function noteDrawioPage(message) {
    if (activeBanner) { drawioDeferredNotice = message; return; }
    drawioDeferredNotice = null;
    showBanner(message, null, null);
  }
  function flushDrawioNotice() {
    if (!drawioDeferredNotice || activeBanner) return;
    const m = drawioDeferredNotice;
    drawioDeferredNotice = null;
  ...
  function drawioBlockHoldsEditingFocus() {
    ...
    for (let i = 0; i < kids.length; i++) {
      ...
      return !!(kids[i].querySelector && kids[i].querySelector('.drawio'));
    }
    return false;
  }
  ...
  async function refreshStaleDrawio(token) {
    ...
    // Review question Q2: with F2's retry in place a stale signal costs a
    // full /api/render (Chromium bake included) every time it is answered, so
    // the cheap "there is nothing on screen this could possibly change"
    // answers are worth giving before the fetch rather than after it. None of
    // them acks: nothing was applied, so the next tick must ask again.
    ...
    try {
      ...
      // pre-fetch check above cannot speak for the time the render took.
      if (!domMatchesLastRender()) return;
      ...
      for (let i = 0; i < j.parts.length; i++) {
        if (j.parts[i] === lastParts[i]) continue;
        if (!DRAWIO_BLOCK_RE.test(j.parts[i]) && !DRAWIO_BLOCK_RE.test(lastParts[i])) continue;
        ...
        // focused". Not applied, so not acked: the next tick tries again.
        ...
        const oldBox = el.querySelector ? el.querySelector('.drawio[data-drawio-pages]') : null;
        let oldNames = [], activeIdx = -1, hadMultiPage = false;
        if (oldBox) {
          try { oldNames = JSON.parse(atob(oldBox.getAttribute('data-drawio-pages') || '')); }
          catch (e2) { oldNames = []; }
          if (!Array.isArray(oldNames)) oldNames = [];
          hadMultiPage = oldNames.length >= 2;
          if (hadMultiPage) {
            const oldPages = oldBox.querySelectorAll('.drawio-page');
            const activePage = oldBox.querySelector('.drawio-page:not([hidden])');
            activeIdx = activePage ? Array.prototype.indexOf.call(oldPages, activePage) : -1;
          }
        }

        const tpl = document.createElement('template');
        tpl.innerHTML = j.parts[i];
        ...
        const newBox = fresh.querySelector ? fresh.querySelector('.drawio[data-drawio-pages]') : null;
        if (!newBox) continue;
        ...
        let newNames = [];
        try { newNames = JSON.parse(atob(newBox.getAttribute('data-drawio-pages') || '')); }
        catch (e3) { newNames = []; }
        if (!Array.isArray(newNames)) newNames = [];
        const newPages = newBox.querySelectorAll('.drawio-page');
        ...
        const bar = newBox.querySelector('.drawio-sheetbar');
        if (bar) {
      ...
      if (applied && typeof token === 'string' && token !== '') drawioAckPending = token;
    } finally {
