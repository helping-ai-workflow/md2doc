...
(function () {
  ...
  // carries none.
  //
  // No markdown MEASURED so far reaches the second branch: across the 131
  // distinct strings a full journey run hands buildBlockMap(), plus 1231
  // trailing-space variants of them, zero produced an `unlocatable` record.
  // The wording below was driven by forcing the flag on (blockmap.js stubbed to
  // set it), which showed this banner where the '- - a' outer still shows the
  // other one. It is a net for a shape nobody has produced yet, not a path.
  ...
  // The degraded-subtree wording (see blockRangeIsUnlocatable() above). Split
  // the same two ways as the pair above, and for the same reason: an insert
  // must not answer with a sentence about deleting.
  const UNLOCATABLE_LINE_MESSAGE = '這段巢狀清單對不到自己的來源行，已暫停編輯以免覆蓋別的項目';
