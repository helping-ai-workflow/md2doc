...
(function () {
  ...
  // NEAREST boundary are both positioned OUTSIDE the same table edge, so
  // with too small a gap their hit rects intersect — e.g. hovering a few px
  // inside the table's left edge, just below a row's top boundary, used to
  // show BOTH the row grip (any body-row-cell hover) and the row insert
  // bubble (within TB_EDGE_PX of that same boundary), and the grip's higher
  // z-index silently ate a click aimed at "insert row here". Fixed
  // structurally, not by fiddling pixel offsets per table: the bubble's
  // own hit box extends TB_BUBBLE_SIZE/2 (9px) past the table edge on
  // whichever axis it sits on (x for the row bubble/grip pair, y for the
  // column pair) — as long as TE_GRIP_GAP_PX (the grip's near edge, i.e.
  // its distance from the table edge) is greater than that 9px, the two
  // rects are separated on THAT axis alone, which is sufficient for
  // non-intersection regardless of the other axis (row height / column
  // width) — see updateTableEdgeGrips() below for the position math this
  // guarantee depends on. 14 gives a 5px margin past the 9px minimum.
  // test/editor-client-runtime.test.js's "grip and insert-bubble hit zones
  // never overlap" scenario asserts this holds via a live
  // getBoundingClientRect() intersection check — re-verify it if either
  // this constant or TB_BUBBLE_SIZE ever changes.
  const TE_GRIP_GAP_PX = 14; // gap, in CSS px, between a grip and the table edge it sits outside of
  ...
  // UNREACHABLE by a real pointer. Root cause — both grips sit
  // TE_GRIP_GAP_PX (14px) OUTSIDE the table (see that constant's own
  // comment), so a pointer travelling from inside a cell toward a grip
  // necessarily crosses that 14px corridor OUTSIDE the table on the way.
  // The naive hit test below ("on a cell, or hide") hid the grip the
  // instant the pointer left the table/cell — BEFORE it ever reached the
  // grip — so only a teleporting click (every existing test used
  // pressReleaseAt()/gripCenter(), which jump straight to the grip's own
  // coordinates) could ever land on it; a real mouse gesture could not.
  // Fixed by treating the pointer as "still on the table's grip zone" for
  // as long as it stays within the UNION of: the table's own rect expanded
  // outward by exactly the grip's own footprint (its width + the gap, on
  // the left; its height + the gap, on the top — the two directions each
  // grip sits outside of), and the grips' own current rects (belt-and-
  // braces, e.g. sub-pixel rounding at the very corner). This keeps a grip
  // that's already shown from disappearing mid-travel, while a pointer that
  // genuinely leaves the table (past the corridor, in any direction) still
  // hides it via hideTableGrips() same as before. It deliberately does NOT
  // touch either grip's position/size, so the a490f6c grip/bubble
  // non-intersection guarantee (see TE_GRIP_GAP_PX's own comment) is
  // unaffected — this only changes how long a grip already shown STAYS
  // visible, never where it sits. See
  // test/editor-client-runtime.test.js's "grip reachability by a REAL
  // (non-teleporting) pointer" scenario.
  function pointInTableGripZone(x, y, tableEl) {
    const r = tableEl.getBoundingClientRect();
    const rgw = rowGrip.offsetWidth || 20;
    const cgh = colGrip.offsetHeight || 24;
    const left = r.left - rgw - TE_GRIP_GAP_PX;
    const top = r.top - cgh - TE_GRIP_GAP_PX;
    if (x >= left && x <= r.right && y >= top && y <= r.bottom) return true;
    if (!rowGrip.hidden) {
      const gr = rowGrip.getBoundingClientRect();
      if (x >= gr.left && x <= gr.right && y >= gr.top && y <= gr.bottom) return true;
    }
    if (!colGrip.hidden) {
      const gc = colGrip.getBoundingClientRect();
      if (x >= gc.left && x <= gc.right && y >= gc.top && y <= gc.bottom) return true;
    }
    return false;
  }
  ...
  function updateTableEdgeGrips(x, y, target) {
    ...
    if (!onValidCell) {
      ...
      // corridor toward a grip that's already shown for the last-hovered
      // table. Keep that grip up while still in the corridor; only actually
      // hide once the pointer has left the zone entirely.
      const activeTableEl = gripColTableEl || gripRowTableEl;
      if (activeTableEl && document.body.contains(activeTableEl) && pointInTableGripZone(x, y, activeTableEl)) return;
      hideTableGrips();
      return;
    }
