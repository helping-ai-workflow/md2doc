...
(function () {
  ...
  function showColumnMenu(tableEl, colIndex) {
    ...
    // Task 9 fix round 1 (Ruling T9-5): this line has no floor — unlike
    // the Math.max(0, top) inside positionTeMenu(), called just above,
    // r.top - offsetHeight - TE_MENU_GAP_PX can go negative once the
    // header row's own top approaches the viewport's top edge, and
    // nothing here stops it. Left unclamped on purpose: measured on this
    // branch by actually clicking the col grip (not just hovering) while
    // sweeping the header's on-screen top down toward the fixed .ed-toolbar
    // (44px tall, z-index 101) — the resulting menu's own top stayed
    // positive down to a header top around 45px (menu top around 6px), and
    // clicking stopped landing on the grip at all — document.elementFromPoint
    // at the grip's own centre resolves to .ed-toolbar instead — starting
    // at a header top around 40px, before the header top could ever drop
    // low enough to push this line negative. A keyboard or synthetic-event
    // path that skips real hit-testing could still reach this line with a
    // negative r.top — this comment is why the guard-looking gap stays
    // open rather than getting a floor clamp added defensively.
  ...
  function showRowMenu(tableEl, rowEl) {
    ...
    // instead and is a separate, already-floored code path — see the
    // comment on positionTeMenu()'s Math.max(0, top) and the one above
    // colGrip.style.top for why that path is not reachable by mouse.
  ...
  function updateToolbar() {
    ...
    // early-return below, because that signature is built from the fixed bar's
    // buttons and .ed-seltb has one — U — that no fixed-bar button speaks for:
  ...
  // the source-mode override that turns off literally everything else).
