...
(function () {
  ...
  async function convertBlockViaMenu(blockEl, target) {
    ...
    // S2 Task 5 routes non-list → list here, with §4.3 rule 2's looseness
    // policy (eat the separator when an adjacent block is a li of the same
    // indent and the same list type, or the merged list goes LOOSE and the
    // whole run degrades read-only). Until then it REFUSES rather than commit
    // a shape this task cannot produce correctly.
