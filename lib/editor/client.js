...
(function () {
  ...
  async function applyPatch(j, plan, declaredRange) {
    ...
    // v3.2.1: undo/redo 的焦點修復。實測 undo/redo 並非普遍掉焦點 ——
    // 游標在被改寫區間【之外】時 activeElement / 所屬 block / caret offset
    // 全部存活（連一次讓自己 data-block-id 8→7 的結構性 undo 都扛得住）。
    // 只有游標落在被改寫區塊【內】才掉到 BODY，之後打字被靜默丟棄。
    // 所以修復只在區間內時觸發，且完全不需要 op→block 映射（實測 op.startLine
    // 在 14 個實例中只有 4 個正確，對 move / insert 都指向沒有擁有者的空行）。
    //
    ...
    if (focusWasInSpan) {
      ...
      // （夾在新中段的範圍內），caret 置尾。
      //
      // 位置刻意在 teardown 之前：收尾最後那一段「patch 收尾的焦點檢查」會
      // 把 activeElement 認養成一個新的 burst，打字才有人追蹤（實測沒有這一
      // 步時 undo 之後打的字連磁碟都到不了）。放在收尾之後就晚了。
