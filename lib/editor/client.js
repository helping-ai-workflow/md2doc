...
(function () {
  ...
  // unsaved" — setDirty() (the title dot) and toolbarContext() (the `save`
  // button's own lit/grey state) both ask this instead of each carrying its
  // own copy of the expression. Do not inline this back into either call
  // site; that is exactly the drift this function exists to prevent.
  ...
  //
  // What captureCaretForSave() saves: which `.ed-block` the caret was in
  // (`data-block-id` — stable across a same-shape commit, since
  ...
  // characters into that block's own rendered text the caret sat (every
  // text node before it, concatenated — the same measure
  // `Range.toString().length` gives over a selectNodeContents() range
  // clipped to the caret's own point). restoreCaretForSave() reverses it:
  // walk the (possibly brand-new) block's text nodes until that many
  // characters are consumed, and collapse a Range there. It is a no-op
  // whenever the caret was never stranded in the first place (the common
  // case — a clean document, or a burst with nothing uncommitted, never
  // touches the DOM at all) — checked by asking whether focus is still on
  // BODY afterwards, not by trying to predict which of the paths above ran.
  function captureCaretForSave() {
    ...
    measure.setEnd(anchor, sel.anchorOffset);
