...
(function () {
  ...
  async function refreshStaleDrawio() {
    ...
    try {
      ...
      // than swap the wrong child; the next 10s tick tries again once it
      // settles.
      ...
      for (let i = 0; i < j.parts.length; i++) {
        ...
        if (!el) continue;
        // Never yank focus (or an open raw-source editor surface) out from
        // under the user for a refresh nobody asked for.
        if (el.contains(document.activeElement)) continue;

        // Ruling B2-2 (c): preserve the reader's currently-open sheet-bar
        // page by NAME, not index — a renamed page must count as "gone" the
        // same as a deleted one — falling back to page 0 with a visible
        // banner when it no longer exists. Only pages actually offered a
        // multi-page bar (>= 2 names) carry this state at all.
        const oldBox = el.querySelector ? el.querySelector('.drawio[data-drawio-pages]') : null;
        let keepName = null, hadMultiPage = false;
        if (oldBox) {
          let oldNames = [];
          try { oldNames = JSON.parse(atob(oldBox.getAttribute('data-drawio-pages') || '')); }
          catch (e2) { oldNames = []; }
          hadMultiPage = Array.isArray(oldNames) && oldNames.length >= 2;
          if (hadMultiPage) {
            const oldPages = oldBox.querySelectorAll('.drawio-page');
            const activePage = oldBox.querySelector('.drawio-page:not([hidden])');
            const activeIdx = activePage ? Array.prototype.indexOf.call(oldPages, activePage) : -1;
            if (activeIdx >= 0) keepName = oldNames[activeIdx];
          }
        }
        ...
        if (hadMultiPage && window.__md2docInitDrawioSheetBar) {
          ...
          if (newBox) {
            ...
            if (foundAt < 0) {
              showBanner('目前檢視的 drawio 分頁已不存在（可能已刪除或改名），已切回第一頁。', null, null);
            }
          }
        }
      }
  ...
  setInterval(() => {
    ...
    //
    // v3.4.0 batch2 Task 6 (Ruling B2-2): `fileId` lets server.js answer
    // whether any `.drawio`/`.xml` this document references changed on disk
    // since the last bake — see its own comment on /api/ping. A 204 (no
    // body) means "nothing stale", the same outcome this heartbeat always
    // had; a 200 with `{stale:true}` is new and triggers a background
    // refresh. Every failure mode (network error, non-200/204 status,
    // malformed JSON) is swallowed exactly like the old bare ping already
    // swallowed everything — a missed signal just waits for the next 10s
    // tick, which is the accepted worst-case latency for this whole feature.
    fetch('/api/ping', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ fileId: ED.fileId }),
    }).then((res) => {
      if (res.status !== 200) return null;
      return res.json().catch(() => null);
    }).then((j) => {
      if (j && j.stale) refreshStaleDrawio().catch(() => {});
    }).catch(() => {});
  }, 10000);
