...
(function () {
  ...
  // diverged past the save marker it is invalidated and the getter answers
  // `_done.length + 1`, never 0. Either way the answer moves when an op is
  // pushed onto or popped off the stack, or when a save re-baselines it. The burst substrate keeps what the user typed in
  ...
  // ── save ───────────────────────────────────────────────────────────────
  // fix 3: which save request is the newest. See save()'s own comment.
  let saveSeq = 0;
  ...
  async function save() {
    ...
    // …and which save this is. There is no in-flight lock (Ctrl+S is not
    // debounced and the modal deliberately does not swallow it), so two requests
    // can be outstanding, and a LATER one's bytes are the ones disk ends up
    // holding. An earlier reply arriving afterwards must not re-baseline over
    // it, nor hand back an older `mtimeMs` — both would be a claim about a file
    ...
    if (seq !== saveSeq && res.status === 200) return; // superseded; the newer save owns the state
    if (res.status === 200) {
      let j;
      try {
        j = await res.json();
