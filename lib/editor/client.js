...
(function () {
  ...
  async function applyRenderResult(j, editRange, scrollY, declaredRange) {
    const plan = (editRange && window.md2docPatchmap && domMatchesLastRender())
      ...
      : null;
    if (plan) return applyPatch(j, plan, declaredRange);
    return applyFullRender(j, scrollY, declaredRange);
  }
  ...
  async function changeHeadingDepth(blockEl, delta) {
    ...
    // — see the note there for why it is `blockSelection` and not operand
    ...
    if (!blockSelection) {
      focusBlockAtLine(result.op.startLine, true);   // caretToEnd
      reaimToolbarBlockAtLine(result.op.startLine);
    }
  }
  ...
  // that it sits at the function's EXIT and every exit reaches it — including
  // the three early refusal gates inside the body (`if (!operands) return`, the
  // mixed-list refusal, the withheld-type refusal), all three of which are
  // reachable from a lit toolbar button. The body returns the operated block's
  // post-commit start line, or undefined when it refused or wrote nothing.
  async function convertBlockViaMenu(blockEl, target) {
    let focusLine = null;
    try {
      const out = await convertBlockViaMenuBody(blockEl, target);
      ...
      if (!blockSelection && focusLine != null) {
        focusBlockAtLine(focusLine, true);   // caretToEnd
      }
      // 降級目標（quote / code）沒有可聚焦編輯面，focusBlockAtLine 會安靜
      // no-op；把 toolbarBlockEl 指回轉換後的 block，工具列才不會塌成 4 顆。
      if (!blockSelection) reaimToolbarBlockAtLine(focusLine);
    }
  }

  async function convertBlockViaMenuBody(blockEl, target) {
  ...
  //  2. 原件的行號可以證明沒動，副本的行號則【不可定址】：副本帶著原件的
  //     data-block-id（那正是 bystanderCarryOver() 逐位元組重播它、而不是重新
  //     escape 的原因），所以 li 路徑上 runLineOfBlock() 的
  //     `lineMeta.findIndex((m) => m.blockId === targetId)` 只會找到原件那一行。
  //  3. 畫面上沒有東西在使用者底下移動：副本長在下面，原件原地不動。
  async function duplicateBlockViaMenu(blockEl) {
    let focusLine = null;
    try {
      const out = await duplicateBlockViaMenuBody(blockEl);
      if (out != null) focusLine = out;
    } finally {
      if (!blockSelection && focusLine != null) {
        focusBlockAtLine(focusLine, true);   // caretToEnd
  ...
  async function duplicateBlockViaMenuBody(blockEl) {
  ...
  async function deleteBlockViaGutter(blockEl) {
    let holeLine = null;
    try {
      const out = await deleteBlockViaGutterBody(blockEl);
      if (out != null) holeLine = out;
    } finally {
      const focusLine = holeLine != null ? blockLineAfterRemoval(holeLine) : null;
      if (!blockSelection && focusLine != null) {
        focusBlockAtLine(focusLine, true);   // caretToEnd
      }
      // 落點是 quote / code 這種沒有可聚焦編輯面的型別時 focusBlockAtLine 會
      // 安靜 no-op；把 toolbarBlockEl 指回去，工具列才不會塌成 4 顆。
      if (!blockSelection) reaimToolbarBlockAtLine(focusLine);
    }
  }

  // The block the user is left holding once a span has been removed: the one
  // that ENDS just above the hole, or — when the removed span was at the very
  // top of the document — the one that has moved UP into it. Answers null for a
  // delete that emptied the document.
  //
  // 「上面那一個、游標落在它的結尾」是 Notion 的答案，也是 Backspace 併行的
  // 落點；這個編輯器的 ⠿ / 建立副本 / 轉換成 整組詞彙都是 Notion 的。
  //
  // 讀的是【render 之後】的 `blocks`：刪除把洞以下每一個 block 的 startLine
  // 都往上搬了，只有這一份陣列描述現在真的存在的文件。比較用嚴格小於，所以
  // commitRangeRemoval() 吸收掉的那一行空白落在洞的上面或下面都不改變答案 ——
  // 真的有前一個 block 時它的 startLine 一定小於這個座標（洞【以上】的行號
  // 不會因為刪除而改變），而刪掉的是文件第一個 block 時沒有任何 startLine
  // 小於它，答案就自動落到「移上來的那一個」。
  function blockLineAfterRemoval(holeStartLine) {
    let prev = null;
    let next = null;
    blocks.forEach((b) => {
      if (b.startLine < holeStartLine) {
        if (!prev || b.startLine > prev.startLine) prev = b;
      } else if (!next || b.startLine < next.startLine) next = b;
    });
    const landing = prev || next;
    return landing ? landing.startLine : null;
  }

  async function deleteBlockViaGutterBody(blockEl) {
    ...
    if (shape.allLi) {
      // recs[0].startLine is the hole's coordinate on this path too: the run's
      // survivors are re-serialized over the RUN's own line range, so no line
      // above the removed span moves.
      return (await deleteListItemsViaGutter(els)) ? recs[0].startLine : undefined;
    }
  ...
  async function runCycleAlign() {
    ...
    requestAnimationFrame(() => {
      ...
      // are now stale relative to the header cell; reposition unconditionally
      // (same math as showColumnMenu(), skipping its toggle-closed branch —
      // that branch would wrongly close a menu that never actually asked to
      // close). A no-op scroll makes this a no-op reposition (same numbers
      // back).
      const headerRow = headerRowOf(liveTableEl);
      const cell = headerRow ? headerRow.cells[colIndex] : null;
      if (cell) {
  ...
  // The other five inline-format buttons never hit this because none of them
  // opens a modal, so none of them ever blurs.
  //
    ...
    try {
      ...
      if (blurred) {
        ...
        if (!blockSelection && line != null) {
          ...
          // 一個 TABLE 儲存格裡的連結是這裡唯一還原不回 caret 的形狀，而且是
          // 實測的：`root` 是 CELL，但 `line` 只能指到整張表這個 block，而
          // blockContentEl() 對 table 回傳 firstElementChild ＝ <table> 本身，
          // 它沒有 tabindex、focus() 是 no-op。實測結果因此是 journey 的
          // bar-only 形狀：連結正確寫進磁碟、工具列 15 顆仍瞄著那張表，但
          // activeElement 是 BODY。reaim 是這個形狀下唯一還拿得回來的東西 ——
          // 沒有它工具列會塌成 4 顆。
