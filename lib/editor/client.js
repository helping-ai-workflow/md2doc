...
(function () {
  ...
  function showBanner(message, actionLabel, onAction) {
    ...
    // and `.ed-toolbar-menu` (toggleToolbarHeadingMenu(): `top = r.bottom + 4`,
    // i.e. right at that floor) already anchor themselves to when there is no
    // room above. Any of those four dynamic fixed overlays could therefore be
  ...
  // reported, and never tested. It also runs into backlog #5 (still open):
  // the raw editor's own Escape discards an uncommitted edit, and
  // auto-opening one right after a DESTRUCTIVE gesture (duplicate/delete)
  // is not free the way it is right after a plain conversion — a user who
  // reflexively presses Escape to back out of a screen they did not expect
  // would lose whatever that textarea held. Narrowed to
  // convertBlockViaMenu() only, which is the one call site backlog #6
  // actually names; widening it to the other three is a separate decision
  // this task did not make and should not make silently.
