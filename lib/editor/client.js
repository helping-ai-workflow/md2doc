...
(function () {
  ...
  // commands. md2doc binds no formatting shortcut of its own: every
  // `ctrlKey || metaKey` branch in this file (twelve of them, counted) tests
  // only `Enter`, `z`, `y`, shift+`Z`, `s`, or shift+`v`/`V` — undo, redo,
  // save, plain-paste arming and the raw editor's commit. So Ctrl+B reaches
  // the contenteditable as the browser's own execCommand and does not pass
  // through any snapBurstIfActive() call site.
  //
  ...
  // below the native-command equivalent of the `*-pre` / post snapshots the
  // twelve programmatic call sites take.
  //
  ...
  document.addEventListener('input', (e) => {
    ...
    if (NATIVE_FORMAT_INPUT_TYPES.has(e.inputType)) {
      ...
      // back to noteTyping(). noteTyping() only marks a pending capture, so
      // the bold would land in the history only when the NEXT keystroke
      // crosses the 400ms threshold, or when undo()'s own flushTyping() takes
      // it — by which time the capture has grown to include everything typed
      // since.
      //
      // MEASURED on the gesture that exposes it: Ctrl+B, then keep typing
      // INSIDE the 400ms window (measured gap 16ms), then Ctrl+Z.
      //
      //   snap('native-format')      undo → "Alpha paragraph. <b>typed</b> sentence"
      //   noteTyping() instead       undo → "Alpha paragraph. typed sentence"
      //
      // i.e. without this line the bold is undone together with everything
      // typed after it — this task's own defect with a different prefix. The
      // journey row named 「原生 Ctrl+B 後【400ms 內】繼續打字」 pins it.
      //
      // Note the earlier four rows (bold, then a 250ms pause, then Ctrl+Z) do
      // NOT pin it: with a pause there is no pending capture to swallow the
      // bold, so they stay green either way. Only the keep-typing row reds.
