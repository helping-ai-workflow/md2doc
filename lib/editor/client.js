...
(function () {
  ...
  // response lands).
  //
  // `editRange` is what gates the patch attempt: the 3 call sites with no
  // committed op pass nothing and fall back by contract. `lastParts` is null
  ...
  async function applyRenderResult(j, editRange, scrollY, declaredRange) {
    const plan = (editRange && window.md2docPatchmap)
  ...
  async function applyPatch(j, plan, declaredRange) {
    ...
    // contentEl's element children are exactly the `.ed-block` parts, 1:1 and
    // in document order: lib/md2doc.js emits each part as a single
    // `<div class="ed-block" …>…</div>` (the only two emit sites), the client
    // only ever assigns `parts.join('\n')` into contentEl, and no code path
    // appends anything else as a DIRECT child of contentEl — the ⠿ handle, the
    // ＋ button, the gutter menu and the insert menu are all appended to a
    // BLOCK, and the source-mode textarea is inserted as contentEl's SIBLING.
    ...
    // ran, so it never got a chance to null this out itself. Done here,
    // unconditionally, on every successful patch: this is what makes the
    ...
    //
    // No window.scrollTo() here (ruling F3): the kept nodes never left the
    // document, so the scroll extent never collapsed and the reader is already
    // where they were.
  ...
  async function discardPristineInsert() {
    ...
    // mirror of the forward one. Using op.endLine here computes the opposite
    // direction and the patch silently widens to a full redraw.
  ...
  // real toggles, and the only ones that may carry aria-pressed. This is the
  // exact set of ids toolbar-model.js's deriveState() assigns `.active` to
  // (preview, headings, quote, code, list, ordered-list); every other button
  // is a command, not a toggle. If that module ever grows a seventh, this
  // set has to grow with it — the cost of getting it wrong is an aria lie,
  // not a functional break.
  const TOOLBAR_TOGGLE_IDS = new Set([
    'headings', 'quote', 'code', 'list', 'ordered-list', 'preview',
  ]);
  ...
  function wireImageDrop() {
    ...
    //
    // In PREVIEW mode the toolbar reports every editing button as disabled, so
    // the mode PRESENTS as read-only; a drop that silently wrote a file and
    // inserted a block would make that presentation a lie.
