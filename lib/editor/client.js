...
(function () {
  ...
  function setBlockSelection(sel) { blockSelection = sel; applySelectionClasses(); }
  function clearBlockSelection() { blockSelection = null; applySelectionClasses(); }

  // Test-only hooks. No gesture creates a selection until S3 Task 4, so the
  // runtime suite drives the state directly through these two. They are the
  // browser-side counterpart of the node-side `module.exports` guard at the
  // top of this file — the same "expose the seam the tests need, in the one
  // environment that has it" split — and nothing in the product calls them.
