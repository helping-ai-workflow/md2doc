...
(function () {
  ...
  //
  // v3.3.0 地基 B — every caller now snapshots on BOTH sides of its mutation
  // (the `*-pre` reasons are the pre-mutation half). The burst history's
  ...
  // commands. md2doc binds none of those keys: `KeyB` / `KeyI` / `KeyU` and
  // `key === 'b' | 'i' | 'u'` (either case) do not occur anywhere in this
  // file, so Ctrl+B reaches the contenteditable as the browser's own
  // execCommand and does not pass through any snapBurstIfActive() call site.
  //
  ...
  const NATIVE_FORMAT_INPUT_TYPES = {
    formatBold: 1, formatItalic: 1, formatUnderline: 1, formatStrikeThrough: 1,
    formatSuperscript: 1, formatSubscript: 1, formatRemove: 1
  };
  ...
  document.addEventListener('beforeinput', (e) => {
    if (!NATIVE_FORMAT_INPUT_TYPES[e.inputType]) return;
  ...
  document.addEventListener('input', (e) => {
    const history = burstHistoryForInputEvent(e);
    if (!history) return;
    if (NATIVE_FORMAT_INPUT_TYPES[e.inputType]) {
      // The post-mutation half. snap() rather than noteTyping() because a
      // formatting command is one discrete edit: noteTyping() only marks a
      // pending capture, so WHEN the entry lands would depend on the next
      // noteTyping()/flushTyping() instead of on this event.
      //
      // Honest about its weight: ABLATED (this line swapped back to
      // noteTyping()) the Ctrl+B journey row still passes, because undo()
      // calls flushTyping() first and that pending capture becomes the
      // post-mutation entry anyway. The load-bearing half of this pair for
      // that row is the beforeinput listener above — ablating THAT turns the
      // row red. This line is here so the entry does not depend on the
      // pending-capture path staying that way.
