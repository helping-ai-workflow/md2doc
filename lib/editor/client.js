...
(function () {
  ...
  async function applyPatch(j, plan, declaredRange) {
    ...
    if (focusWasInSpan) {
      ...
      // teardown 十二項彼此的相對順序沒有被動到，條件式的 activeEditor 歸零
      // 仍然在 DOM 手術之後求值。
    ...
    // ── teardown ────────────────────────────────────────────────────────
    // Controller ruling F1: the patch path runs the SAME twelve teardown items
  ...
  // those, produced zero `unlocatable` records (see
  // .superpowers/sdd/2026-09-07-md2doc-v3.3.0/task-4-report.md §4). So the
  // branch is driven instead by making the real blockmap search fail — grep
  ...
  function restoreAfterStructuralOp(focusLine, anchorLine) {
    ...
    if (heldSurface) {
      ...
      // is what happens on the PATCH render path, i.e. on every commit after
      // the first of a page load (`lastParts` is null only for that first one,
      // so the fallback route — the one this guard was measured on — is
      ...
      // surface, and `burstSurvived` is only evaluated afterwards. On this
      // route `currentBurst` is still the OLD burst (resolveBurst()'s own
      // teardown has not nulled it yet, which is also why focusin's
      // `if (currentBurst) return;` stops the focus() from opening a fresh
      // one), and that old burst's editEl was detached by the patch — so
      // `burstSurvived` is false and `resetToolbarBlock()` runs AFTER the
      // caret was handed back. The unprimed run of the same gesture took the
      // FALLBACK route and arrived here with active:BODY, which is why the
      // first wave's measurements never saw this.
      //
  ...
  // no way to be tested at all. This is the seventh __edTest seam and the
  // only production surface that change adds.
  ...
  // of its mutation (the `*-pre` reasons are the pre-mutation half). The seven
  // table structural ops (insert/delete row+column, align, row+column drag)
  // take the same paired shape but call currentBurst.history.snap() directly;
  // grep `history.snap('` to see all of them at once. The burst history's
  ...
  // being torn. NOT extractRangeInto()'s empty-leftover cleanup, though:
  // measured, Enter with the caret on the first character of the <em> in
  // `- Alpha *ital* rest` leaves the upper item on disk as `- Alpha **`.
  //
  ...
  // text the user typed into a cell just before the structural edit. These
  // seven sites call currentBurst.history.snap() DIRECTLY rather than through
  // snapBurstIfActive(), which is why they were not in that function's
  ...
  // and then removes the marks that extraction emptied. The editor's own
  // mark-writing paths go through here: applyMarkToggle()'s wrap branch via
  // wrapRangeIn() below, and applyLinkToggleBody()'s new-link path directly.
  // A native Ctrl+B does NOT: client.js binds no Ctrl/Cmd + b/i/u at all, so
  // that keystroke reaches the contenteditable as the browser's own
  // execCommand and produces a <b> without ever entering this function. The
  // journey suite drives that keystroke and the toolbar button side by side,
  // each with its own tag pattern, for exactly that reason.
  //
  // extractContents() CLONES an element the range only partially contains and
  // leaves the ORIGINAL in place. When the boundary sits at offset 0 of that
  // element's own first child, everything it held moves into the clone and the
  // original is left holding nothing that renders. Measured, bolding a selection
  // that starts at the first character of an <em> (the T9-4 rows in
  // test/editor-journey.test.js drive exactly that range):
  //
  //   before  `Alpha <em>ital</em> bold text here.`
  //   after   `Alpha <em></em><strong><em>ital</em> bold</strong> text here.`
  //
  // — the emptied <em> stays behind as a preceding sibling of the new mark.
  // Nothing downstream drops it: an empty <em> still serialises to its
  // delimiter on each side, so the bytes that reached disk were
  // `Alpha *****ital* bold** text here.`, and on the link path
  // `Alpha **[*ital* bold](https://example.com) text here.` — leading
  // asterisks that re-parse as literal text and come back escaped on the next
  // save.
  //
  // Candidates are collected BEFORE the extraction, because the Range's
  // boundary points move the moment it runs: the element ancestors of both
  // boundary containers, up to (not including) `root`.
  //
  // "Holds nothing" is `no child ELEMENT and no text`, and each half is pinned
  // by its own journey row:
  //
  //  * not "no child NODES": a boundary text node is not removed by the
  //    extraction, its data is replaced with the remainder, so a fully-taken
  //    one is left behind holding the empty string. Probed inside this
  //    function right after the extraction, on the bold row described above —
  //    the surviving <em> reported `<em></em>`, its childNodes still holding a
  //    text node whose data was "". A child-count test never fires on the very
  ...
  //    what a selection starting partway into that mark leaves behind.
  //    Journey row `keeps-text`.
  //
  // An ancestor the new `el` was inserted into holds a child element and is
  // never removed. Deepest first, so an element left empty only by the removal
  // of an inner one is still seen as empty when its own turn comes.
  //
  // `parentNode` is dereferenced unguarded: a candidate is an ancestor of a
  // boundary container, and extractContents() removes only nodes CONTAINED in
  // the range, which an ancestor of a boundary never is — while the removal
  // loop below reaches a candidate only after every candidate it contains, so
  // nothing detaches one before its turn. Each call site additionally refuses
  // a range whose ends are outside `root`, so the walk always stops there.
  function extractRangeInto(range, el, root) {
    ...
    const collect = (node) => {
      ...
      // below `root` — a value that compares correctly between the start
      // chain and the end chain, which a plain per-chain index would not.
      chain.forEach((n, i) => {
    ...
    range.insertNode(el);
    leftovers.sort((a, b) => b.depth - a.depth);
    leftovers.forEach((x) => {
  ...
  // 22-button roster, the group order that decides separator placement, and
  ...
  // v3.2.0: derived, not mirrored. A seventh toggle added to toolbar-model.js
  // used to need a matching edit here, and forgetting it made aria-pressed lie.
  ...
  function updateToolbar() {
    ...
    const state = toolbarModel.deriveState(toolbarContext());
  ...
  //
  // Of the seven inputTypes below, three were actually driven end to end and
  // are pinned by journey rows: formatBold (Ctrl+B), formatItalic (Ctrl+I),
  // formatUnderline (Ctrl+U) — measured tags `<b>` / `<i>` / `<u>`. The other
  // four are listed on the same mechanism but have NOT been driven here; they
  // cost nothing when they never fire, and they are not a claim that they work.
  //
  ...
  function onAnyScroll() {
    ...
    selScrollRaf = requestAnimationFrame(() => {
      selScrollRaf = 0;
      if (!selToolbar.parentNode) return;          // 沒升起 → 0 次 layout read
      const sel = window.getSelection();
