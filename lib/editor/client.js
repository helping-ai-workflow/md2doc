...
(function () {
  ...
  if (typeof module === 'object' && module.exports) {
    module.exports = { extractBlockSource, commitEdit, commitListBlockRemoval, commitBlockInsertion, headingDepthOf, withHeadingDepth };
  ...
  // grip and its anchor row/column ONLY — for the row grip, x between the
  // grip's own left edge and the table's left edge, y clamped to the
  // ANCHOR ROW's own vertical extent, padded; the column grip is symmetric
  // against its anchor column's horizontal extent). A pointer at the SAME
  // x/y-corridor position but outside the anchor row/column's own extent is
  // a genuine exit and still hides the grip via hideTableGrips(), same as
  // before. Neither fix touches either grip's position/size, so the
  // a490f6c grip/bubble non-intersection guarantee (see TE_GRIP_GAP_PX's
  // own comment) is unaffected — this only changes how long an already-
  // shown grip STAYS visible, never where it sits. See
