...
(function () {
  ...
  function showBanner(message, actionLabel, onAction) {
    ...
    // fixing it. This is the identical bundle onAnyScroll() already closes
    // together for an analogous reason (something big just happened; these
    // four don't outlive it) — reused rather than re-invented.
    hideTableGrips();
  ...
  // Task 9 review I3: MEASURED — `.ed-conflict` is `position:fixed; top:0;
  // z-index:999` against `.ed-toolbar`'s `top:0; height:44px; z-index:101`,
  // so the keynav-exit banner sits OVER the entire bar (23/23 buttons'
  // centre points hit-test to the banner, not their own button) the moment
  // it appears — and a keyboard user had no way to take it down: Escape did
  // not dismiss it (showBanner() only wires a click handler onto ✕) and a
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (toolbarRovingIndex >= 0 && toolbarKeynavBlockedByRealControl()) {
      ...
      // direction. MEASURED: dismissKeynavExitBanner() is the only way a
      // keyboard user could take the earlier, unconditional banner back
      // down at all (`.ed-conflict` is `z-index:999` over the bar's own
      // `z-index:101`, covering all 23 buttons; a bare Tab with nothing
      // focused is swallowed by this file's own §3.5 contract, so Escape
      // was the only route left) — excluding Tab here removes most of the
      // cases that needed that route in the first place, and Escape's own
      // rung below closes the rest.
