...
(function () {
  ...
  let lastParts = null;
  // F10 fix round 1: 上一次【成功套用】的 render 之後，文件是不是以一道沒有
  // 閉合的圍欄收尾。跟 `blocks` 成對，兩者都只在 applyRenderResult() 套用成功
  // 之後才一起前進，所以「這次 render 之前是什麼樣子」永遠有得比。開場值從
  // bootstrap payload 直接算（ED.lines 與 ED.blocks 是 lib/editor/server.js 的
  // GET /edit/:id 在同一個回應裡送出來的），這樣一份【開檔時就已經沒閉合】的
  // 文件不會在使用者的下一個手勢上被誣賴成吞噬。
  ...
  //     （兩個分支），加上 safeRerenderAll() 的 'Render failed — unexpected error'
  //   - save 失敗：save() 的 'Save failed — network error'／
  //     'Save failed — malformed server response'／'Save failed — ' + reason
  //   - 手勢在兩次 render 之間失去目標：DROPPED_GESTURE_MESSAGE
  //   - 清單／表格的結構性拒絕：refuseStructuralListEdit()、'無法刪除最後一欄'、
  //     '無法刪除標題列'、'無法刪除最後一列'
  //   - burst 降級：'含不支援的格式，改用原始碼編輯'
  //   - 工具列插圖：TOOLBAR_UNSUPPORTED_IMAGE_MESSAGE、'圖片讀取失敗 — '、
  //     '圖片上傳失敗 — '（網路 throw／非 2xx／回應格式不正確三個分支）、
  //     TOOLBAR_NO_ANCHOR_MESSAGE
  // 光看 class 分不出是哪一個。
  const SWALLOW_MESSAGE = '這道 ``` 圍欄沒有閉合，它後面的內容全部被吃進' +
    '同一個程式碼區塊裡了。把收尾的圍欄補回去就會復原。';

  // F10 fix round 1 —— 吞噬的【形狀】，不是計數。
  //
  // 一道沒有閉合的圍欄一定吃到檔尾，所以它一定是最後一個區塊。這支問的就是
  // 「最後一個區塊是不是一道沒閉合的圍欄」：type 是 code、它的開頭那一行真的
  // 是一道圍欄（縮排式 code block 的 type 也是 code，開頭行不是圍欄，這裡要
  // 排掉），而它的最後一行不是一道光禿禿的收尾圍欄。單行的 code 區塊（只有
  // 開頭圍欄、本文與收尾都還沒有）也算沒閉合。
  //
  ...
  function endsWithUnclosedFence(srcLines, blockList) {
    ...
    const open = srcLines[last.startLine - 1];
    if (typeof open !== 'string' || !/^ {0,3}(?:`{3,}|~{3,})/.test(open)) return false;
    if (last.endLine <= last.startLine) return true;
    const close = srcLines[last.endLine - 1];
    return typeof close !== 'string' || !/^ {0,3}(?:`{3,}|~{3,})\s*$/.test(close);
  }
  ...
  async function applyRenderResult(j, editRange, scrollY, declaredRange) {
    // F10 fix round 1 —— 判別式改成【形狀 ＋ 效果】的合取，不再是純計數。
    //
    // 出貨的第一版是 `Δblocks < 0 && Δlines >= 0`。實測它在再普通不過的編輯上
    // 就會叫（scratchpad/r2/corpus2.log，每一列都是驅動出來的，磁碟位元組都
    // 正確）：
    //   全文原始碼裡把一行的字清掉、行留著   blocks 4->3 lines 8->8  → 誤報
    //   同上，清掉的是最後一段                blocks 4->3 lines 8->8  → 誤報
    //   ⠿ MD 原始碼 裡把一段清成空字串        blocks 4->3 lines 8->8  → 誤報
    //   同上，清成一個空白                    blocks 4->3 lines 8->8  → 誤報
    //   全文原始碼裡把每一段的字都清掉        blocks 4->1 lines 8->8  → 誤報
    // 「一個區塊因為自己的內容被清空而消失」與「一個區塊因為被別人吸收而消
    // 失」在計數上長得一模一樣，這正是要分開的那件事。
    //
    // 換成問吞噬本身的形狀：**文件現在以一道沒閉合的圍欄收尾，而上一次
    // render 之後不是**（endsWithUnclosedFence() / `lastRenderEndedUnclosed`），
    // 再合取一個效果條件 —— 區塊真的少了。形狀那一半把上面五種誤報全部擋掉
    // （它們的最後一個區塊分別是 paragraph／paragraph／paragraph／paragraph／
    // heading，沒有一個是 code）；效果那一半擋掉「在最後一個區塊之後打進一道
    // 還沒閉合的圍欄」與「把最後一個區塊整段換成一道光禿禿的圍欄」——那兩種
    // 形狀成立但它後面沒有東西可以吃，區塊數不減反增或持平（實測 blocks 4->5
    // 與 4->4），對它們喊吞噬是說謊。
    //
    // `blocks` 必須在任何套用【之前】讀。applyFullRender() 的第一行就是
    // `blocks = j.blocks`；applyPatch() 也在它那段 mutation window 之前把
    // `blocks` 換掉（先存一份 prevBlocks 好在自己的 catch 裡回捲）。`lines`
    // 則已經是這次 render 的輸入 —— rerenderAll() 送出去的 body 就是
    // `lines.join('\n')`，所以 j.blocks 描述的正是現在這個 `lines`。
    //
    // 明文承認的漏網（false negative，不是誤報）：一道新的圍欄如果只吃掉
    // 【正好一個】區塊，新生的 code block 補回一個，區塊數就不減。把一道
    // '```' 插進 baseline（'# Doc' / 'Alpha.' / 一段 ```js 圍欄 / '## Next' /
    // 'Bravo.' / 'Charlie.' / '- x' / '- y'，blocks 8、lines 17）的每一個
    // 位置逐一量過，吃掉東西卻不叫的落點是：插在第 4、第 5、第 16 行之前。
    // 其餘吃到東西的落點全部會叫，插在最後一個區塊之後（第 17／18 行之前）
    // 什麼都沒吃到、不叫才是對的。
    const nowUnclosed = endsWithUnclosedFence(lines, j.blocks);
    const swallowed = nowUnclosed && !lastRenderEndedUnclosed &&
                      j.blocks.length < blocks.length;
    ...
    // 這兩支今天都只回 true：applyFullRender() 全函式唯一的 return 就是尾端
    // 那個 `return true`，而 applyPatch() 要嘛回 true、要嘛在自己的 catch 裡
    // 把整件事交給 applyFullRender()。所以這道門今天不會擋下任何東西；它存
    // 在是為了把「render 沒套用成功就不要前進 lastRenderEndedUnclosed、也不
  ...
  // '```js\nconst a = 1;\n```'，長度正是 22 —— caret 就在收尾那三個反引號
  // 之後，打一個字進去，收尾那一行變成 '```Z'，圍欄不再閉合。
  //
  // 這支回 `{ value, caret }`：`value` 是要放進 textarea 的字串（通常就是傳
  // 進來的原始碼，只有一種形狀會被動到，見下），`caret` 是要放 caret 的
  // offset。規則是「caret 不得落在任何一行圍欄標記上」，分三種形狀：
  //
  //  a) 第一行是一道開頭圍欄（最多三個前導空格，接著 ``` 或 ~~~，可帶 info
  //     string）→ caret 落在下一行行首，那是第一個位在程式碼本文裡的 offset。
  //  b) 【空的圍欄】（a 的次分類：下一行就是光禿禿的收尾圍欄）→ 本文區一行
  //     都沒有，兩端的候選位置都在圍欄標記上，**原始碼裡沒有任何安全的
  //     offset**。所以這裡在兩道圍欄之間插一行空白進 `value`，caret 落在
  //     那一行。這是唯一會改動 `value` 的分支。
  //  c) 最後一行是一道光禿禿的收尾圍欄、而第一行不是開頭圍欄（⠿ 選單的
  //     「MD 原始碼」可以框出這種跨區塊的 span）→ 結尾落點會落在那道收尾
  //     圍欄上，改落在前一行的行尾。
  //  其餘一律維持結尾落點。
  //
  ...
  const FENCE_BARE_RE = /^ {0,3}(?:`{3,}|~{3,})\s*$/;
  function rawEditorSeed(text) {
    const rows = text.split('\n');
    if (rows.length === 1) return { value: text, caret: text.length };
    if (FENCE_OPEN_RE.test(rows[0])) {
      if (FENCE_BARE_RE.test(rows[1])) {
        const grown = [rows[0], ''].concat(rows.slice(1));
        return { value: grown.join('\n'), caret: rows[0].length + 1 };
      }
      return { value: text, caret: rows[0].length + 1 };
    }
    const lastRow = rows[rows.length - 1];
    if (FENCE_BARE_RE.test(lastRow)) {
      return { value: text, caret: text.length - lastRow.length - 1 };
    }
