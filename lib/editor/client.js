...
(function () {
  ...
  async function runCycleAlign() {
    ...
    requestAnimationFrame(() => {
      ...
      // nothing to rehighlight. `.ed-te-menu` is `position: fixed`
      // (viewport-relative) though, so if the suppressed span above
      // actually scrolled the page, its left/top are now stale relative to
      // the header cell; reposition unconditionally (same math as
      // showColumnMenu(), skipping its toggle-closed branch — that branch
      // would wrongly close a menu that never actually asked to close). A
      // no-op scroll makes this a no-op reposition (same numbers back).
      const headerRow = headerRowOf(liveTableEl);
      const cell = headerRow ? headerRow.cells[colIndex] : null;
      if (cell) {
