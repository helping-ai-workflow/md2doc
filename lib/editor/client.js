...
(function () {
  ...
  function toggleOutlineSidebar() {
    ...
    //
    // Scoped to the KEYBOARD path only (`toolbarRovingIndex >= 0`, i.e. this
    // call came from F12's Enter/Space on the `outline` button): a mouse
    // click reaches this same function, and buildToolbar()'s own
    // mousedown-preventDefault() note is explicit that a toolbar press must
    // NOT steal focus from whatever burst the mouse user has open — forcing
    // focus into the search box on every click would blur that burst
    // (committing it) for no reason a mouse user asked for. Also scoped to
    // `expanded`: a toggle that just HID the drawer has nowhere sensible to
    // send focus.
    if (expanded && toolbarRovingIndex >= 0) {
