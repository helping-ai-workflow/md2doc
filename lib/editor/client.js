...
(function () {
  ...
  async function convertBlockViaMenu(blockEl, target) {
    ...
    // S2 Task 4 routes li → non-list here and Task 5 routes non-list → list
    // here, each with its own blank-line policy (§4.3 rules 1 and 2). Until
    // then both REFUSE rather than commit a shape this task cannot produce
    // correctly: a li converted by the plain path below would leave the file
    // mid-list with no blank line and lazy continuation would swallow the
    // result into the item above it (measured, §4.3 rule 1).
    if (kind === 'li' || convertMd.targetIsList(target)) {
