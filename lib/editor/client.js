...
(function () {
  ...
  // 轉換成 ONLY — see convertBlockViaMenu(). §7 「表格 block 的『轉換成』」 is
  // on the explicitly-not-doing list, and §3.7 hides the item on a table's own
  // ⠿; this is the same ruling reaching the batch path, where a grip on some
  // other member of the span would otherwise reach the table anyway.
  const BATCH_TABLE_CONVERT_MESSAGE = '選取範圍含有表格，無法整批轉換';
  ...
  async function convertBlockViaMenu(blockEl, target) {
    ...
    // §3.7 / §7: a table block has NO 轉換成 at all — toggleGutterMenu() hides
    // the item outright — because no target can carry its cells. That reason
    // does not stop applying because the table is one member of a SET: a batch
    // whose grip is a paragraph reaches the table through the same span, and
    // MEASURED it wrote '> | A | B |' … / '- | A | B |' with the other two rows
    // as continuations. Nothing is LOST there (both still lex as a table,
    // nested in the blockquote / in the list item) — but the single-block ⠿ and
    // the batch ⠿ then give OPPOSITE answers to the same question, which is
    // exactly the shape §3.6's 2026-08-31 「不得另寫一條」 ruling exists to
    // prevent; and a user selecting a stretch of prose to quote it would almost
    // never mean to wrap an intervening table in a blockquote, with nothing on
    // screen saying so. ⇒ REFUSE, with a banner (§3.6: 靜默不動作是缺陷), file
    // byte-identical.
    //
    // Its own message, not one of the three above: BATCH_MIXED names list items
    // vs other blocks (a paragraph+table span holds no list item at all) and
    // BATCH_GAP names non-contiguity — reusing either would name something the
    // user's selection has not done.
    //
    // SCOPED TO 轉換成. 建立副本 / 刪除 / the Delete key / Tab over a span
    // holding a table are measured correct and have no equivalent objection —
    // they do not need a target that can carry cells — so they are deliberately
    // NOT gated here, and the T8 sweep's 'table in span' row keeps them as
    // applying cells so a widening shows up as a failure too. Placed BELOW the
    // mixed gate so no span that already refuses changes which banner it gets.
    if (shape.kinds.indexOf('table') !== -1) {
      refuseStructuralListEdit(BATCH_TABLE_CONVERT_MESSAGE);
      return;
    }
