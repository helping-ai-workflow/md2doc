...
(function () {
  ...
  async function resolveBurst() {
    ...
    hideTableInsertBubbles();
    // Task 6: same reasoning for the edge-click menu and any in-flight row
    // drag — both reference elements of whichever table this resolution is
    // about to commit/detach.
    hideTableEdgeMenu();
    cancelTeDrag();
  ...
  function tableBurstUndo(cellEl) {
    ...
    if (state !== null) {
      ...
      // edge menu or an in-flight drag currently reference on THIS table —
      // clear both before the swap, same belt-and-braces idiom as
      // rerenderAll()/resolveBurst() above.
      hideTableEdgeMenu();
      cancelTeDrag();
  ...
  async function ensureTableBurstOpen(tableEl) {
    if (currentBurst && currentBurst.blockType === 'table' && currentBurst.editEl === tableEl) return true;
    const ok = await switchAwayFrom();
    if (!ok || !document.body.contains(tableEl)) return false;
    const cell = tableCellsOf(tableEl)[0];
    if (!cell) return false;
    cell.focus();
    return !!(currentBurst && currentBurst.blockType === 'table' && currentBurst.editEl === tableEl);
  }
  ...
  async function onColInsertBubbleClick() {
    ...
    const ok = await ensureTableBurstOpen(tableEl);
    if (!ok || currentBurst.editEl !== tableEl) return;
    insertColumn(tableEl, colIndex);
    armNewTableCells(tableEl);
  ...
  async function onRowInsertBubbleClick() {
    ...
    const ok = await ensureTableBurstOpen(tableEl);
    if (!ok || currentBurst.editEl !== tableEl) return;
    const afterRow = afterRowIndex >= 0 ? bodyRowsOf(tableEl)[afterRowIndex] : null;
    insertRow(tableEl, afterRow);
    armNewTableCells(tableEl);
  ...
  // Click a column's TOP edge (within TE_EDGE_PX of the table's top, over
  // that column's own header-cell span) selects the column: every th/td in
  // it gets '.ed-te-hl' and a floating menu (delete / align-cycle) appears.
  // Click a row's LEFT edge (within TE_EDGE_PX of the table's left, over
  // that row's own span) selects the row the same way, with a delete-only
  // menu. Both zones sit INSIDE an already-permanently-contenteditable cell
  // (armEditables()'s 'table' branch above), so the delegated `pointerdown`
  // listener below must preventDefault() there to stop native caret
  // placement from stealing the click — same "keep focus put" idiom
  // buildTableInsertBubble() documents for the hover-insert bubbles above
  // (and for the same reason: without it, focus jumping into the cell would
  // fire a focusout on whatever was previously focused before this
  // handler's own menu-open logic even runs).
  //
  // Row drag starts from the SAME left-edge zone (body rows only — the
  // header <tr> is never draggable, per the brief): after a small movement
  // threshold (distinguishing "click to open the menu" from "press-and-
  // drag"), a drop-indicator line tracks the pointer between body rows;
  // releasing performs the reorder via a plain `insertBefore()` on the SAME
  // <tr> node (never a clone/innerHTML-replace). The DOM's "insert"
  // algorithm reparents a node in one synchronous step without an
  // observable disconnected state, so — unlike tableBurstUndo()/
  // tableBurstRedo()'s innerHTML-snapshot restore just above — moving the
  // dragged row this way never blurs it even if it happened to contain the
  // active cell, so this needs no suppressTableFocusout guard. The menu's
  // delete ops (which DO remove nodes) sidestep the same hazard a different
  // way — see refocusAwayFromColumn()/refocusAwayFromRow() below.
  //
  ...
  // the brief's explicit "wire, don't rewrite").
  const TE_EDGE_PX = 8; // proximity threshold, in CSS px, for an edge zone
  ...
  async function runDeleteColumn() {
    ...
    refocusAwayFromColumn(tableEl, colIndex);
  ...
  async function runDeleteRow() {
    ...
    refocusAwayFromRow(tableEl, rowEl);
    deleteRow(rowEl);
  ...
  // Recomputes which edge zone (if any) viewport coordinates `(x, y)` fall
  // in, for the armed table nearest `target` — same "closest
  // '.ed-wys-table'" gate updateTableInsertBubbles() above uses. Column
  // zones take priority at the top-left corner cell, where both could
  // technically match.
  function hitTestEdgeZone(x, y, target) {
    const blockEl = target && target.closest && target.closest('.ed-block[data-block-type="table"]');
    const tableEl = blockEl ? blockContentEl(blockEl) : null;
    if (!tableEl || !tableEl.classList.contains('ed-wys-table')) return null;
    const tableRect = tableEl.getBoundingClientRect();
    const headerRow = headerRowOf(tableEl);
    if (headerRow && Math.abs(y - tableRect.top) <= TE_EDGE_PX) {
      const cells = Array.prototype.slice.call(headerRow.cells);
      for (let i = 0; i < cells.length; i++) {
        const r = cells[i].getBoundingClientRect();
        if (x >= r.left && x <= r.right) return { kind: 'col', tableEl, colIndex: i };
      }
    }
    if (Math.abs(x - tableRect.left) <= TE_EDGE_PX) {
      const rows = allRowsOf(tableEl);
      for (let i = 0; i < rows.length; i++) {
        const r = rows[i].getBoundingClientRect();
        if (y >= r.top && y <= r.bottom) {
          return { kind: 'row', tableEl, rowEl: rows[i], isHeader: rows[i] === headerRow };
        }
      }
    }
    return null;
  ...
  // drag), or null between gestures. `hit` is whatever hitTestEdgeZone()
  ...
  async function performRowDrop(tableEl, rowEl, beforeRow) {
    const ok = await ensureTableBurstOpen(tableEl);
    if (!ok || currentBurst.editEl !== tableEl || !document.body.contains(rowEl)) return;
    const tbody = tableEl.tBodies[0];
    if (!tbody || rowEl.parentElement !== tbody) return;
    const prevNext = rowEl.nextSibling;
    if (beforeRow && beforeRow.parentElement === tbody) tbody.insertBefore(rowEl, beforeRow);
    ...
    if (rowEl.nextSibling !== prevNext) currentBurst.history.snap('drag-row');
  }
  ...
  document.addEventListener('pointerdown', (e) => {
    ...
    const hit = hitTestEdgeZone(e.clientX, e.clientY, e.target);
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    requestAnimationFrame(() => {
      ...
      // the row zone's left-edge strip (TE_EDGE_PX=8) sits well inside the
      // hover-insert row bubble's own proximity threshold (TB_EDGE_PX=10),
      // so without this gate, an active row drag would repaint the ＋
      // bubble on TOP of the drop indicator on every real drag. Explicitly
      // HIDE (not just skip recomputing) so a bubble already showing from
      // the moment just before the drag threshold was crossed doesn't
      // linger stale for the rest of the gesture.
      if (tePointer && tePointer.dragging) { hideTableInsertBubbles(); return; }
