...
(function () {
  ...
  function drawioFingerprint(box) {
    ...
    for (let i = 0; i < pages.length; i++) out.push(pages[i].innerHTML);
  ...
  function noteDrawioPage(message) {
  ...
  async function refreshStaleDrawio(token) {
    ...
    try {
      ...
      for (let i = 0; i < j.parts.length; i++) {
        ...
        // Both sides hold one and the bake is identical: nothing to do. This
        // is what keeps one external write from re-swapping every OTHER
        // diagram on the page (G4).
        if (oldBox && newBox &&
            drawioFingerprint(oldBox) === drawioFingerprint(newBox)) continue;
        ...
        let oldNames = [], activeIdx = -1, hadMultiPage = false;
        const oldPagedBox = (oldBox && oldBox.hasAttribute('data-drawio-pages')) ? oldBox : null;
        if (oldPagedBox) {
          try { oldNames = JSON.parse(atob(oldPagedBox.getAttribute('data-drawio-pages') || '')); }
          catch (e2) { oldNames = []; }
          if (!Array.isArray(oldNames)) oldNames = [];
          hadMultiPage = oldNames.length >= 2;
          if (hadMultiPage) {
            const oldPages = oldPagedBox.querySelectorAll('.drawio-page');
            const activePage = oldPagedBox.querySelector('.drawio-page:not([hidden])');
            activeIdx = activePage ? Array.prototype.indexOf.call(oldPages, activePage) : -1;
          }
        }
        ...
        if (!hadMultiPage) continue;
        const newPagedBox = fresh.querySelector ? fresh.querySelector('.drawio[data-drawio-pages]') : null;
        if (!newPagedBox) continue;
        ...
        let newNames = [];
        try { newNames = JSON.parse(atob(newPagedBox.getAttribute('data-drawio-pages') || '')); }
        catch (e3) { newNames = []; }
        if (!Array.isArray(newNames)) newNames = [];
        const newPages = newPagedBox.querySelectorAll('.drawio-page');
        const landing = resolveDrawioLanding(oldNames, activeIdx, newNames);
  ...
  setInterval(() => {
    ...
    // taken (see noteDrawioPage) gets its next chance here, on the same
