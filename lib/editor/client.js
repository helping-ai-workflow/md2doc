...
(function () {
  ...
  //
  // 需要重造時，屬性一律照 cell.attributes 的**原順序**複製（唯一例外是
  // contenteditable，抽出來最後補）——不可寫死成某個順序。armEditables()
  // 只是把 contenteditable append 到 renderer 給的屬性後面，而 renderer
  // 先寫 class 再寫 style（見 lib/md2doc.js 的 renderer.table），所以 armed
  // 形是 `class, style, contenteditable`；寫死 `style, class,
  // contenteditable` 會讓「拖下去再拖回來」列序還原、位元卻沒還原，同樣
  // 觸發整表 canonical 重寫。照原順序複製則對任何 renderer 輸出都成立，
  // 不必再跟 renderer 的屬性順序綁在一起。
  function retagCell(cell, tagName) {
    ...
    Array.prototype.slice.call(cell.attributes).forEach((attr) => {
      if (attr.name === 'contenteditable') return; // 最後補，見下
      next.setAttribute(attr.name, attr.value);
    });
    while (cell.firstChild) next.appendChild(cell.firstChild);
    // armEditables()/armNewTableCells() 都是最後才 append contenteditable，
    // 所以它在 armed 形裡永遠排在最後——照樣擺。
    const ce = cell.getAttribute('contenteditable');
    if (ce !== null) next.setAttribute('contenteditable', ce);
  ...
  document.addEventListener('pointermove', (e) => {
    if (!tePointer) return;
    if (tePointer.hit.kind !== 'row') return; // 欄拖曳在 Task 8 才加分支
  ...
  document.addEventListener('pointerup', async (e) => {
    ...
    if (st.dragging) {
      ...
      // grip below needs no such treatment: it is OUR element and always
      // carries at least 'ed-te-grip-row', so removing one class can never
      // empty its attribute.)
      if (!st.hit.rowEl.className) st.hit.rowEl.removeAttribute('class');
      rowGrip.classList.remove('ed-te-grip-dragging');
      teDropIndicator.hidden = true;
      await performRowDrop(st.hit.tableEl, st.hit.rowEl, st.dropTarget);
      return;
    }
