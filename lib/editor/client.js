...
(function () {
  ...
  function markerInSourceStyle(marker, src) {
    if (!src) return marker;
    if (src.bullet) return /^[-*+]/.test(marker) ? src.bullet + marker.slice(1) : marker;
    if (src.delim) return marker.replace(/([.)])(\s*)$/, src.delim + '$2');
    return marker;
  }
  ...
  // it), not a reformat.
  //
  // So the replacement re-emits each such ancestor's MARKER, one per line,
  // ahead of the edited block's own lines. That is the canonical form the
  // whole-run path already produces for a tight run ('- a\n-\n  - b'), and the
  // only form the flat model can express: an item that owns no line has
  // nowhere to put its marker except a line of its own. It round-trips —
  // marked reads '-\n  - b' and '- - b' as the same tree — and it removes the
  // zero-line shape from the file, so the child's parent becomes editable too.
  //
  // MARKER ONLY, never the ancestor's emitted line: an item whose own content
  // resumes AFTER its child ('- - b' + a later '  tail' paragraph) owns that
  // content on a line OUTSIDE this commit range, so emitting the serializer's
  // full line for it would duplicate the text. Its indent prefix and marker
  // WIDTH come from lineMeta's own record rather than from a guess — an ordered
  // outer contributes three columns, not two, and getting that wrong drops the
  // child out of the item entirely (spec §3.4's errata table) — while the marker
  // CHARACTERS are taken back from the source line (markerInSourceStyle below).
  //
  ...
  function sharedMarkerLinesBefore(lineMeta, firstIdx, editedBlock) {
    ...
    for (let k = 0; k < firstIdx; k++) {
      ...
      // The ancestors sharing this line ARE its leading markers, outermost
      // first, so the n-th one collected here is the n-th marker on the line.
      const marker = markerInSourceStyle(m.marker, chain[out.length]);
  ...
  async function resolveBurst() {
    ...
    if (burst.blockType === 'li') {
      ...
      if (unsupported.length > 0) {
        ...
        // Round 5: the edited block's source line may also carry the markers of
        // zero-line ancestors (same-line nesting, '- - b'). They emit their own
        // lines in runMd but are not attributed to this block, so slicing by id
        // alone dropped them and the child lost its parent. See
        // sharedMarkerLinesBefore().
  ...
  function runLineOfBlock(runEls, targetBlock) {
    ...
    const { lineMeta } = listMd.serializeBlocks(runEls);
