...
(function () {
  ...
  async function applyPatch(j, plan, declaredRange) {
    ...
    // next click). It has THREE further side effects, and all
    // three ride along safely: (1) the `docMode === 'preview'` re-arm cannot
    // matter here — preview sets every surface contenteditable="false", so no
    // burst can be open to survive; (2) updateToolbar() is re-derived from the
    // DOM on the next interaction; (3) closeToolbarMenu() — skipping it leaves
