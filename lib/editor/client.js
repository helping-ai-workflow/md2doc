...
(function () {
  ...
  // The span order after "the member at `from` goes immediately before the
  // member at `insertAt`". `insertAt` names a slot in the array BEFORE the
  // removal — it comes straight from the drop target's `before-block N` — so a
  // DOWNWARD move is decremented and an upward one is not: the same `insertAt`
  // means two different slots depending on the direction, which is the whole
  // reason this is a named function with its own test rather than a splice
  // pair at the call site.
  //
  // `null` means the order would be UNCHANGED, which is a byte no-op and not a
  // failure: `insertAt === from` is "released on my own top edge" and
  // `insertAt === from + 1` is "released on the seam immediately below me" —
  // visually the same place, and the case the drag's own
  // release-where-you-started gesture depends on. An out-of-range index
  // answers `null` too; there is no order it could name.
  function reorderSpanIndices(length, from, insertAt) {
    ...
    const rest = [];
    for (let i = 0; i < length; i++) if (i !== from) rest.push(i);
    const at = insertAt > from ? insertAt - 1 : insertAt;
    return rest.slice(0, at).concat([from], rest.slice(at));
  }
  ...
  // Task 7 REPLACES the refusal this drives with applyIndentClamp(); see
  // performListItemDrop() for the single site and for why the existing clamp
  // cannot express a move today.
  ...
  if (typeof module === 'object' && module.exports) {
    module.exports = { extractBlockSource, commitEdit, commitRangeEdit, commitRangeRemoval, commitListBlockRemoval, commitBlockInsertion, planBlockMove, commitBlockMove, reorderSpanIndices, spanIndentsAreAnchored, rollbackFailedRender, headingDepthOf, withHeadingDepth };
  ...
  // S4 Task 3: the ⠿ drag's list-seam refusal. A move whose SOURCE seam or
  // whose DESTINATION seam sits between two list items cannot be expressed as
  // a verbatim line relocation without changing what the surrounding lists
  // ARE — see performBlockDrop() for the two measurements. §3.6: a silent
  // no-op is a defect, so it refuses out loud. Task 5 owns the full
  // cross-boundary enumeration and may widen this wording; the RULING it
  // implements ("cross-boundary moves are refused with a banner in 3.0.0")
  // is already made.
  const BLOCK_MOVE_LIST_SEAM_MESSAGE = '區塊搬移不能跨越清單邊界';
  // S4 Task 4: §4.5 also says the moved block KEEPS ITS OWN INDENT and is
  // clamped when the destination makes it illegal. That clamp is Task 7's, and
  // applyIndentClamp() cannot express a move yet — MEASURED: its rule-2 scope
  // starts after the operated block's NEW index, while a move's orphans are
  // left behind at its OLD one, so on '- alpha / (2sp)- a1 / - bravo' with
  // alpha moved to the end it clamps NOTHING and a1 is emitted at column 0.
  // Until Task 7 the move is refused out loud rather than committed at a
  // column the screen never showed. §3.6: not silently.
  const BLOCK_MOVE_ORPHAN_MESSAGE = '搬移後子項目會失去上層項目，暫時無法搬移到這裡';
  ...
  // Geometry, per axis (they are NOT mirror images — see below):
  ...
  //     side (P0-a).
  //   ROW grip: its LEFT EDGE coincides with the table's LEFT border, so the
  //     whole grip is INSIDE the table. This is spec §4.2 hit-test
  //     conflict 2's ruling, landed in S4 T1.
  //
  // Why the row axis had to break the symmetry. The block's own gutter ⠿
  // hangs outside the content box at [blockLeft-22, blockLeft-4], and a
  // table block's own table starts AT blockLeft, so a row grip centred on
  // the table's left border occupied [blockLeft-10, blockLeft+10] and
  // overlapped the ⠿'s right 6px. Both are position: fixed / absolutely
  ...
  //
  // An earlier revision (v2.10.1) already tried the inset and was reverted,
  // because a 20px grip inside a cell whose left padding is 14px sits on
  // that cell's TEXT. That is fixed on the CSS side this time rather than by
  // giving the grip back its outside half: lib/md2doc.js pads the first
  // column to --ed-te-cell-pad-left (the grip's own width plus the gutter
  // pair's breathing gap) in EDIT MODE ONLY. The ⠿ ↔ grip clearance that
  // remains is exactly that same breathing-gap token — see the edit-mode
  // :root block in lib/md2doc.js, which spells both tokens out; this file
  // deliberately does not name them, because test/editor-client.test.js
  // forbids the retired hover-gutter class name as a bare substring and the
  // token names contain it. Neither edge is written down twice, so the two
  // cannot drift apart again — drift is what produced this defect (the CSS
  // comment claiming "an 8px gap" was describing v2.11.0's pair position,
  // which v2.11.1 replaced).
  ...
  //
  // Current geometry (the two axes DIVERGED in S4 T1, §4.2 conflict 2): the
  // COLUMN grip still straddles the table's TOP border, half above / half
  // below, so the corridor-crossing bug described above still applies to it
  // exactly as originally described and pointInColGripZone() still covers
  // the outside-the-border corridor. The ROW grip is now entirely INSIDE the
  // table (its left edge on the table's left border), so a pointer
  // travelling from a cell to it never leaves the table and there is no
  // corridor to cross — pointInRowGripZone() is the padded grip rect alone.
  // See its own comment for why the old formula was deleted rather than
  // left in place to evaluate to the empty set.
  //
  ...
  //
  // What each keep-zone covers TODAY — and the two axes diverged when §4.2
  // conflict 2 moved the row grip inside the table (S4 T1):
  //   pointInColGripZone() still keeps the ORIGINAL corridor shape: the
  //     union of (the column grip's own padded rect) and (the straight strip
  //     between the grip's own TOP edge and the table's top border, x clamped
  //     to the anchor COLUMN's own horizontal extent, padded). The column
  //     grip still straddles the top border, so that corridor is still real.
  //   pointInRowGripZone() is now the padded grip rect ALONE. Its corridor
  //     was the strip between the grip's own LEFT edge and the table's LEFT
  //     border; with the grip's left edge ON that border the strip is empty,
  //     and §4.6 explicitly warns that leaving the formula in place would
  //     make it (and the two scenarios downstream of it) silently vacuous
  //     rather than red. It is deleted rather than left to read as if it did
  //     something. Nothing is lost: the corridor existed because a pointer
  //     travelling from a cell to a border-straddling grip had to leave the
  //     table on the way, and a grip that is entirely inside the table has no
  //     such approach — the pointer goes cell -> grip, and the
  //     `target.closest('.ed-te-grip-row')` early return at the top of
  //     updateTableEdgeGrips() covers the moment it arrives. The observable
  //     change is that leaving the table on the left now hides the row grip
  //     immediately, at the anchor row's own height as well as at any other
  //     row's; the scenario named below asserts both halves, because "hidden
  //     at another row's height" alone is true of any geometry once the
  //     corridor is gone.
  ...
  function pointInRowGripZone(x, y) {
    ...
    // §4.2 衝突 2（S4 T1）之後 row grip 整個落在表格內側，左緣就壓在表格
    // 左邊界上，所以舊的走廊公式 `x >= gr.left && x <= tableRect.left` 塌成
    // 空集合。§4.6 已經預告過這件事：留著它不會變紅，只會變成空測。這裡
    // 直接刪掉，keep-zone 只剩 grip 自己的 padded rect。
    // 走廊本來只為了一件事存在——指標從儲存格走向一個跨在邊界上的 grip
    // 時必定會先離開表格；grip 內移之後那條路徑整段都在表格內（先是
    // 儲存格，接著是 grip 自己，由 updateTableEdgeGrips() 開頭那個
    // `target.closest('.ed-te-grip-row')` 早退接手），所以沒有走廊可走。
    // 對使用者可見的差別是：從左側離開表格會立刻收掉 row grip，錨定那一
    // 列的高度也不例外。
  ...
  function updateTableEdgeGrips(x, y, target) {
    ...
    // 「grip 位置都一樣」) — its LEFT EDGE on the table's left border, i.e.
    // entirely inside the table (spec §4.2 hit-test conflict 2, landed
    // S4 T1 — see the geometry note above buildTableGrip() for the
    // measurement that forced it), vertically centred on its own row; no
    ...
    if (rowEl && (rowEl !== headerRow || bodyRowsOf(tableEl).length > 0)) {
      ...
      // `hidden = false` assignment below takes effect. There is no width
      // fallback any more, and that is the point: the left edge is the
      // table's own border, so the grip's WIDTH is not an input to its
      // position at all. It used to be (`tableRect.left - gw / 2`), which
      // meant the CSS width and this literal `20` had to agree for the
      // gutter clearance to hold — exactly the drift §4.2 conflict 2 is
      // about. The width now lives in one place only, the
      // --ed-te-grip-row-w token in lib/md2doc.js.
  ...
  function updateBlockDrag(e) {
    ...
    if (!blockDragState.dragging) {
      ...
      // model: it dims a node INSIDE the content and paid for it with a
      // leftover `class=""` that the burst's zero-edit guard (which compares
      // innerHTML) then had to strip by hand.
  ...
  async function performBlockDrop(st) {
    ...
    // A multi-block operand set and a MIXED one belong to Task 6, and both keep
    // the behaviour Task 2 shipped — the drop does nothing. They are the last
    // two silent shapes in this function; Task 8 Step 0 is where §3.6's
    // 「靜默不動作是缺陷」 is verified to have no remaining exception. Do
    // not add a third.
    if (operands.batch) return;
    const liveSrcEl = operands.els[0];
    // ── the FIRST home position, and it is shared by BOTH kinds ────────────
    ...
    // untouched either way. The li path below has a SECOND home position of
    // its own — see performListItemDrop().
    if (liveDestEl === liveSrcEl) return;
    // S4 Task 4: a li source has its own path. Its lines cannot be relocated
    // verbatim (markers and ordinals are run-global), so the whole run is
    // re-serialized in the new span order instead — still ONE commitRangeEdit,
    // still one Ctrl+Z. Everything below this line is the non-li move.
    if (liveSrcEl.getAttribute('data-block-type') === 'li') {
      await performListItemDrop(liveSrcEl, liveDestEl);
      return;
    }
    ...
    const si = live.indexOf(liveSrcEl);
    const rest = live.filter((el) => el !== liveSrcEl);
    const di = liveDestEl ? rest.indexOf(liveDestEl) : rest.length;
    if (si < 0 || di < 0) { showBanner(DROPPED_GESTURE_MESSAGE, null, null); return; }
    const srcRec = blockRecOf(liveSrcEl);
    ...
    // the block inside somebody else's territory. Unreachable while the seam
    // gate below holds (every no-line phantom this model produces is a li),
    // and checked anyway for the same reason blockOwnsNoLine() is checked
    // twice in resolveGutterOperands().
    if (destRec && destRec.endLine < destRec.startLine) return;
    ...
    // Deliberately conservative: two adjacent li of DIFFERENT runs are refused
    // too, though that case is safe. Task 5 owns the cross-boundary
    // enumeration and can narrow it; corrupting a list in the meantime cannot
    // be narrowed afterwards.
    const isLiEl = (el) => !!el && el.getAttribute &&
      el.getAttribute('data-block-type') === 'li';
    if ((isLiEl(live[si - 1]) && isLiEl(live[si + 1])) ||
        (isLiEl(rest[di - 1]) && isLiEl(rest[di]))) {
      refuseStructuralListEdit(BLOCK_MOVE_LIST_SEAM_MESSAGE);
      return;
    }
    ...
    // range. Neither is reachable from here — the home positions are already
    // out — and neither pushed an op, so there is nothing to render.
    if (!result.op) return;
    const prevLines = lines;
    lines = result.lines;
  ...
  async function performListItemDrop(liEl, destEl) {
    ...
    // went wrong.
    const afterSrcEl = all[all.indexOf(liEl) + 1] || null;
    if (destEl ? destEl === afterSrcEl : all[all.length - 1] === liEl) return;
    // ── §4.3's run-wide gate ───────────────────────────────────────────
    ...
    if (insertAt < 0) { refuseStructuralListEdit(BLOCK_MOVE_LIST_SEAM_MESSAGE); return; }
    ...
    const order = reorderSpanIndices(run.length, srcIdx, insertAt);
    if (!order) return;
    ...
    // The moved item keeps its own `data-indent` (nothing here writes one),
    // and §4.5 says it is CLAMPED when the destination makes it illegal. That
    // clamp is Task 7's subject and it is NOT written here, because
    // applyIndentClamp() cannot express a move yet: `clampIndents()`'s rule-2
    // scope starts after the operated block's NEW index, while a move's
    // orphans are left behind at its OLD one. MEASURED on
    // '- alpha / (2sp)- a1 / - bravo' with alpha dragged to the end — the span
    // becomes [a1, bravo, alpha], the clamp's scope is empty, a1 keeps
    // `data-indent="1"` and serializeBlocks() emits it at column 0 anyway
    // (the width stack is rebuilt per span, so the FIRST block emits at column
    // 0 whatever it claims). The DOM and the file would then disagree about
    // the nesting, which is the "looks right on screen, cannot be saved"
    // class indent-clamp.js's own header describes.
    //
    // So until Task 7 the move is REFUSED, out loud. Task 7 replaces exactly
    // this `if` with its `applyIndentClamp(span, liEl, oldIndent, …)` call —
    // ONE site, and the span it needs is already built and already contains
    // the moved block AFTER the reorder, which is applyIndentClamp()'s own
    // precondition (it indexes by `spanEls.indexOf(el)` and abandons the whole
    // clamp when an operated element is missing). Nothing else here has to
    // change, and nothing here has to be undone.
    ...
    mutateListRun(() => {
      ...
      liEl.parentNode.insertBefore(liEl, refEl);
    });
    ...
    // to its new lines.
    declareCollapse(movedLiRangeAfterReorder(span, liEl, carryOver, range));
    await commitListStructure(span, null, false,
  ...
  // several.
  function movedLiRangeAfterReorder(span, liEl, carryOver, range) {
    const id = liEl.getAttribute('data-block-id');
    if (id === null) return null;
    const { lineMeta } = listMd.serializeBlocks(span, { carryOver: carryOver || null });
    let first = -1;
    let n = 0;
    lineMeta.forEach((m, i) => {
      if (m.blockId !== id) return;
      if (first < 0) first = i;
      n++;
    });
    if (first < 0) return null;
    return { startLine: range.startLine + first, endLine: range.startLine + first + n - 1 };
  }
