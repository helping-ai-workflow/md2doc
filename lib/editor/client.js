...
(function () {
  ...
  function handleBurstKeydown(e, editEl) {
    ...
    // Task 7 (Phase 4): per-li burst. Structural Enter/Tab semantics will be
    // wired in Task 8; for THIS task Enter and Tab are temporarily inert —
    // absorbed so the browser does not insert a newline or focus-cycle out
    // of the surface. All other keys (Escape, Ctrl+Z, Ctrl+Y) fall through
    // to the shared branches below — they work the same as for paragraphs.
    if (currentBurst.blockType === 'li') {
      if (e.key === 'Enter' || e.key === 'Tab') {
        e.preventDefault();
        return;
      }
      // Fall through: Escape / Ctrl+Z / Ctrl+Y handled by shared branches.
    }
