...
(function () {
  ...
  async function applyPatch(j, plan, declaredRange) {
    ...
    // next click). Its two side effects ride along safely: the
    // `docMode === 'preview'` re-arm cannot matter here (preview sets every
    // surface contenteditable="false", so no burst can be open to survive),
    // and updateToolbar() is re-derived from the DOM on the next interaction.
