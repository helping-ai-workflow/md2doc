...
(function () {
  ...
  function openWysiwygEditor(blockEl, block) {
    ...
    async function commit() {
      ...
      // Final-review Finding 5: same empty-heading no-trailing-space rule
      // as withHeadingDepth() above — an emptied-out heading (user deleted
      // all its text) must not commit '#'.repeat(depth) + ' ' with nothing
      // after the space.
      const newText = depth === null ? result.md :
        (result.md === '' ? '#'.repeat(depth) : '#'.repeat(depth) + ' ' + result.md);
