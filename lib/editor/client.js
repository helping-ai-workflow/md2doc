...
(function () {
  ...
  let lastParts = null;
  // F10: 上一次【成功套用】的 render 當時的 `lines.length`。跟 `blocks` 成
  // 對：兩者都只在 applyRenderResult() 套用成功之後才一起前進（見那裡），所
  // 以「跟上一次上畫面的狀態相比，區塊少了幾個／行數少了幾行」這兩個差永遠
  // 跨同一段區間。開場值取 ED.lines.length，跟 `blocks` 取 ED.blocks 是同一
  // 份 bootstrap payload（lib/editor/server.js 的 GET /edit/:id 把
  // `mdText.split(/\r\n|\r|\n/)` 與 buildBlockMap() 的結果一起送出來）。
  let lastRenderLineCount = ED.lines.length;
  ...
  // F10 ——「這次編輯把後面的內容吞掉了」那一句，單獨一個常數。
  // test/editor-journey.test.js 的 F10 那一列釘的是這串文字本身，而不是
  // 「畫面上有一條 .ed-conflict」。那個 class 只有 showBanner() 一個產生點，
  // 而它的呼叫端橫跨好幾個彼此無關的家族：磁碟衝突（showConflictBanner()）、
  // render 失敗（rerenderAll() 的網路 throw／非 2xx／壞掉的 JSON／形狀不對的
  // payload，加上 safeRerenderAll() 的 unexpected error）、save 失敗（save()
  // 的網路 throw／壞掉的 JSON／其他狀態碼）、手勢在兩次 render 之間失去目標
  // （DROPPED_GESTURE_MESSAGE）、清單／表格的結構性拒絕
  // （refuseStructuralListEdit()、'無法刪除最後一欄'／'無法刪除標題列'／
  // '無法刪除最後一列'），以及 burst 降級的'含不支援的格式，改用原始碼編輯'。
  // 光看 class 分不出是哪一個。
  const SWALLOW_MESSAGE = '這次編輯之後區塊變少了，行數卻沒有變少 —— 後面的內容' +
  ...
  async function applyRenderResult(j, editRange, scrollY, declaredRange) {
    // F10 —— 吞噬判別式：Δblocks < 0 且 Δlines >= 0。
    //
    // 必須在任何套用【之前】算。applyFullRender() 的第一行就是
    // `blocks = j.blocks`；applyPatch() 也在它那段 mutation window 之前把
    // `blocks` 換掉（先存一份 prevBlocks 好在自己的 catch 裡回捲）。兩支任何
    // 一支跑過之後，`blocks` 都不再是「上一次上畫面的那一份」。
    //
    // 另一半是 `lines.length`：它就是這次 render 的輸入 —— rerenderAll() 送
    // 出去的 body 是 `lines.join('\n')`，所以 j.blocks 描述的正是現在這個
    // `lines`。而 `blocks` 與 `lastRenderLineCount` 只在下面套用成功之後才一
    // 起前進，兩個差因此跨的是同一段區間。
    //
    // MEASURED（直接呼叫 lib/editor/blockmap.js 的 buildBlockMap；baseline 是
    // '# Doc' / 'Alpha.' / 一段 ```js 圍欄 / '## Next' / 'Bravo.' /
    // 'Charlie.' / '- x' / '- y'，量到 blocks 8、lines 17）：
    //
    //   收尾 fence 改成 '``` MID'           db=-5  dl= 0  → 真
    //   刪掉一整段 para（連它前面的空行）   db=-1  dl=-2  → 假
    //   併兩段 para（＝刪掉段前的空行）     db=-1  dl=-1  → 假
    //   刪掉一個 li                         db=-1  dl=-1  → 假
    //   在最後一個區塊之後打進一道 '```'    db=+1  dl=+1  → 假
    //   刪掉尾端四行                        db=-2  dl=-4  → 假
    //
    // 兩件明文承認的事：
    //  * 「併兩段 para」與「刪掉 para 前面的空行」不是判別式分不開的兩件
    //    事 —— 它們是同一次編輯的兩種說法，送進 buildBlockMap 的位元組完全
    //    一樣，任何判別式都只能給出同一個答案。
    //  * 未閉合圍欄那一族的盲點比它看起來窄，而且量得出邊界在哪。把一道新
    //    的 '```' 插進上面那份 baseline 的每一個位置量過：吞掉不只一個區塊
    //    時 db 仍然是負的而 dl=+1，判別式為真 —— 這一族大部分【有】覆蓋到。
    //    唯一真的漏掉的是「吞掉正好一個區塊」那一格：新生的 code block 的
    //    +1 剛好抵掉被吞的 -1，db=0。再往後插（最後一個區塊之後）什麼都沒
    //    吞到，db=+1，那一格沒有信號才是對的。
    const swallowed = (j.blocks.length - blocks.length) < 0 &&
                      (lines.length - lastRenderLineCount) >= 0;
    ...
    // 在是為了把「render 沒套用成功就不要前進 lastRenderLineCount、也不要為
    // 一次沒上畫面的編輯升起警告」這件事寫在這裡，而不是留給讀者去讀那兩支。
    if (!ok) return false;
    ...
    if (swallowed) showBanner(SWALLOW_MESSAGE, null, null);
    return true;
  }
  ...
  //
  // 規則看的是【圍欄形狀】，不是 data-block-type：值的第一行是一道開頭圍欄
  // （最多三個前導空格，接著 ``` 或 ~~~，可帶 info string）、而且值不只一行
  // 時，caret 落在那一行的下一行行首 —— 那是第一個位在程式碼本文裡的 offset。
  // 其餘每一種原始碼都維持結尾落點。
  //
  ...
  // 也就是說對 blockquote 而言「第一行行首」在 '>' 的前面，打第一個字就把
  // 那一行踢出引用區塊，比它要取代的結尾落點更糟。image 兩邊都無所謂
  // （'X![a](b.png)' 與 '![a](b.png)X' 都還是 paragraph）。圍欄是唯一一種
  // 「結尾落點會落在收尾標記之後」的形狀，所以這支就只處理它。
  function rawCaretOffset(text) {
    const nl = text.indexOf('\n');
    if (nl === -1) return text.length;
    if (!/^ {0,3}(?:`{3,}|~{3,})/.test(text.slice(0, nl))) return text.length;
    return nl + 1;
  }
  ...
  async function openRawEditor(blockEl, range) {
    ...
    autoSize(ta);
    ta.focus();
    // F9: 不再無條件落在值的結尾 —— 見 rawCaretOffset() 自己的註解。
    const caret = rawCaretOffset(ta.value);
  ...
  async function focusInsertedBlock(newStartLine, kind) {
    ...
    if (kind === 'code') {
      ...
      // rawCaretOffset() (see its own comment), and for that skeleton
      // rawCaretOffset() returns indexOf('\n') + 1 — the very expression
      // this branch used to compute. So the fix-up is deleted rather than
      // kept as a no-op that reads like it is still load-bearing.
