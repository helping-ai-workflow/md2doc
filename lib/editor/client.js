...
(function () {
  ...
  if (typeof module === 'object' && module.exports) {
    module.exports = { extractBlockSource, commitEdit, commitRangeEdit, commitRangeRemoval, commitListBlockRemoval, commitBlockInsertion, planBlockMove, commitBlockMove, rollbackFailedRender, headingDepthOf, withHeadingDepth };
  ...
  async function performBlockDrop(st) {
    ...
    // Task 3 owns exactly one shape: a SINGLE block that is not a list item.
    // A li source belongs to Task 4 (its lines cannot be relocated verbatim —
    // markers and ordinals are run-global), a multi-block operand set and a
    // mixed one to Task 6. All three keep the behaviour Task 2 shipped — the
    // drop does nothing — rather than acquiring a half-implemented one here;
    // NONE of them shows a banner yet, and Task 6 is where the existing
    // BATCH_MIXED_MESSAGE refusal gets wired to this gesture.
    if (operands.batch || spanListKinds(operands.els).anyLi) return;
    const liveSrcEl = operands.els[0];
    // ── the HOME positions, before every gate ──────────────────────────
    // A drop that lands where the block already is is a silent byte no-op —
    // NOT a refusal and NOT a dropped gesture. It has to be answered first,
    // and MEASURED, because both later gates get it wrong on their own:
    //   * `before-block <sourceIndex>` — released on the block's own top edge,
    //     which is exactly what the drag's "released where it started"
    //     scenario drives — makes the destination the SOURCE, and the
    //     destination's index in the source-less array below is then -1, i.e.
    //     the "文件已更新，請重試這個操作" banner, for a gesture in which
    //     nothing whatsoever went wrong. Caught by driving a real gesture and
    //     reading the banner back; the byte assertions could not see it,
    //     because the file is untouched either way.
    //   * `before-block <sourceIndex + 1>` — the seam immediately below, the
    //     same position on screen — is a no-op planBlockMove() answers null
    //     for, but the list-seam gate below would have refused it OUT LOUD
    //     first whenever the block sits between two list items.
