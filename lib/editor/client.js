...
(function () {
  ...
  // attempt to open a block's editor (any block) hits the "a different
  // editor is open" guard above forever (a real regression found in review: silent total lockout,
  ...
  function openRawEditor(blockEl) {
    ...
    async function commit() {
      ...
      if (!ok) {
        ...
        // pre-edit content back on screen so state stays consistent with
  ...
  function buildBar() {
    ...
    editBtn.addEventListener('click', (e) => {
      ...
      dismissBar();
      if (blockEl) openRawEditor(blockEl);
    });
  ...
  // Single delegated listener, wired once at the bottom of this file.
  function wireBlockSelection() {
    document.addEventListener('click', (e) => {
      if (!e.target || !e.target.closest) { dismissBar(); return; }
      ...
      if (!blockEl) { dismissBar(); return; } // clicked outside any block
      if (blockEl.querySelector('.ed-raw')) return; // this block's raw editor is already open
      showBarFor(blockEl);
    });
  }
