...
(function () {
  ...
  async function openRawEditor(blockEl, range) {
    ...
    async function cancelAndMaybeDiscard() {
      ...
      // branch it does `blockEl.innerHTML = original`, and for rangeMode a
      // full rerender — either way the very textarea this closure holds is
      // gone by the time a Ctrl+Z could ask for it back. Skipped for a
      // pristine insert: discardPristineInsert() (below) REMOVES the block
      // outright, so there is no surface left to restore a stash INTO —
      // mirrors revertBurstAndEnd()'s identical wasPristineForThisBlock
      // exception for the WYSIWYG burst twin. Skipped when nothing was
      // actually typed (ta.value === seed.value): the zero-edit
      // auto-cancel-on-unchanged-blur path in resolveOpenSession() reaches
      // this same function, and there is nothing to lose there.
  ...
  // question. What has to hold instead is exactly what reanchor() (this
  // same file, openRawEditor()'s own re-derivation for a session that
  // survives a patch) already asks: the stashed block id still names a
  // block at the stashed span, and the bytes at that span are still
  // byte-identical to what Escape saw — i.e. nothing else committed over
  // this exact span in between. Returns false whenever there is nothing to
  // restore or the stash has gone stale; the caller then falls through to
  // the ordinary document undo, same contract as restoreDiscardedBurst().
