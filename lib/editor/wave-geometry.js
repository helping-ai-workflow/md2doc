 ...
 * （它問 `codec.lanePaths`），第二份走樹邏輯會跟它漂移。`codec` 是參數，不是
 * 這個檔案自己 `require` 的：介面把它當輸入,呼叫端傳進來的必須是同一顆 codec。
 *
 * 引用到不存在字母的 edge 直接跳過：那是一份使用者手寫壞掉的文件，畫不出那條線
 * 是對的，讓整張圖倒掉不是。
 */
function edgeLayout(doc, layout, codec) {
  const out = [];
  if (doc === null || typeof doc !== 'object' || !Array.isArray(doc.edge)) return out;
  if (layout === null || typeof layout !== 'object') return out;
...
function pathFor(shape, from, to) {
  ...
  if (has('|')) {
    const midX = has('-|-') ? (from.x + to.x) / 2 : to.x;
