...
function createPanels(deps) {
   ...
   * showing.
   *
   * v3.6.0 Task 18: `config.hscale` and a lane's `period` / `phase` used to
   * head this list — three properties that change what the engine paints and
   * that the drawing ignored. Task 4 made all three part of `layoutOf`'s
   * per-lane geometry (`cycleWidth` carries `period * hscale`, `originX`
   * carries `phase`), and `test/wave-draw.test.js`'s `period: 4` and
   * `hscale: 2` fixtures pin the ruler and grid pitch against the engine's
   * own, so naming them as "not shown" is now the false statement. They are
   * gone from the list; `found` stays as the place the NEXT such property
   * lands, and is empty today.
   *
   * The brief for this task asked instead for a notice when the skin ratios
   * could not be derived (`SKIN_METRICS.derived !== true`). That field does
   * not exist: controller ruling R12 (Task 3) cancelled the module-load
   * derivation, because the shipped `wavedrom.min.js` attaches no `waveSkin`
   * to the page global, so a page-time derivation would read `undefined`
   * every time and the notice would be permanent wallpaper rather than a
   * signal. The ratios are pinned from a measurement, and
   * `test/wave-geometry.test.js` re-derives them from the real package on
   * every run — a wavedrom bump that moves the skin turns that test red and
   * names the number to re-pin. That is where the failure is reported,
   * because it is a build-time fact and there is no runtime state for a
   * notice to read.
   *
  ...
  function renderUnmodelled(doc, layout) {
    // Empty today — see the block comment above for what used to be pushed
    // here and why it no longer is. Still assembled (and still published as
    // `data-wave-unmodelled`) so the next genuinely-unmodelled property has
    // somewhere to land, and so a test can tell "nothing to report" from
    // "this notice was never rendered at all".
    const found = [];
  ...
  function laneRow(doc, layout, i, state) {
    ...
    // Deliberately NO `data-focus-key`. `restoreFocus` nominates a key and
    // calls `.focus()` on whatever carries it; a <span> with no tabindex
    // cannot take focus, so a key here would be a silent route to
    // `document.body` the moment anything nominated it — the exact state
    // that turns the next Backspace into「delete the selected blocks」.
    handle.textContent = '⠿';
    // v3.6.0 Task 16: this tooltip is the reorder affordance's own description
    // of how reordering is done, so it names BOTH doors — the drag below and
    // `wave-ui.js`'s Alt+↑/↓, which reach the same `codec.moveLane`.
    //
    // fix round 1 WITHDRAWS the reason the first cut gave for this line
    // ("leaving it drag-only would leave the keyboard route discoverable
    // nowhere"). That was false six lines above its own head: this is a
    // HOVER tooltip on a `<span>` that deliberately carries no
    // `data-focus-key` and cannot take focus, so a keyboard-only user never
    // reads a character of it. What the line is actually worth runs the
    // other way — the person already reaching for the handle with a mouse
    // learns there is a cheaper route. The three surfaces a KEYBOARD user
    // does read are the canvas's `aria-label` (`wave-draw.js`'s
    // `renderCanvas`), the opening status line, and `handleDrawingKey`'s own
    // docstring (both in `wave-ui.js`); fix round 1 is where those three
    // were made true.
    handle.title = '拖曳這裡可以調整這條 lane 的順序（或選起來按 Alt+↑／Alt+↓ 搬）';
