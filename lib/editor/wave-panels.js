...
function createPanels(deps) {
  ...
  function laneRow(doc, layout, i, ungroupSpan, state) {
    ...
    // v3.6.0 Task 6: same fix as the group tag above — `layout.lanes[i].y`
    // carries the ruler/head-title band's offset, `i * layout.laneHeight`
    // does not. Left as `i * layout.laneHeight` this row would sit
    // `layout.originY` px too high, out of step with its own canvas row.
  ...
  function insertButton(doc, index, layout, isTail) {
    ...
    if (isTail) {
      // v3.6.0 Task 6: the tail insert sits one row past the last lane —
      // `layout.originY + layout.lanes.length * layout.laneHeight` is that
      // position with the ruler/head-title band's offset folded in. Not
      // `layout.lanes[layout.lanes.length - 1].y + layout.laneHeight`: that
      // reads `lanes[-1]` and throws when the document has zero lanes, which
      // this tail button is drawn for too (`renderLanes` always appends one).
      b.style.top = (layout.originY + layout.lanes.length * layout.laneHeight) + 'px';
    }
