...
// 逃生口" (whole-document source escape hatch): 編輯 ⇄ 原始碼.
// v3.2.1: 預覽（preview）已移除，見下方 MODES 上的說明。
//
...
// v3.2.1: 兩態循環 edit <-> source。preview 移除 —— 實測它相對 edit 只做
// 兩件事（applyPreviewEditability(false) 與兩行隱藏 ⠿／＋ 的 CSS），卻擋在
// source 回 edit 的路上，而且 22 顆工具列按鈕中 13 顆在它底下仍然寫得進磁碟。
//
// 為什麼是移除而不是加閘門：一個「列舉可及面」推導出來的唯讀閘門必然漏掉
// 一整族 —— 切換前就已經開著的 ⠿ 選單在切換後仍是 140x118、display:flex，
// 其 16 個項目每一個都寫得進磁碟。閘門要約 8 個守衛點跨 2 個檔案才擋得住，
// 而 preview 本身沒有任何 edit 給不了的能力。
//
// An unrecognized mode (defensive: a caller passing something stale) falls
// back to the first state rather than throwing, matching how the rest of
// this module prefers a safe default over a thrown exception for malformed
// input.
function next(mode) {
