...
<style>
  ...
  .ed-block.ed-selected { outline: 2px solid #3b82f6; }
  /* Click-invoked edit bar: a single floating bar (moved between blocks by
     the client runtime, never re-created) anchored to the selected block's
     top edge. Visual language matches the lightbox's own .lightbox-bar /
     .lightbox-bar button above — dark translucent strip, bordered pill
  ...
  .ed-bar-btn:hover { background: rgba(255, 255, 255, 0.18); }
  .ed-bar-btn[hidden] { display: none; }
  /* Task 3: in-place WYSIWYG editing outline for the block's own content
     element (the <p>/<h#> tag itself becomes contenteditable — no textarea
     wrapper) plus a caret color that stays visible against normal prose. */
  .ed-wys {
    outline: 2px solid #3b82f6; outline-offset: 2px;
    caret-color: #3b82f6; cursor: text;
  }
  /* Task 5: a table CELL's editing outline must be INSET (negative offset),
     not the paragraph editor's outset default above — the enclosing
     table element has overflow-x: auto (see the table ruleset below),
     which would clip an outset outline on any cell near the table's
     horizontal edges. */
  .ed-block[data-block-type="table"] .ed-wys {
    outline-offset: -2px;
  }
  /* Phase 3 Task 2: always-on paragraph/heading editing. Every eligible
  ...
  /* Phase 3 Task 2: the ⠿ block-actions handle — one real per-block node
     (not a floating, JS-repositioned element like .ed-bar/.ed-seltb), sat
     in the block's left gutter and revealed on hover. Only ever a
  ...
  /* The ⠿ handle's small menu: heading ± / MD 原始碼 / close. Same visual
     language as .ed-bar (dark translucent pill, bordered icon buttons). */
  .ed-handle-menu {
  ...
  /* Task 4: floating selection toolbar (bold/italic/code/link), shown over a
     ...
     computes via Range.getBoundingClientRect(). Visual language matches
     .ed-bar above (dark translucent pill, bordered icon buttons); z-index
     sits above .ed-bar (5) so both can be visible at once, but below
     .ed-conflict (999). */
  .ed-seltb {
