...
` : ''}
  ...
  // The width the diagram was actually drawn at, in its own user units.
  //
  // Both halves are load-bearing and were MEASURED, not assumed. The waveform
  // engine emits width="2480". Mermaid emits width="100%" and keeps the real
  // number in the viewBox ("0 0 6838.25 70" for a 40-node flowchart here), so
  // a parseFloat of the width attribute reads 100, every mermaid diagram comes
  // out "nine times wider than the column is available", and the floor below
  // silently never applies to any of them.
  function diagramNaturalWidth(svg) {
    ...
    // them for you: this whole runtime is emitted from a JS template literal,
    // where "\s" is an unrecognised escape and collapses to a bare "s". The
    // first draft of this line shipped /^s*[0-9.]+(px)?s*$/ and a viewBox
    // split on /[s,]+/, which parsed "0 0 6838.25 70" as one token, returned
    // 0, and turned the mermaid half of this feature into a silent no-op.
    // Precedent: the normaliseText helper above spells the same thing /\\s+/g.
