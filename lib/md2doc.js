...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  //
  // GEOMETRY, AS MEASURED (S4 T1; the previous version of this comment
  // claimed "an 8px gap" between the gutter and the table row grip, a number
  // left over from the v2.11.0 pair position that v2.11.1 invalidated — the
  // real gap was MINUS 6px, i.e. an overlap):
  //   ⠿          [contentLeft-22, contentLeft-4]   (--ed-gutter-btn wide)
  //   row grip   [contentLeft,    contentLeft+20]  (--ed-te-grip-row-w wide)
  // The grip used to be centred on the table's left border at
  // [contentLeft-10, contentLeft+10], which overlapped the ⠿'s right 6px and
  // — being position: fixed, z-index 7, on document.body — won the hit test
  // there. spec §4.2 conflict 2's ruling (grip entirely inside the table)
  // is landed in lib/editor/client.js's updateTableEdgeGrips(); the
  // clearance that remains is exactly --ed-gutter-gap, because the ⠿'s right
  // edge and the grip's left edge are both derived from it. The grip is kept
  // off the first cell's TEXT by --ed-te-cell-pad-left below, not by the
  // 14px default padding, which is narrower than the grip.
  ...
  /* 56px, not 48px — spec §4.2's hit-test conflict 1 (gutter vs
     ...
     the one the §4.2 conflict-1 guard is written against.) The table row grip no
     longer reaches this far left at all: §4.2 conflict 2 moved it inside the
     table to [contentLeft, contentLeft+20] (S4 T1), so nothing but the gutter
     pair itself occupies [contentLeft-40, contentLeft-4]. Guarded by the
     elementFromPoint(splitter.right - 2, y) assertion §4.2 asks for, in
  ...
  /* D6: ＋ immediately left of ⠿, on the same row, no gap — replacing the
     ...
     reaches a negative viewport x and never covers the splitter. The table
     row grip is at [contentLeft, contentLeft+20] since §4.2 conflict 2
     landed (S4 T1), so the clearance from the ⠿'s right edge is exactly
     --ed-gutter-gap and cannot drift: both edges are derived from it. */
  ...
  :root {
    ...
    /* spec §4.2 hit-test conflict 2, landed S4 T1. The table row grip now
       sits ENTIRELY inside the table (its LEFT edge on the table's left
       border — see updateTableEdgeGrips() in lib/editor/client.js), which
       leaves the ⠿'s right edge and the grip's left edge exactly
       --ed-gutter-gap apart: the 4px breathing gap §4.2 already keeps on the
       right of the gutter pair IS the separation. Nothing new is written
       down, so nothing new can drift.
       The consequence is this token: the grip is 20px wide and the first
       column's default padding is 14px, so an inset grip would sit on that
       cell's TEXT — the v2.10.1 user-acceptance defect that got the first
       attempt at this ruling reverted. The first column is padded past the
       grip instead, by the grip's own width plus that same gap. */
    --ed-te-cell-pad-left: calc(var(--ed-te-grip-row-w) + var(--ed-gutter-gap));
  }
  .ed-handle {
  ...
  /* §4.2 conflict 2's other half: the row grip lives inside the first column
     now, so that column's cells must be padded past it. EDIT MODE ONLY —
     reader HTML and PDF output keep the shared 7px 14px and stay
     byte-identical. Only the LEFT padding changes, and only on the first
     cell of each row; every other cell is untouched. */
  .ed-block[data-block-type="table"] th:first-child,
  .ed-block[data-block-type="table"] td:first-child {
    padding-left: var(--ed-te-cell-pad-left);
  }`;
...
<style>
  ...
  .ed-handle-menu-btn {
    ...
    /* v2.12.0: a flex ROW so the icon and the label share a baseline-free
       ...
       justify-content: flex-start is explicit, not the initial value: a
       lone flex item (the old icon-less submenu row) rendered centered
       under the default value normal, which is why text-align: left
       above used to be the only thing holding the label to the left
       edge. Now that every row has two items (icon + label), flex-start
       is what pins the icon flush left regardless of label length. (No
  ...
  /* Notion-style grip handles: replace the original invisible TE_EDGE_PX=8
     ...
     the header is draggable too), sitting ENTIRELY INSIDE the table with its
     LEFT edge on the table's left border (spec 4.2 hit-test conflict 2,
     landed in S4 T1). The two axes are therefore NOT mirror images any more:
     '.ed-te-grip-col' still straddles the table's TOP border, because nothing
     of the page's own chrome lives up there. The row axis cannot: MEASURED on
     v2.12.0 at 1400x900, the straddling grip occupied [tableLeft-10,
     tableLeft+10] while the block's own '.ed-handle' occupied [blockLeft-22,
     blockLeft-4] with tableLeft == blockLeft, so the grip (position: fixed,
     z-index 7, on document.body) won the hit test for the RIGHT 6px of every
     ⠿ whose Y band it reached. The first column's own padding is widened in
     the edit-mode-only layout chunk near the end of this stylesheet so the
     inset grip still never reaches cell TEXT; '.ed-te-grip-col' is a horizontal 6-dot
  ...
  /* spec §4.2 hit-test conflict 2 (S4 T1): the row grip's width is a TOKEN
     because two other numbers are derived from it — lib/editor/client.js pins
     the grip's LEFT edge to the table's left border (so the whole grip is
     inside the first column and shares no pixel with the block gutter's ⠿),
     and the edit-mode first-column padding must be wide enough to host it
     without covering cell text. Written once here, derived twice below. */
