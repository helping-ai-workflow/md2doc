...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  /* 56px, not 48px — spec §4.2's hit-test conflict 1 (gutter vs
     ...
     both worked. Worse, .ed-insert is only 20px tall and li rows have none at
     all, so the dead zone was striped both vertically and within a row: the
     same x resized the sidebar or silently opened an insert menu depending on
     which pixel row the user grabbed.
