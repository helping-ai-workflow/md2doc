...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  .ed-wave-lane-row {
    ...
    gap: 3px; padding-right: 6px; box-sizing: border-box;
  }
  .ed-wave-lane-row.is-selected { background: #eaf2fe; }
  /* v3.5.0 Task 12: Shift+click multi-select for group creation — a
     distinct tint from .is-selected above (that one is the CYCLE selection
     the brush/cycle-ops toolbar acts on; this one is a SET of whole lanes
     the group buttons act on, and a user with both standing at once — a
     cycle range selected on a lane that is also multi-selected for
     grouping — needs to tell which is which at a glance). A lane can carry
     both classes together, so this is additive (a left border) rather than
     a second background that would just overwrite .is-selected's. */
  .ed-wave-lane-row.is-multi-selected { border-left: 3px solid #7c3aed; }
  .ed-wave-lane-name { flex: 1 1 auto; min-width: 0; font-size: 12px; padding: 2px 4px; }
  .ed-wave-lane-row button {
    border: 1px solid #cfcfcf; border-radius: 3px; background: #fff;
    font-size: 11px; line-height: 1; padding: 3px 4px; cursor: pointer;
  }
  .ed-wave-lane-row button:disabled { opacity: .4; cursor: default; }
  .ed-wave-lane-add-tail { position: absolute; left: 6px; }
