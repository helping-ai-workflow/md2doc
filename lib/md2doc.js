...
// the first block of every TOP-LEVEL list token. Spec §3.8's three operational
// rules cannot separate two ADJACENT top-level lists of the same type —
...
// re-flowing a list the user never touched (caught by
// test/editor-client-runtime.test.js's two-list fixture, whose second list is
// written with '*' bullets). The renderer is the only place that still knows
// the token boundary, so it stamps it, and the client's run scan treats it as
// rule (d): a run never crosses a data-list-start.
...
// Rule (c): any non-li block terminates every open run.
...
function renderEditModeList(listToken, blocks, biRef, out) {
  const parts = out || [];
  // A TOP-LEVEL list token (out === undefined) begins a new list: close every
  // open run so its first block is stamped, and remember to stamp it.
  let listStart = !out;
  if (listStart) resetLiRunState(biRef);
...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  // covering the first cell's text. 48px of content padding plus moving both
  // gutter buttons out to left:-36px separates them properly: the gutter
  // occupies [contentLeft-36, contentLeft-18] and the 20px-wide row grip
  // straddles contentLeft at [contentLeft-10, contentLeft+10] — an 8px gap,
  // and the grip's inner half stays within the cell's own 14px padding, so
  // it never touches cell text.
...
<style>
  ...
  /* S1: a list item is a flat <div class="ed-block" data-block-type="li">,
     ...
     ordinal, the --ed-indent padding and the run-scoped ordered counter are
     Task 5's; this task only emits their hooks (see renderEditModeList()). */
