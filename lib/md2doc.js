...
// v3.4.0 batch2 final review R9 — the deferred-diagram placeholders.
//
// A placeholder is written by the synchronous marked() pass and read back by
// an async post-pass roughly four thousand lines away in this same file
// (bakeDrawio / bakeGraphviz). The two sites used to agree only by
// character-for-character attribute ordering, and nothing failed if they
// drifted: a matcher that no longer matches collects zero jobs, the post-pass
// returns the string untouched, and the placeholder — which carries the
// base64 of the WHOLE source .drawio file or dot block — ships inside an
// empty div. Silent, and invisible to every test that only asserts on the
// baked shape.
//
// So the emitter and the matcher are both generated from one ordered field
// list. Adding, renaming or reordering an attribute changes both at once;
// there is no edit that can move one without the other.
//
// Two rules the field list relies on:
//   - every value must already be safe inside a double-quoted attribute —
//     base64 for anything free-form (the source, the alt text), digits for
//     the page index;
//   - a field is never omitted. A field with nothing to say carries the empty
//     string, so the matcher's arity is fixed and "this one had no alt text"
//     cannot degrade into a no-match.
function makePlaceholderShape(className, fields) {
  const open = `<div class="${className}" `;
  const quoteRe = (str) => str.replace(/[.*+?^${}()|[\]\\]/g, (c) => '\\' + c);
  return {
    // `probe` is the cheap "is there anything to do at all?" test the bake
    // passes run before doing any work. Derived from the first field rather
    // than spelled out again, for the same no-drift reason as `re`.
    probe: new RegExp(quoteRe(fields[0][0]) + '='),
    html(values) {
      return open + fields.map((f, i) => `${f[0]}="${values[i]}"`).join(' ') + '></div>';
    },
    re: new RegExp(
      quoteRe(open) + fields.map((f) => quoteRe(f[0]) + '="' + f[1] + '"').join(' ') + '><\\/div>',
      'g'),
  };
}

const DRAWIO_PLACEHOLDER = makePlaceholderShape('drawio', [
  ['data-drawio-src',  '([^"]*)'],
  ['data-drawio-page', '(\\d+)'],
  // v3.4.0 batch2 final review R5 — the markdown author's alt text, base64 of
  // the already-HTML-escaped string marked hands us, so it survives the trip
  // to bakeDrawio() and becomes the baked box's accessible name.
  ['data-drawio-alt',  '([^"]*)'],
]);

const GRAPHVIZ_PLACEHOLDER = makePlaceholderShape('graphviz', [
  ['data-graphviz-src', '([^"]*)'],
]);
...
async function renderMarkdown(mdText, srcPath, opts = {}) {
  ...
  let usesDrawio = false;
  // Set only when some drawio reference actually resolves to 2+ pages — the
  // sheet bar (CSS + runtime) is gated on this, not on usesDrawio alone, so a
  // single-page .drawio file never pays for multi-page UI.
  let usesDrawioMultiPage = false;
  // v3.4.0 batch2 Task 6: every `.drawio`/`.xml` source path this render
  // actually resolved and read, so a caller (lib/editor/server.js) can stat
  // them later to notice an external edit — resolveAssetPath()'s result was
  // previously a pure local inside drawioPlaceholderFor(), never surviving
  // past the base64 it gets embedded as (see the investigation note this
  // task shipped with). A Map, not an array, so the same referenced file
  // linked from two different images is only stat'd once.
  //
  // Fix round 1 (review F4): the VALUE is the file's stamp taken here, at
  // read time, not later by the caller. bakeDrawio() launches headless
  // Chromium and bakes every page after this function returns — seconds, not
  // microseconds, on first use — so a stamp taken by the caller AFTER the
  // bake records bytes that were never baked, and every later comparison
  // then answers "up to date" about a revision nobody ever rendered. Taken
  // BEFORE the readFileSync below for the same reason in miniature: a stamp
  // that is older than the bytes we baked costs one redundant re-render,
  // while one that is newer loses the change permanently.
  const drawioRefs = new Map();
  let usesMath = false;
  let blocks = null;
  // ── Local image assets ───────────────────────────────────────────────────────
  ...
  // v3.4.0 §4. drawio (.drawio / .xml) goes down the graphviz-style path — a
  // placeholder carrying the source, baked to inline SVG by an async post-pass
  // — instead of image inlining. Returns the placeholder markup, or null to
  // mean "not a drawio reference, fall through to the normal image path"
  // (missing file, non-drawio content, or an extension we don't touch at all).
  // Must run BEFORE inlineImageSrc(): that function warnOnce()s "not a known
  // image extension" for anything outside IMAGE_MIME_BY_EXT, and .drawio/.xml
  // are not image extensions — by the time control reached inlineImageSrc it
  // would already be too late.
  function drawioPlaceholderFor(href, altText) {
    if (!href || isExternalRef(href)) return null;
    const { filePart, fragment } = splitAssetRef(href);
    if (!filePart) return null;
    ...
    const abs = resolveAssetPath(filePart);
    if (!abs) return null;   // No such file: let the ordinary image path warn.

    // Stamp BEFORE the read — see the drawioRefs comment above.
    const stamp = drawio.drawioStampOf(abs);
    let xml;
    try { xml = fs.readFileSync(abs, 'utf8'); }
    catch (e) { return null; }

    if (!drawio.isDrawioXml(xml)) {
      // Extension matched but the content didn't — worth a warning for
      // .drawio (that extension promises mxfile/mxGraphModel content), but
      // not for .xml (most .xml files are not diagrams, so silently falling
      // through to the normal image/HTML path is the right default).
      if (ext === '.drawio') {
    ...
    usesDrawio = true;
    drawioRefs.set(abs, stamp);
    const names = drawio.pageNamesOf(xml);
    if (names.length >= 2) usesDrawioMultiPage = true;
    // splitAssetRef() keeps the leading '#' (it's re-appended verbatim to a
    // data: URI elsewhere), but resolvePageIndex() expects the bare page
    // name/number — strip it here rather than teach resolvePageIndex two
    // input shapes.
    const pageFragment = fragment ? fragment.slice(1) : '';
    const pageIndex = drawio.resolvePageIndex(names, pageFragment);
    const b64 = Buffer.from(xml, 'utf8').toString('base64');
    // v3.4.0 batch2 final review R5 — carry the author's alt text through to
    // bakeDrawio(), which turns it into the baked box's accessible name. It
    // travels base64-encoded for the same reason the source does: the shared
    // placeholder shape's contract is that every value is already safe inside
    // a double-quoted attribute, and an alt text can contain anything.
    //
    // `altText` is marked's `token.text`, which is ALREADY HTML-escaped
    // (MEASURED on marked 14: `![A "q" & <b>](x)` arrives as
    // `A &quot;q&quot; &amp; &lt;b&gt;`) — the same string the ordinary
    // `<img alt="${text}">` path below interpolates raw. So it must not be
    // escaped again here or on the way out.
    const altB64 = Buffer.from(String(altText || ''), 'utf8').toString('base64');
    return '\n' + DRAWIO_PLACEHOLDER.html([b64, pageIndex, altB64]) + '\n';
  }
  ...
  let bodyHtml;
  // v3.2.0: the per-block edit-mode parts (1:1 with `blocks`), already baked.
  // Stays null outside editMode — the reader/PDF path has no per-block DOM to
  // patch, only the whole-document `bodyHtml`.
  let parts = null;
  let tocHtml = '';
...
${itemsHtml}
    ...
    // Real HTML tags always have a letter (optionally after a `/`) immediately
    // after `<` — `<x>`, `</div>`, `<br/>`. This deliberately does NOT match
    // `<[^>]*>` generically: once flattenTokenText() started handing back
    // unescaped plain text (v3.4.0 §1), literal author text like "C < D > E"
    // would otherwise look like a tag and get silently eaten here.
    //
    // HTML comments (`<!-- ... -->`) don't start with a letter either, so they
    // need their own alternative — without it a heading like
    // "## Section <!-- note --> Title" leaks "note" into the slug
    // (v3.4.0 Task 1 fix round 1). Known gap, deliberately left open: this
    // does NOT special-case `<!DOCTYPE ...>` or `<? ... ?>` (processing
    // instructions) — same non-letter-after-`<` shape, but neither has shown
    // up in a real heading/prose fixture. Add an alternative here if one does.
    function stripHtmlTags(value) {
      return String(value || '').replace(/<!--[\s\S]*?-->|<\/?[a-zA-Z][^<>]*>/g, '');
    }
    ...
    renderer.image = function(token) {
      ...
      // v3.4.0 §4: drawio 走 graphviz 那條路（建置時烤成內嵌 SVG），不走圖片
      // 內嵌。判別看的是檔案的根元素而不是副檔名 —— ![](foo.xml) 只有在內容
      // 真的是 mxfile / mxGraphModel 時才是圖，否則原樣落到下面既有的路徑。
      const drawioOut = drawioPlaceholderFor(href, text);
    ...
    if (opts.editMode) {
      ...
      // v3.2.0: bake EACH part, then join — not join-then-bake. The patch path
      // inserts parts[i] straight into the DOM and the client has no graphviz
      // renderer (__md2docInitDiagrams handles only mermaid and WaveDrom), so
      // an unbaked part lands as an empty <div class="graphviz">. bakeGraphviz
      // short-circuits on parts with no data-graphviz-src, and each placeholder
      // falls entirely inside one part, so this is byte-identical to baking the
      // joined string.
      ...
      // 變成一個空的 <div class="drawio">。與 bakeGraphviz 不同的是 drawio
      // 需要一顆 headless Chromium，所以逐 part 依序（而非並行）烤，讓所有
      // part 共用同一顆延遲啟動的 browser，一份文件最多開一次。
      const drawioBrowser = makeLazyBrowserRef();
      try {
        for (let i = 0; i < parts.length; i++) {
          parts[i] = await bakeDrawio(parts[i], drawioBrowser);
        }
      } finally {
...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  //
  // Why it exists: the gutter buttons hang OUTSIDE the content box. 56px of
  // content padding plus moving the gutter buttons out puts the pair at
  // [contentLeft-40, contentLeft-4] (v2.11.1; it was
  // [contentLeft-54, contentLeft-18] until spec §4.2's own numbers were
  // restored), clear of .sidebar-splitter — see the .content rule below.
  //
  // GEOMETRY, AS MEASURED (v2.12.0/S4 T1; the previous version of this
  // comment claimed "an 8px gap" between the gutter and the table row grip, a
  // number left over from the v2.11.0 pair position that v2.11.1
  // invalidated — the real gap was MINUS 6px, i.e. an overlap):
  //   ⠿          [contentLeft-22, contentLeft-4]   (--ed-gutter-btn wide)
  //   row grip   [contentLeft-10,  contentLeft+10]  (--ed-te-grip-row-w wide,
  //              centred on the table's left border)
  // The grip straddling the table's left border overlapped the ⠿'s right 6px
  // and — being position: fixed, z-index 7, on document.body — won the hit
  // test there. v2.12.0/S4 T1's fix moved the grip entirely inside the table
  // instead (its left edge on the border), which fixed the overlap but
  // broke the row/column symmetry the user expected (the column grip still
  // straddles the table's TOP border) — reported as a v3.0.1 regression.
  // v3.0.1 fixes it at the source instead: the ⠿/＋ pair itself moves
  // further LEFT by --ed-gutter-shift below, which removes the overlap
  // without moving the grip at all, so the grip can straddle the border
  // again (see updateTableEdgeGrips() in lib/editor/client.js). The
  // clearance that remains is exactly --ed-gutter-gap, because the ⠿'s right
  // edge and the grip's left edge are both derived from it. The grip's
  // straddling half (10px) sits well inside the first cell's default 14px
  // left padding, so no extra edit-mode padding is needed any more — the
  // v2.10.1-era --ed-te-cell-pad-left token this comment used to describe is
  // gone.
  ...
  /* 56px, not 48px — spec §4.2's hit-test conflict 1 (gutter vs
     ...
     the one the §4.2 conflict-1 guard is written against.) v3.0.1 shifts the
     pair a further --ed-gutter-shift (10px) left, to [contentLeft-50,
     contentLeft-14], so the table row grip can straddle the table's own left
     border again instead of sitting inside it — see the --ed-gutter-shift
     comment above .ed-handle below. That still leaves 6px of clearance from
     splitterRight (contentLeft-56), so the conflict-1 guard still holds.
     Guarded by the elementFromPoint(splitter.right - 2, y) assertion §4.2
     asks for, in test/editor-client-runtime.test.js. */
  ...
  /* The MD 原始碼 raw editor replaces the item's children with one .ed-editing
     wrap, which is then this flex row's only child. Without a grow it shrank
     to the textarea's intrinsic size and .ed-raw's width: 100% resolved
     against that: 189px at every viewport width (measured), where every other
     block's raw editor spans the column. The marker, and with it the indent,
     is gone while the textarea is open, so it spans the full column at every
     depth; the source line's own leading spaces show the nesting. */
  .ed-block[data-block-type="li"] > .ed-editing { flex: 1 1 auto; min-width: 0; }
  ...
  /* D6: ＋ immediately left of ⠿, on the same row, no gap — replacing the
     ...
     reaches a negative viewport x and never covers the splitter. As of
     v3.0.1 the table row grip straddles the table's left border again
     (centred on it, [tableLeft-10, tableLeft+10] — see updateTableEdgeGrips()
     in lib/editor/client.js), and the pair itself has moved left by
     --ed-gutter-shift below so the clearance from the ⠿'s right edge is
     still exactly --ed-gutter-gap and cannot drift: both edges are derived
     from it. */
  /* Stated ONCE, as tokens, because §4.2's three numbers are not independent:
     ...
     breathing gap, and the hover zone below is exactly the gutter plus
     whatever --ed-gutter-shift moved the pair by. Writing any of them as a
     second literal is how the corridor comes back. */
  :root {
    --ed-gutter-btn: 18px;
    --ed-gutter-gap: 4px;
  ...
  /* v3.0.1: the pair moves LEFT by --ed-gutter-shift so the table row grip
     ...
     two rules below and the acceptance test all derive from it. Measured at
     1400x900 (README.md, first table): pre-shift ⠿=[390,408] ＋=[372,390]
     tableLeft=412; a 10px shift lands ⠿=[380,398] ＋=[362,380], grip
     (straddling, 20px wide, centred on tableLeft) = [402,422], leaving
     exactly --ed-gutter-gap (4px) between the ⠿'s right edge (398) and the
     grip's left edge (402), and ＋'s left edge (362) stays well clear of 0. */
  ...
  /* v2.11.1: the gutter's HOVER ZONE, and it is the reason the pair could move
     ...
     past the ＋, at every Y. Its span is derived from the SAME two tokens the
     pair's own offsets are: --ed-gutter-w (the pair's own width, the gutter
     proper) plus --ed-gutter-shift (how far v3.0.1 moved the pair left to
     clear the straddling table row grip). Both edges therefore track the
     pair automatically — the zone's left edge is exactly the ＋'s left edge
     and its right edge is the block's, with no literal repeated anywhere —
     so the two cannot drift apart. If they ever do, the corridor comes
     straight back: v3.0.1 shifted the pair to [blockLeft-50, blockLeft-14]
     while this rule still read the un-shifted --ed-gutter-w alone, leaving
     it spanning [blockLeft-40, blockLeft] — 10px short on the left of the
     very buttons it exists to keep reachable.
  ...
  .ed-block::before {
    ...
    left: calc(-1 * (var(--ed-gutter-w) + var(--ed-gutter-shift))); top: 0;
    width: calc(var(--ed-gutter-w) + var(--ed-gutter-shift)); height: 100%;
  }
  /* v3.1.0 §4: room for the fixed toolbar (.ed-toolbar, further up this
     stylesheet). The bar is position:fixed — NOT sticky — because
     lib/editor/client.js mounts it on document.body: rerenderAll() replaces
     .content's innerHTML wholesale, so a toolbar living inside the content
     column would be destroyed by the first commit with nothing to rebuild
     it. Fixed chrome takes no layout space, so the page has to make its
     own: this is the only place that offset is written. The mobile rule at
     max-width:1080px above already reserves 60px for .sidebar-toggle; the
     bar sits above it. That 60px is NOT re-reserved here: T11-2 hides
     .sidebar-toggle in edit mode (the rule further down this stylesheet), so
     in edit mode there is no button under the bar to clear, and adding the
     two offsets left a band of empty page nothing was ever painted into.
     Measured on the build that did add them, at 1080/1000/800 wide:
     .page-layout's computed padding-top was 104px against a 44px toolbar —
     60px of dead space, and a 105px gap between the toolbar's bottom edge
     and the first block against 45px at 1200 wide, where the desktop rule
     applies. The @media block below therefore still has to carry an explicit
     value, identical to the desktop one: the READER stylesheet's own
     'max-width: 1080px' rule sets padding-top: 60px for the drawer toggle it
     still shows, and deleting this override would hand the edit page that
     60px straight back.
     .reader-sidebar is sticky at top: 24px in the base stylesheet — pushed
     down by the same amount here so the outline does not slide under the
     bar, with its height reduced to match so it still ends at the viewport
     bottom. */
  .page-layout { padding-top: var(--ed-toolbar-h); }
  .reader-sidebar {
    top: calc(24px + var(--ed-toolbar-h));
    height: calc(100vh - 48px - var(--ed-toolbar-h));
  }
  @media (max-width: 1080px) {
    .page-layout { padding-top: var(--ed-toolbar-h); }
    .reader-sidebar { top: 0; height: 100vh; padding-top: calc(16px + var(--ed-toolbar-h)); }
  }
  /* The 大綱 button's DESKTOP half, on an attribute of its OWN.
     ⚠ It deliberately does NOT reuse body[data-sidebar-open], and the reason
     was measured rather than reasoned: the rule
     'body[data-sidebar-open] .sidebar-scrim { display: block; }' sits at BASE scope in this
     stylesheet, not inside the max-width:1080px drawer block. Giving that
     same attribute a second, desktop meaning therefore also paints
     .sidebar-scrim (position:fixed; inset:0; z-index:98; rgba(0,0,0,.35))
     across the whole viewport — the desktop page went grey and
     click-through-dead, and the scrim's own click handler (which calls
     setSidebarOpen(false)) meant the next click anywhere silently undid it.
     A private attribute, written by toggleOutlineSidebar() in
     lib/editor/client.js, leaves data-sidebar-open to the mobile drawer
     alone and lets #sidebar-toggle's aria-expanded stay truthful.
     The attribute names the non-default state ("hidden") because on desktop
     the outline is a sticky column that is SHOWN by default; the mobile
     drawer keeps its own "open" attribute for the same reason in reverse. */
  @media (min-width: 1081px) {
    body[data-ed-outline-hidden] .reader-sidebar,
    body[data-ed-outline-hidden] .sidebar-splitter { display: none; }
  }
  /* T11-2 step 3: this button sits at z-index 100 (base stylesheet, above)
     directly under .ed-toolbar's z-index 101. Measured at 1080x900 and
     800x900: document.elementFromPoint() at the button's own rect centre
     returns the toolbar, not the button, at both widths — a real pointer
     click never lands on it there, only a synthetic .click() does. The
     toolbar's own left/right are both 0, spanning the full viewport width,
     so nothing about that occlusion is a function of width; the two
     measured points are a sample of a band the toolbar covers edge to
     edge, not two lucky ones. It is safe to remove in edit mode because
     lib/editor/client.js's toolbar carries its own outline button
     (runToolbarAction()'s 'outline' case, which calls toggleOutlineSidebar(),
     the same function this button calls) which opens the identical drawer,
     and Task 8 gave that toolbar a keyboard cursor (Alt+F10) — the outline
     button is not among the ids Task 8 measured as stranding the caret on
     BODY after activation. This rule is edit-mode-only (editModeLayoutCss);
     reader-mode output never includes it, so the mobile drawer toggle there
     is unaffected. */
  .sidebar-toggle { display: none; }
  /* T11-2 step 5: enterSourceMode() (lib/editor/client.js) sets
     contentEl.hidden = true and mounts a plain textarea in its place, and
     setDocMode() writes docMode onto body as data-ed-mode. Measured on that
     build: with ten TOC links listed, clicking the last one moved neither
     the (hidden) reading position nor the textarea's own scrollTop
     (3924 before and after) — the click target was a normal anchor jump /
     scrollIntoView() reaching into a subtree rooted at .content, and
     .content's own display:none is what absorbed it; the same subtree
     relationship holds for a search result's jump target. Hiding the TOC
     nav and the search-results panel while data-ed-mode="source" removes
     the two controls whose only reachable target sits in that hidden
     subtree; the search input row (.reader-tools) is left as-is — driving
     it against the textarea's own line numbers instead is a separate
     feasibility question, not answered here. */
  body[data-ed-mode="source"] .toc,
  body[data-ed-mode="source"] .search-results { display: none !important; }

  /* ── v3.4.0 batch3 Task 6: the wavedrom editing surface ───────────────
     Both of these are mounted on document.body by lib/editor/client.js, and
     that is not a styling choice: .content's children are what the client
     reads back as its own render and what the WYSIWYG path serialises into
     the user's markdown, so a panel or a button parked in there ends up in
     their file. Everything below is therefore positioned against the
     viewport rather than against .content.

     z-index 998 sits UNDER .ed-conflict (999) on purpose — a save conflict
     has to stay reachable even while the wave editor is open — and over the
     toolbar's own band, because the editor is modal over the document. */
  .ed-wave-edit-btn {
    position: fixed; z-index: 12; padding: 3px 10px; font-size: 12px;
    border: 1px solid #bbb; border-radius: 4px; background: #ffffffee;
    color: #222; cursor: pointer; box-shadow: 0 1px 3px rgba(0,0,0,.2);
  }
  .ed-wave-edit-btn:hover { background: #fff; border-color: #888; }
  .ed-wave-overlay {
    position: fixed; inset: 0; z-index: 998; background: rgba(0,0,0,.45);
    display: flex; align-items: center; justify-content: center;
  }
  /* v3.6.0 Task 13: width cap raised from 1200px to 1920px — MEASURED
     (createEditorServer, four real viewports): at 1200px the cap itself
     was the binding constraint at 1280/1600/1920 alike (94vw there is
     1203/1504/1805px, all above the old 1200px cap), so every screen from a
     1280 laptop up to a 1920 monitor got the IDENTICAL canvas width — the
     exact complaint that opened this version ("a bigger monitor buys the
     user nothing"). 1920px is chosen so 94vw is the actual governor at
     every one of those four measured sizes (94% of 1920 is 1804.8, still
     under the new cap) — this number is a ceiling for a screen wider than
     that, not a value that binds within the measured range. At 800x600,
     94vw (752px) was already narrower than either cap, so that viewport is
     unchanged by this. See task-13-report.md for the re-measured
     toolbar/canvas table. */
  .ed-wave-panel {
    background: #fff; border-radius: 6px; width: min(1920px, 94vw);
    height: min(760px, 90vh); display: flex; flex-direction: column;
    box-shadow: 0 6px 28px rgba(0,0,0,.35); overflow: hidden;
  }
  .ed-wave-head {
    display: flex; align-items: center; justify-content: space-between;
    padding: 8px 12px; border-bottom: 1px solid #e2e2e2; font-weight: bold;
  }
  .ed-wave-close {
    border: 1px solid #bbb; border-radius: 4px; background: #f6f6f6;
    padding: 3px 12px; cursor: pointer;
  }
  /* Which way out keeps the work and which throws it away. Escape is a modal's
     universal close key and here it discards a whole session, so the header
     says so next to the button that does not. Normal weight against the bar's
     bold heading: it is a caption, not a warning the user has to clear. */
  .ed-wave-escape-hint {
    margin-left: auto; margin-right: 12px; font-weight: normal;
    font-size: 11px; color: #7a4b00;
  }
  .ed-wave-toolbar {
    display: flex; flex-wrap: wrap; gap: 12px; align-items: center;
    padding: 8px 12px; border-bottom: 1px solid #e2e2e2; background: #fafafa;
  }
  /* v3.6.0 Task 12: the eight captioned sections themselves. .ed-wave-tools
     is the row that holds them (the SAME element as .ed-wave-toolbar
     above -- that class still owns the bar's padding/border/background,
     this one owns only the group row's own gap). Each .ed-wave-tool-group
     is a bordered chip so eight adjacent sections read as eight things, not
     one longer wall of buttons; the caption sits to its buttons' LEFT (see
     wave-panels.js's toolGroup() for why not above). */
  .ed-wave-tools { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; }
  .ed-wave-tool-group {
    display: flex; align-items: center; gap: 4px;
    padding: 2px 6px; border: 1px solid #ececec; border-radius: 6px;
  }
  .ed-wave-tool-caption {
    font-size: 10px; color: #8a8a8a; letter-spacing: .5px;
    white-space: nowrap; margin-right: 2px;
  }
  .ed-wave-brushes, .ed-wave-meta {
    display: flex; flex-wrap: wrap; gap: 4px; align-items: center;
  }
  /* v3.6.0 Task 10: widened from a 28x28 bare-character square to a small
     column (glyph on top, the character kept as a caption underneath) so
     the 22-brush palette says what each brush paints instead of relying on
     the letter alone -- nine of those 22 (P, N, u, d, h, l, 2, 6-9) have no
     obvious on-canvas picture from the character by itself. */
  /* '.ed-wave-brushes .ed-wave-brush', not the bare class: '.ed-wave-toolbar
     button' below outranks a single class on specificity (class+type beats
     class alone) and would otherwise win the padding fight and squeeze the
     24px-wide icon into a content box narrower than it is. */
  .ed-wave-brushes .ed-wave-brush {
    width: 34px; height: 40px; border: 1px solid #c4c4c4; border-radius: 4px;
    background: #fff; font-family: monospace; cursor: pointer;
    display: flex; flex-direction: column; align-items: center;
    justify-content: center; gap: 1px; padding: 2px 0; box-sizing: border-box;
  }
  .ed-wave-brush.is-on { background: #1a73e8; color: #fff; border-color: #1a73e8; }
  .ed-wave-brush-char { font-size: 11px; line-height: 1; }
  /* The glyph itself. Its shapes are drawn by wave-draw.js's levelIcon,
     which reuses renderCanvas's own drawing functions (bandOf, brickPath,
     the bus/clock/gap helpers) -- never a second hand-drawn set, so a
     change to how a brush paints on the real canvas cannot leave the
     toolbar showing the old picture. */
  .ed-wave-level-icon { display: block; overflow: visible; }
  .ed-wave-toolbar button {
    border: 1px solid #c4c4c4; border-radius: 4px; background: #fff;
    padding: 3px 8px; font-size: 12px; cursor: pointer;
  }
  .ed-wave-toolbar button:disabled { opacity: .45; cursor: default; }
  /* v3.5.0 Task 7: the 關聯線 arm button, armed or mid-drag. Same on/off
     treatment as .ed-wave-brush.is-on above — a picked tool reads the same
     way whichever toolbar it lives in. */
  .ed-wave-edge-arm.is-on { background: #1a73e8; color: #fff; border-color: #1a73e8; }
  /* v3.6.0 Task 13: the fixed 110px input width that used to live here moved
     into .ed-wave-section-body input below — every .ed-wave-metafield
     now lives inside a right-hand panel (關聯線/文件/Lane), not the toolbar,
     and a panel field should fill the ~340px column rather than sit at a
     width chosen for the old horizontal toolbar row. */
  .ed-wave-metafield { display: flex; align-items: center; gap: 4px; font-size: 12px; }
  /* v3.6.0 Task 13: the 關聯線 panel's own content, stacked vertically now
     that it lives in a narrow side column rather than a wide flex-wrap
     toolbar row (the old 'flex: 1 1 100%' here was what made it claim a
     full row of THAT row; a panel body has no siblings to share a row
     with). */
  .ed-wave-edgebar { display: flex; flex-direction: column; gap: 4px; }
  /* The "this drawing is not showing you everything" notice. period / phase /
     config.hscale all change what the ENGINE paints and none of them is
     modelled by the hand drawing, so a document carrying one says so instead
     of drawing a confident wrong picture. Full row, so it cannot be mistaken
     for another toolbar control. */
  .ed-wave-unmodelled {
    flex: 1 1 100%; font-size: 12px; color: #7a4b00; background: #fff6e0;
    border: 1px solid #e8c987; border-radius: 4px; padding: 4px 8px;
  }
  /* The stronger of the two notices: not "this drawing is incomplete" but
     "this block has nothing this editor can edit" (a reg: / assign: block).
     Same shape as its neighbour so the two read as one family, one step
     louder because the information gap is bigger. */
  .ed-wave-nolanes { color: #7a1f00; background: #ffe9e0; border-color: #e8a987; }
  /* v3.6.0 Task 13 fix round 1 (Critical 2): position: relative added.
     .ed-wave-side[data-expanded] below is position: absolute, and neither
     .ed-wave-panel nor (until this line) this element had a position of
     their own — so its containing block was .ed-wave-overlay
     (position: fixed; inset: 0), the WHOLE viewport, not the dialog.
     .ed-wave-panel's own overflow: hidden did not clip it either: clipping
     follows the containing-block chain, not DOM nesting, so the rail
     rendered against viewport corners at a narrow window and covered
     .ed-wave-head's 關閉 button — the only discoverable way to close the
     dialog (a hit-test at 關閉's own rectangle returned the edge panel's
     delete button instead; Escape still worked but was not discoverable).
     Anchoring the rail to THIS element rather than to .ed-wave-panel itself
     is deliberate, not incidental: .ed-wave-body sits below .ed-wave-head
     and above .ed-wave-status, so top: 0 / bottom: 0 on the rail now lands
     at body's own top/bottom — never over the header (關閉) or the status
     line — without this rule having to know either one's height. */
  .ed-wave-body { display: flex; flex: 1 1 auto; min-height: 0; position: relative; }
  /* v3.6.0 Task 13: the old comment here hand-computed the canvas's leftover
     width from the (now gone) .ed-wave-preview column's own flex-basis;
     that arithmetic no longer applies — the third column is .ed-wave-side
     below, which collapses to a 28px rail under 1100px specifically so the
     canvas is not squeezed the way a fixed 160px minimum used to squeeze it
     at 800x600 (MEASURED before this task: canvas width there was only
     282px). Re-measured after this task at four real viewports; see
     task-13-report.md rather than a formula here — a number derived by hand
     drifts the moment any sibling column's own width changes, which is
     exactly what this task did. */
  .ed-wave-lanes {
    flex: 0 0 200px; border-right: 1px solid #e2e2e2; overflow: auto;
    padding: 8px 0 8px 0;
  }
  /* The rows are absolutely positioned at index * laneHeight so they line up
     with the drawing beside them cell for cell. A group title is NOT a row —
     measured against the pinned engine in Task 2 — so it is drawn as a label in
     the rail spanning its lanes' rows instead, which is the shape the engine
     itself gives it (horizontal space in the name column, no vertical space). */
  .ed-wave-lane-rows { position: relative; }
  /* v3.6.0 Task 14: a row is a drag handle, a name field and (on a {}
     spacer) a badge. It used to be six buttons plus the field, and the six
     did not fit: flexed into what was left of 200px the field measured
     45px on the most crowded row, and a browser given a 24px box and a
     three-character Chinese caption lays that caption out VERTICALLY — the
     editor shipped columns of stacked glyphs where labels were meant.
     Those six are toolbar buttons now (the 訊號 / 群組 sections), acting
     on the one selection. */
  .ed-wave-lane-row {
    position: absolute; left: 0; right: 0; display: flex; align-items: center;
    gap: 6px; padding-right: 6px; box-sizing: border-box;
  }
  .ed-wave-lane-row.is-selected { background: #eaf2fe; }
  .ed-wave-lane-handle {
    flex: 0 0 auto; cursor: grab; color: #aaa; user-select: none; font-size: 13px;
  }
  .ed-wave-lane-handle:active { cursor: grabbing; }
  /* min-width, not just flex-grow: min-width is what a flex item will not
     shrink past, and shrinking past it is the vertical-text failure above.
     MEASURED after this change (real page, 800x600, the journey suite's own
     wave fixture): rail 201px, handle 9.5px, badge 36px, and the field comes
     out 178.5px ungrouped, 162.5px one group deep, 136.5px on the {} spacer
     row that also carries the badge — writing-mode horizontal-tb on all six.
     So 120 is slack at every depth this fixture reaches, NOT a computed
     exact fit. Deeper nesting spends 16px a level and would eventually hit
     the floor; there the field stops shrinking and .ed-wave-lanes scrolls,
     which is the trade this rule chooses on purpose. */
  .ed-wave-lane-name {
    flex: 1 1 auto; min-width: 120px; font-size: 12px; padding: 2px 4px;
  }
  .ed-wave-lane-badge { flex: 0 0 auto; font-size: 10px; color: #888; }
  .ed-wave-group, .ed-wave-group-input {
    position: absolute; width: 14px; padding: 0; font-size: 11px;
    writing-mode: vertical-rl; text-orientation: mixed; overflow: hidden;
    border: 1px solid #d8d8d8; border-radius: 3px; background: #f2f2f2;
    color: #555; cursor: pointer; box-sizing: border-box;
  }
  .ed-wave-group-input { width: 16px; height: 90px; cursor: text; }
  /* position: relative so the data-label field below (v3.5.0 Task 11) has
     an offsetParent to measure from: wave-ui.js reads canvas.offsetLeft /
     offsetTop against THIS element, not a hard-coded copy of the padding
     number just below, so the two cannot drift apart if that padding ever
     changes. */
  .ed-wave-canvas-wrap { flex: 1 1 auto; overflow: auto; padding: 8px; position: relative; }
  .ed-wave-canvas { display: block; }
  /* v3.5.0 Task 11: the bus data-label field, positioned by wave-ui.js's
     openDataEdit() from geometry.cellRect() — left/top/width/height are all
     set inline per cell, so only the visual skin lives here. Sized to sit
     exactly over the box it labels, text centred to match .ed-wave-buslabel
     beside it in the drawing underneath, box-sizing so the border does not
     push the field past the cell it is meant to cover. */
  .ed-wave-data-input {
    position: absolute; z-index: 1; box-sizing: border-box; text-align: center;
    font-size: 11px; padding: 0 2px; border: 1px solid #1a73e8; border-radius: 2px;
    background: #fff; color: #222;
  }
  .ed-wave-grid { stroke: #ececec; stroke-width: 1; }
  .ed-wave-line, .ed-wave-edge { stroke: #111; stroke-width: 2; fill: none; }
  /* v3.6.0 Task 10: the toolbar glyph's plain-level backbone (0/1/h/l/z/u/d
     only -- bus/clock/x/gap glyphs draw their own shape with their own
     fixed canvas colour instead, further down). Declared after the rule
     above on purpose: both are single-class selectors, so source order is
     what lets currentColor win over the fixed #111 for this one case,
     turning the icon white together with the rest of the button's text
     under .ed-wave-brush.is-on. */
  .ed-wave-level-icon-path { stroke: currentColor; }
  .ed-wave-weak { stroke-dasharray: 4 3; }
  /* v3.5.0 Task 10: the P/N explicit-edge arrowhead — see wave-ui.js's
     drawLane isClock branch for the measurement behind its shape and
     placement. Filled, no stroke, matching the line colour above. */
  .ed-wave-clock-arrow { fill: #111; stroke: none; }
  .ed-wave-x { fill: #d9d9d9; stroke: #111; stroke-width: 2; }
  .ed-wave-bus { fill: #f5e9c8; stroke: #111; stroke-width: 2; }
  .ed-wave-bus-3 { fill: #d8ead3; }
  .ed-wave-bus-4 { fill: #d6e4f7; }
  .ed-wave-bus-5 { fill: #f7d6e4; }
  /* v3.5.0 Task 11 fix round 2: this text, not the box under it, is the
     click target that opens the data-label field (wave-ui.js's onCanvasDown
     gates on ev.target.classList.contains('ed-wave-buslabel') — pressing
     anywhere else in the bus box still paints). pointer-events:
     visiblePainted is the SVG initial value already, so this changes
     nothing on its own; it is written out explicitly because that default
     is now load-bearing for a real gesture, not incidental — a future
     blanket pointer-events: none added upstream of this rule would
     silently make the label unclickable again, and this line is what a
     reviewer would have to notice and remove. */
  .ed-wave-buslabel { font-size: 11px; fill: #222; pointer-events: visiblePainted; cursor: text; }
  /* v3.5.0 Task 11 fix round 3: the hit target for an UNLABELLED bus run —
     drawLane draws this circle in place of the text when the run has no
     label, sharing the ed-wave-buslabel class above (same pointer-events,
     same cursor: text) so onCanvasDown's hit test needed no change. Kept
     visually quiet on purpose: dim and small, so a diagram full of
     unlabelled buses reads as buses first and marks second, not as a field
     of dots. Geometry (position, radius) is inline per cell, sized against
     that cell's own box in drawLane so it always stays a small fraction of
     it — this rule is skin only. */
  .ed-wave-buslabel-empty { fill: #666; opacity: .35; }
  /* v3.5.0 Task 11 fix round 4: this rect's job is to SHOW what cycle range
     is selected, never to be pressed. pointer-events: none, so it can never
     swallow a press meant for whatever it happens to be sitting over —
     reviewer-measured, a bus label whose run is one cycle wide can sit
     right up against this rect's own right edge (stroke included), and
     without this a second press on that same label landed on the
     selection's stroke instead of the label text underneath it, and the
     data-label field never opened. Same principle as .ed-wave-cursor two
     rules below, applied to the OTHER highlight this drawing draws. */
  .ed-wave-selection {
    fill: rgba(26,115,232,.18); stroke: #1a73e8; stroke-width: 1;
    pointer-events: none;
  }
  /* The KEYBOARD's own cell, and deliberately not the same mark as the
     selection: the selection is the RANGE a cycle operation acts on, the
     cursor is the ONE cell the next keystroke paints, and after Shift+left /
     right both are on screen at once. pointer-events: none so the rect can
     never swallow the press the drag underneath it is made of. */
  .ed-wave-cursor {
    fill: none; stroke: #d93025; stroke-width: 2; pointer-events: none;
  }
  /* The drawing is in the Tab cycle, so it has to SAY when it holds the
     keyboard: a cell cursor alone leaves a user who just Tabbed onto it with
     nothing that distinguishes "the drawing has the keyboard" from "the
     drawing is where I left a cursor before I walked away". */
  .ed-wave-canvas:focus { outline: 2px solid #1a73e8; outline-offset: 2px; }
  /* The discontinuity marker. The engine draws one gap <use> per pipe
     character; drawing none left the two pictures disagreeing on a lane whose
     levels agreed exactly. v3.5.0 Task 11 fix round 4 audit: drawLane's own
     comment already calls this "last so it sits over the trace it
     interrupts" — purely decorative, drawn on top purely to be seen, the
     exact shape of thing that swallowed a press meant for the bus label
     underneath it (see .ed-wave-selection above). A gap cycle inside a bus
     run can land at that run's own label position, so pointer-events: none
     here too, on the same principle, not because a concrete collision was
     reproduced. */
  .ed-wave-gap { stroke: #111; stroke-width: 2; fill: none; pointer-events: none; }
  /* v3.5.0 Task 6: edge: relationship lines, drawn by wave-ui.js's
     renderEdges() after every lane. Named .ed-wave-edge-* (with a hyphen)
     rather than .ed-wave-edge, which is already the brick-to-brick
     transition tick inside ONE lane (two rules above) — a different mark
     with a different meaning, and CSS class matching is exact-token so the
     two never collide, but the names are kept visually distinct too.
     Colours match the engine's own arc-shape.js style string
     (stroke:#0041c4) so a line drawn here and the same line in the
     WaveDrom preview beside it read as the same colour. */
  .ed-wave-edge-path { stroke: #0041c4; stroke-width: 1; fill: none; }
  .ed-wave-edge-path.is-on { stroke: #1a73e8; stroke-width: 2; }
  /* v3.5.0 Task 8: the edge an in-flight endpoint drag is dragging, drawn at
     its own ACTUAL (still-committed) position — the document does not
     change until the drop — but faded so it reads as "not where this will
     end up" once wave-ui.js's own dashed .ed-wave-edge-pending preview line
     (same mechanism Task 7's not-yet-written edge already uses) is standing
     next to it. Combines with .is-on: the dragged edge is always the
     selected one (renderEdges()'s own comment), so both classes are present
     together and this only lowers opacity on top of that colour. */
  .ed-wave-edge-path.is-drag-source { opacity: .35; }
  /* The invisible, wide-stroke click target layered over each visible path
     (see renderEdges()'s own comment) — pointer-events: stroke is what makes
     a transparent stroke hit-testable at all, since a stroke that paints
     nothing is not visiblePainted by the SVG default. */
  .ed-wave-edge-hit { stroke: transparent; stroke-width: 10; fill: none; pointer-events: stroke; }
  /* v3.5.0 Task 11 fix round 4 audit, closed in round 5: this text (drawn
     by renderEdges, AFTER .ed-wave-edge-hit) is the same shape of hazard as
     .ed-wave-selection and .ed-wave-gap above — purely decorative, sits on
     top of the fat invisible click target a press aimed at SELECTING the
     edge is meant to land on. Round 4 found this but could not apply
     pointer-events: none to the bare class: ed-wave-edge-label is not this
     text's own class alone — wave-ui.js's sidebar edgeLabelInput (the real,
     keyboard-and-mouse text field for editing an edge's label) uses the
     IDENTICAL class name, and a blanket rule would have also landed on
     that input field and blocked clicking into it with the mouse.
     Round 5's fix: scope the SELECTOR by element instead of renaming
     anything. text.ed-wave-edge-label only ever matches the SVG <text>
     wave-ui.js draws on the canvas — the sidebar control is an <input>, a
     different element entirely, and keeps its own default pointer-events
     untouched no matter what class it shares. */
  text.ed-wave-edge-label { pointer-events: none; }
  .ed-wave-edge-label { font-size: 11px; fill: #333; }
  /* v3.6.0 Task 8: the canvas's own name column — a mirror of the rail's
     .ed-wave-lane-name / .ed-wave-group (see the comment above
     .ed-wave-lane-rows). Unlike .ed-wave-edge-label these two class names
     are not shared with any non-SVG control, so pointer-events: none goes
     straight on the class rather than needing the element-scoped selector
     that rule uses. MEASURED (task-8-report.md): the longest lane name in
     this repo's fixtures is 12 chars ("he said \"hi\""), which at 11px in
     both the app's default sans-serif and the worst case (monospace)
     measures well under the 110px a top-level name has to fit in
     (nameColWidth 120 - NAME_GAP 10), so SIZES.nameColWidth stays 120. */
  .ed-wave-lane-label, .ed-wave-group-label { pointer-events: none; }
  .ed-wave-lane-label { font-size: 11px; fill: #333; }
  .ed-wave-group-label {
    font-size: 11px; fill: #555; writing-mode: vertical-rl; text-orientation: mixed;
  }
  /* v3.6.0 Task 9: the cycle ruler and the head/foot bands — see
     wave-draw.js's drawRuler()/drawBand() comment for the geometry (tick on
     boundaries, tock at centres, both filtered by every) and why this is
     read-only with respect to the document. Purely decorative like the name
     column above it, so pointer-events: none on the same principle. The
     ruler's own fill matches the pinned engine's own muted class (fill
     #aaa, node_modules/wavedrom's waveSkin CSS) so a tick/tock painted here
     reads the same shade as the identical mark in the WaveDrom preview
     beside it; the head/foot titles use the darker ed-wave-lane-label shade
     instead, matching the engine's own plain black fill for head.text /
     foot.text closely enough at this scale without introducing a third
     grey. */
  .ed-wave-ruler-tick, .ed-wave-ruler-tock, .ed-wave-head-text, .ed-wave-foot-text {
    pointer-events: none;
  }
  .ed-wave-ruler-tick, .ed-wave-ruler-tock { font-size: 11px; fill: #aaa; }
  .ed-wave-head-text, .ed-wave-foot-text { font-size: 11px; fill: #333; }
  .ed-wave-edge-handle { fill: #1a73e8; stroke: #ffffff; }
  .ed-wave-edge-arrow { fill: #0041c4; }
  .ed-wave-edge-tee { stroke: #0041c4; }
  /* v3.5.0 Task 7: the pending line an armed drag or keyboard two-step is
     standing on, from wave-ui.js's renderCanvas() — UI state only, never a
     document read, so it is dashed rather than the solid .ed-wave-edge-path
     colour to read as "not written yet". pointer-events: none so it can
     never itself become the thing a press lands on. */
  .ed-wave-edge-pending {
    stroke: #1a73e8; stroke-width: 2; stroke-dasharray: 5 4; fill: none;
    pointer-events: none;
  }
  /* v3.6.0 Task 11: the transition dots wave-draw.js's drawTransitions()
     paints while edge-creation is armed -- the visual answer to "where can
     I aim". pointer-events: none for the same reason ed-wave-selection and
     the other UI-state marks above are: the actual hit-testing runs through
     geometry.boundaryAt against the pointer's raw coordinates
     (wave-ui.js's cellFromEvent/onCanvasHover), never through this circle
     as a DOM target, so it must never itself swallow the press it exists
     to help aim. The [data-hot] variant is the same dot, larger and
     filled solid, for the one nearest the pointer right now
     (state.hoverBoundary in drawTransitions). */
  .ed-wave-transition {
    fill: #ffffff; stroke: #1a73e8; stroke-width: 1.5; pointer-events: none;
  }
  .ed-wave-transition[data-hot] { fill: #1a73e8; stroke: #0041c4; }
  /* v3.6.0 Task 13: the old flex-basis/border/padding here were for
     .ed-wave-preview as a direct flex child of .ed-wave-body; it now
     lives inside .ed-wave-section-body (the 預覽 panel), which already
     supplies the column width and padding, so only its own scroll behaviour
     is left to set — a wide diagram's preview SVG must scroll within the
     panel rather than stretching the whole 340px column. */
  .ed-wave-preview { overflow: auto; max-height: 260px; }
  .ed-wave-preview-label { font-size: 11px; color: #666; margin-bottom: 6px; }
  /* v3.6.0 Task 13: the right-hand column — five collapsible panels
     (關聯線/文件/Lane/預覽/原始碼) replacing the old direct
     .ed-wave-preview column. 'flex: 0 0 340px' (not '0 1'): unlike the
     old preview column, this one does not shrink when the canvas wants
     more room — width-budget wins go through .ed-wave-panel's own cap
     above, not by starving this column, because starving it re-introduces
     the exact 160px-minimum squeeze this task's own .ed-wave-lanes
     comment measured as the 800x600 defect. Below 1100px it collapses to a
     28px rail instead (media query below) for the identical reason. */
  /* v3.6.0 Task 13 fix round 4: box-sizing: border-box added. wave-ui.js's
     clearSelectedEdgeOfSide() sets an inline width (in border-box, i.e.
     rendered, pixels — the same unit getBoundingClientRect() reports
     everywhere else it measures this element) to keep the selected edge's
     own handle clear of this panel when expanded. Under the default
     content-box sizing, width excludes padding (16px) and the border
     (1px), so a JS-computed width landed 17px narrower on screen than
     intended — MEASURED, not assumed: an inline width of 310px rendered at
     327px. border-box makes width mean the same thing in CSS and in
     getBoundingClientRect(), so the two agree without wave-ui.js having to
     hardcode this rule's own padding/border numbers into its arithmetic. */
  .ed-wave-side {
    box-sizing: border-box; flex: 0 0 340px; border-left: 1px solid #e2e2e2;
    overflow: auto; padding: 8px; display: flex; flex-direction: column; gap: 8px;
  }
  .ed-wave-section { border: 1px solid #ececec; border-radius: 6px; }
  .ed-wave-section-title {
    display: block; width: 100%; text-align: left; border: 0; background: #fafafa;
    font: inherit; font-size: 11px; letter-spacing: .5px; color: #555;
    padding: 5px 8px; cursor: pointer; border-radius: 6px 6px 0 0;
  }
  .ed-wave-section-body { display: none; padding: 8px; }
  /* v3.6.0 Task 13 fix round 3: data-open is this SECTION's own toggle
     state (wave-panels.js's section()) and says NOTHING about whether the
     RAIL is currently showing it — those are two independent attributes on
     two different elements. A section can report data-open with a
     zero-size, unhittable body the whole time .ed-wave-side itself is
     collapsed to its <1100px 28px rail (below): 關聯線/文件/Lane all default
     data-open, and none of that changes just because the rail hides them.
     Left deliberately this way rather than making data-open track actual
     visibility — that would conflate "this section is open" with "the rail
     happens to be showing right now", and toggling ONE section would then
     have to silently relabel every OTHER already-open section's own state
     too. What actually closes the gap this could otherwise leave (a control
     that LOOKS reachable in the DOM but is not) is wave-ui.js's render()
     expanding the rail on the one transition that matters (selecting an
     edge) rather than this attribute pretending to mean something it does
     not. */
  .ed-wave-section[data-open] .ed-wave-section-body { display: block; }
  .ed-wave-section-body label { display: flex; align-items: center; gap: 6px; margin: 4px 0; }
  .ed-wave-section-body label span { flex: 0 0 96px; font-size: 12px; color: #555; }
  .ed-wave-section-body input {
    flex: 1 1 auto; min-width: 0; font-size: 12px; padding: 2px 4px;
  }
  .ed-wave-source {
    margin: 0; font-size: 11px; line-height: 1.45; white-space: pre;
    overflow: auto; max-height: 260px;
  }
  /* 視窗窄於這條線時右欄收成一條直立標籤，展開時覆蓋在畫布上 —— 否則
     800x600 的測試視窗下畫布會被 340px 固定寬的側欄吃掉大半，拖曳手勢會
     不可用（v3.6.0 Task 13 自己的 .ed-wave-lanes 註解量到的舊值：只剩
     282px）。收起時仍是普通文件流寬度（28px），展開時疊在畫布上方而不擠
     壓它。data-expanded 有三個來源：wave-panels.js 的 section() 點擊 title、
     同檔案的 collapseBtn 反過來移除它、以及 fix round 3 新增的
     wave-panels.js 的 expandSide()（由 wave-ui.js 的 render() 在
     selectedEdge 從 null 變成有值的那一拍呼叫——選取一條 edge 之後，
     整個回饋只長在這個側欄的關聯線面板裡，窄視窗下不展開就等於使用者
     什麼都看不到、也按不到）。 */
  /* v3.6.0 Task 13 fix round 1: the collapse-back button. Hidden everywhere
     except the one state it exists for — the <1100px rail, expanded — so it
     never shows, and never sits in the tab order, at a wide viewport or in
     the rail's own default collapsed state. */
  .ed-wave-side-collapse { display: none; }
  @media (max-width: 1100px) {
    .ed-wave-side { flex: 0 0 28px; padding: 4px 2px; }
    .ed-wave-side:not([data-expanded]) .ed-wave-section-body { display: none; }
    .ed-wave-side[data-expanded] {
      position: absolute; right: 0; top: 0; bottom: 0; width: 340px;
      background: #ffffff; z-index: 3; flex: 0 0 340px; padding: 8px;
    }
    .ed-wave-side[data-expanded] .ed-wave-side-collapse {
      display: block; width: 100%; text-align: right; border: 0;
      background: transparent; font-size: 12px; color: #555; cursor: pointer;
      padding: 2px 4px 6px;
    }
  }
  .ed-wave-status {
    border-top: 1px solid #e2e2e2; padding: 6px 12px; font-size: 12px;
    color: #444; min-height: 18px;
  }
  .ed-wave-parse-error { padding: 16px; }
  .ed-wave-parse-message { color: #b00020; font-weight: bold; }
  .ed-wave-parse-excerpt {
    background: #f6f6f6; border: 1px solid #e2e2e2; padding: 6px;
...
<style>
  ...
  /* The scroll LOCK for this same attribute lives inside the
     '@media (max-width: 1080px)' block below, next to
     'body[data-sidebar-open] .reader-sidebar { transform: translateX(0); }'
     — deliberately NOT here at base scope, unlike the rule immediately
     above. Reason (Ruling I1, review round 2): the mobile-drawer meaning of
     data-sidebar-open, including .sidebar-scrim's own display:block rule
     just above, already leaks past its breakpoint by design (see the
     ⚠ comment above '@media (min-width: 1081px) { body[data-ed-outline-hidden]
     … }' for why the desktop 大綱 toggle needed its OWN attribute instead of
     reusing this one — reusing it once already painted the whole desktop
     viewport grey and click-dead). Nothing here clears data-sidebar-open on
     resize (grep this file's two resize listeners — scroll anchor and
     lightbox fit — neither touches it), so a lock at base scope would ALSO
     survive: open the drawer narrow, widen the window, and the desktop page
     would go unscrollable on top of the pre-existing grey scrim. The grey
     scrim is that already-accepted gap; an unscrollable desktop page is not
     — it is new, and scoping the lock to the same breakpoint the drawer
     itself is scoped to is what keeps it from outliving the drawer. */
  .reader-tools,
  .search-results,
  .toc {
    border: 1px solid #d0d7de;
...
${usesDrawio ? `  /* v3.4.0 §4: a document with no drawio reference must not pay for this —
     conditional injection, same pattern as usesWaveDrom / usesMath below.
     .drawio needs position: relative because the (conditionally injected,
     see below) sheet bar is its absolute child — same relationship as
     .lightbox-stage / .lightbox-canvas, see CLAUDE.md. */
...
` : ''}${usesDrawioMultiPage ? `  /* Sheet bar: only when a document actually has a multi-page drawio
     diagram — a single-page file must not pay for it either. */
  .drawio-sheetbar {
...
` : ''}  .content img {
  ...
  /* A diagram wide enough that fitting it makes its own labels unreadable is
     drawn at natural size in a scroll box instead. The verdict is never "is
     this a waveform" -- it is "how small does fitting make the smallest
     glyph", because the damage is proportional to the aspect ratio. MEASURED,
     918px column: an 8-node flowchart fits at 0.71 and still draws 11.4px
     labels, while a 60-cycle waveform fits at 0.37 and draws 5.4px ones.
     applyDiagramScale() in the reader runtime below measures that and is the
     only thing that adds this class. The margin override is there because the
     ".content svg" rule centres with auto margins, which has nothing to
     centre once the svg is wider than the box.
     Source order is load-bearing: ".diagram-scroll > svg" and the
     ".content svg" rule above are both (0,1,1), so this wins only by coming
     after it. The child combinator matches the JS, which looks for the host's
     own ":scope > svg" -- one rule, stated twice, deliberately identically.
     One thing this rule does that it does not say: setting ONE axis to
     something other than visible promotes the OTHER axis to auto as well, per
     the CSS overflow spec, so every host here is a clipping block formatting
     context in BOTH directions. MEASURED on the three-engine fixture: all
     three scrolling hosts report a computed overflow-y of "auto" although only
     overflow-x is written, and all three have scrollHeight === clientHeight,
     so nothing is being clipped today. Recorded because a diagram that one day
     overflows VERTICALLY inside its host would be silently cut rather than
     overhanging, and neither this rule nor the tests would say why.
     (No backticks in this comment: it lives inside a JS template literal.) */
  .diagram-scroll { overflow-x: auto; }
  .diagram-scroll > svg { max-width: none; margin: 0; }
  ...
  /* The 'html, body { overflow-x: clip }' rule above keeps documentElement's
     own overflow-x at clip, not visible, so overflow does not propagate from
     body to the viewport per the CSS spec's root-propagation rule; the
     scrolling element stays documentElement, and its overflow-y sits at the
     visible default. That is the gap the body rule above does not close --
     measured in test/lightbox.test.js and test/editor-journey.test.js: a
     wheel over .lightbox-bar, PageDown and End all reach documentElement
     while it is open. Locking documentElement's own overflow here closes
     it; JS mirrors the data-lightbox-open attribute onto the html element
     in openLightbox/closeLightbox below. */
  ...
  @media (max-width: 1080px) {
    ...
    body[data-sidebar-open] .reader-sidebar { transform: translateX(0); }
    /* Scroll lock for the drawer, same two-rule shape the lightbox uses a
       few lines above (body[data-lightbox-open] / html[data-lightbox-open])
       and for the identical reason: 'html, body { overflow-x: clip }' keeps
       documentElement's own overflow-y at the visible default, so it stays
       the scrolling element regardless of what 'body' says — a body-only
       rule leaves the page scrollable behind the scrim. Was missing
       entirely before this task (a missing lock, not a defeated one). Scoped
       to THIS breakpoint on purpose — see the comment above
       '.sidebar-scrim { display: none; … }' near the top of this stylesheet
       for why base scope would leak an unscrollable desktop page past a
       resize, since nothing clears data-sidebar-open on one. JS mirrors
       data-sidebar-open onto documentElement in setSidebarOpen(), matching
  ...
  @media print {
    body { font-size: 11pt; }
    /* No scrollbars in a PDF, so an unscaled diagram there is not a
       scrollable one -- it is a clipped one, and half a waveform is worse
       than a small one. Hand it back to shrink-to-fit. Same (0,1,1) tie as
       the base rule, won the same way: source order.
       The !important is not decoration and was MEASURED: mermaid writes its
       own style="max-width: <natural>px" onto the svg element, and an inline
       declaration beats any rule in here regardless of specificity, so the
       plain version of this line clamped nothing and the first PDF built from
       it came out with a 16-stage flowchart cut off after Stage 3.
       applyDiagramScale's inline width is clamped by the same declaration --
       a max-width always beats a width -- so the runtime needs no print
       branch of its own. */
    .diagram-scroll { overflow-x: visible; }
    .diagram-scroll > svg { max-width: 100% !important; height: auto; }
    .page-layout {
      display: block;
      max-width: 100%;
      margin: 0;
  ...
  /* v3.0.2: a table's row grip is a document.body child at z-index 7 that
     straddles the table's left border, and a table block's ⠿ sits at the
     header row's own Y — so a pointer travelling left toward the ⠿ crosses
     the grip, elementFromPoint stops returning the block, and :hover above
     goes false mid-approach. The grip cannot move (it must be a body child
     to stay put while the table scrolls), so lib/editor/client.js holds the
     gutter lit with this class for the duration instead. It does NOT
     restore :hover, so .ed-block:hover's own outline still blinks there —
     a recorded, unfixed residue. */
  .ed-block.ed-keep-lit .ed-handle,
  .ed-block.ed-keep-lit .ed-insert { opacity: 1; }
  ...
  .ed-handle-menu-btn {
    ...
    /* v2.12.0: a flex ROW so the icon and the label share a baseline-free
       ...
       put it.
       v3.0.1: gap dropped from 8px to 4px — one space-width at this 12px
       font size — now that every row (both menu levels) carries an icon. */
    display: flex; align-items: center; gap: 4px;
  }
  /* Notion's own sizing. The stroke colour is NOT set here on purpose: the
     ...
     v3.0.1: the selector is ".ed-handle-menu-btn .ed-menu-icon", not the
     bare class — the gutter menu is appended INTO a ".ed-block"
     (lib/editor/client.js's blockEl.appendChild(gutterMenu)), which is a
     descendant of <main class="content">, and ".content svg" above
     carries "margin: 0 auto" at specificity (0,1,1) (one class, one
     type). The bare ".ed-menu-icon" is only (0,1,0) and LOSES that fight
     regardless of source order, so every icon picked up centering
     auto-margins instead of sitting flush left — measured on this branch
     as an icon offset that swung with label length (11px-40px) instead
     of a constant one. Scoping under ".ed-handle-menu-btn" raises this
     to (0,2,0), which wins on class count alone. (No backticks in this
     comment: it lives inside a JS template literal.) */
  .ed-handle-menu-btn .ed-menu-icon { flex: 0 0 auto; width: 16px; height: 16px; margin: 0; }
  ...
  /* Viewport-clamp backstop: openConvertSubmenu() (lib/editor/client.js)
     shifts the panel's top up so its bottom clears the viewport when there
     is room to do so — that shift is the clamp. This rule covers what the
     shift cannot: a viewport shorter than the panel, where the viewport's
     own top edge still leaves the panel's bottom edge off-screen. What
     happens under max-height, in the order it happens: CONVERT_TARGETS's
     buttons (lib/editor/convert-md.js), each carrying .ed-handle-menu-btn's
     own height (26px), first shrink via flex as the cap tightens (measured
     on this branch: 26px at a cap around 342px, stepping down to 18px
     around a 250px cap) — no scrolling yet, the list still fits end to
     end, just shorter per row. overflow-y: auto only starts moving content
     once the rows have already reached that floor and the cap is still
     tighter than the list at 18px per row. (No backticks in this comment:
     it lives inside a JS template literal.) */
  .ed-handle-submenu { left: 100%; margin-left: 4px; max-height: calc(100vh - 8px); overflow-y: auto; }
  ...
  /* Table cell-range selection: the painted rectangle, and the table that
     holds one (its caret and native selection are hidden while it stands). */
  .ed-cell-range { background: rgba(59, 130, 246, 0.18) !important; }
  .ed-cell-range-active { caret-color: transparent; }
  .ed-cell-range-active ::selection { background: transparent; }
  .ed-te-menu {
  ...
  /* S4 Task 2 (spec §4.5): the block drag's own singleton drop indicator,
     shown while a block is being dragged by its ⠿. Same 3px blue rule as the
     table's, on a SEPARATE node rather than sharing '.ed-te-drop-indicator':
     that one is shared by the row and column axes and each of them has to
     rewrite the OTHER axis's dimension on every move or a stale value leaks
     in — a third writer multiplies that coupling. It is still ONE node for the
     whole document (the Global Constraint is "never one per boundary"), and
     the separate class is what lets a test assert that a block drag raised
     the BLOCK line rather than table chrome that happened to be on screen. */
  .ed-block-drop-indicator {
    position: fixed; z-index: 8; height: 3px; border-radius: 2px;
    background: #3b82f6; pointer-events: none;
  }
  .ed-block-drop-indicator[hidden] { display: none; }
  /* S4 Task 2b (user request, 2026-09-01): the drag's feedback on the SOURCE
     side. lib/editor/client.js puts '.ed-block-dragging' on <html> the moment
     a ⠿ drag engages and takes it off in teardownBlockDrag(), which every one
     of the five abort paths funnels through. On <html> rather than on the
     dragged block because the plan forbids mutating anything inside
     '.content' during a drag — that invariant is the only reason Escape /
     pointercancel / window blur can all clean up with nothing to revert.
     The universal selector plus !important is what the rule needs to be worth
     having: '.ed-handle' (the button the gesture starts on), '.ed-handle-menu-btn'
     and every table grip carry their own 'cursor: pointer', and a plain
     inherited cursor loses to all of them. */
  html.ed-block-dragging, html.ed-block-dragging * { cursor: grabbing !important; }
  /* Notion-style grip handles: replace the original invisible TE_EDGE_PX=8
     ...
     the header is draggable too), STRADDLING the table's left border,
     centred on it (spec 4.2 hit-test conflict 2). v2.12.0/S4 T1 tried moving
     it entirely inside the table instead (left edge on the border) to stop it
     winning the hit test against the block's own ⠿ — MEASURED at 1400x900,
     the straddling grip occupied [tableLeft-10, tableLeft+10] while
     '.ed-handle' occupied [blockLeft-22, blockLeft-4] with tableLeft ==
     blockLeft, so the grip (position: fixed, z-index 7, on document.body)
     won the hit test for the RIGHT 6px of every ⠿ whose Y band it reached —
     but that broke the row/column symmetry the user expected ('.ed-te-grip-col'
     still straddles the table's TOP border, because nothing of the page's
     own chrome lives up there), and was reported as a regression. v3.0.1
     restores the straddle and instead moves the ⠿/＋ pair itself further left
     by --ed-gutter-shift, which removes the overlap at its source (see the
     edit-mode-only layout chunk near the end of this stylesheet); the row
     grip's straddling half sits well inside the first cell's default 14px
     padding, so no extra padding token is needed either.
     '.ed-te-grip-col' is a horizontal 6-dot
  ...
  /* spec §4.2 hit-test conflict 2: the row grip's width is a TOKEN because
     lib/editor/client.js's updateTableEdgeGrips() reads it (via
     gripRowWidthPx()) to CENTRE the grip on the table's left border —
     tableRect.left - gw / 2. Written once here, read once there, so the CSS
     width and the JS position can never drift apart the way they did when
     the position was a hard-coded literal 20. */
  :root { --ed-te-grip-row-w: 20px; }
  .ed-te-grip-row {
    width: var(--ed-te-grip-row-w); height: 28px; cursor: grab; z-index: 7;
  ...
  .ed-seltb-btn:hover:not([aria-disabled="true"]) { background: rgba(255, 255, 255, 0.18); }
  /* v3.3.0 (F2): the four states these buttons report about the selection.
     'true' (the selection is entirely inside a mark of this type) is the
     filled treatment the fixed toolbar already uses. 'mixed' (the selection
     overlaps marks of this type without lying inside one) is deliberately NOT
     the filled one — pressing it removes marks rather than adding any — so it
     gets a dashed edge and a half-strength ground instead. 'false' keeps the
     rule above. INERT is aria-disabled, never the disabled DOM property; see
     paintSelToolbarMarks() in lib/editor/client.js for why. */
  .ed-seltb-btn[aria-pressed="true"] {
    background: #e6edf3; color: #101215; border-color: #e6edf3;
  }
  .ed-seltb-btn[aria-pressed="mixed"] {
    background: rgba(230, 237, 243, 0.3);
    border-style: dashed; border-color: rgba(230, 237, 243, 0.8);
  }
  .ed-seltb-btn[aria-disabled="true"] { opacity: 0.38; cursor: default; }
  ...
  /* v3.1.0 §4: the always-visible editing toolbar. Its numeric language is
     .lightbox-bar's (8px/12px padding, 6px gap, 13px type) over .ed-seltb's
     dark pill, so the two floating surfaces read as one family.
     position:fixed, NOT sticky: lib/editor/client.js mounts this on
     document.body because rerenderAll() replaces .content's innerHTML
     wholesale and would silently destroy a toolbar mounted inside it.
     Deliberately does NOT read any --ed-gutter-* token — those describe the
     ⠿/＋ pair's geometry and this is a new, unrelated scale (the height is
     its own token because the edit-mode layout chunk at the end of this
     stylesheet has to reserve exactly that much room for it).
     z-index sits above the mobile drawer (99) and .sidebar-toggle (100) but
     below .ed-conflict (999): a save conflict must always be reachable. */
  :root { --ed-toolbar-h: 44px; }
  .ed-toolbar {
    position: fixed; top: 0; left: 0; right: 0; z-index: 101;
    box-sizing: border-box; height: var(--ed-toolbar-h);
    display: flex; align-items: center; justify-content: flex-start;
    flex-wrap: nowrap; overflow-x: auto; overflow-y: hidden;
    /* v3.4.0 §3: gap was 6px until the save button (id 'save', file group)
       landed at the FRONT of the row. Ruling T11-2 (below) requires the
       outline (☰) button's own centre point to hit-test successfully at
       800/1080px WITHOUT any scrolling — it is the only way to open the
       sidebar drawer in edit mode, and that test deliberately does not
       scroll first. A leading addition shifts every button after it right
       by its own width; MEASURED at 800px wide before this change: 22
       buttons put outline's centre at x=795 (row content 975px, overflow
       175px) — already only a 5px margin inside the 800px viewport. Adding
       save (23 buttons, row content 1026px, overflow 226px) pushed it to
       x=846, entirely off screen (elementFromPoint at that point returns
       null — off-screen, not occluded by another element; contrast the
       .ed-toolbar-status overlap a few lines down, which returns the
       covered BUTTON because that slot has pointer-events: none, an
       entirely different failure shape). This gap value was chosen
       empirically (a one-off puppeteer script, not a formula — flex gap
       plus intrinsic button/separator widths do not sum cleanly by hand):
       re-measured at 800px, outline's centre lands at x=765, a 35px margin
       — comfortably more headroom than the original 5px, not just barely
       restored. Also re-verified against the F12 "narrow viewport,
       scrolling allowed" sweep just below (420/640/820px): every button,
       save included, still reaches an empty unreachable list and an empty
       neverClear list at all three — this change does not trade the T11-2
       no-scroll guarantee for the F12 with-scroll one. */
    gap: 3px; padding: 8px 12px;
    /* F12, both halves measured on this build at 900px viewport height.
       justify-content was center. Centering an overflowing flex row splits
       the overflow across BOTH ends, and the leading end of a scroll
       container's content is not reachable by scrolling: driven at 820 /
       640 / 420 px wide, sweeping scrollLeft from 0 to its maximum in 8px
       steps and reading every button's rect, the buttons that never once sat
       inside the viewport were [undo] / [undo, redo, headings] /
       [undo, redo, headings, quote, code, list]. On this build — leading-edge
       layout plus the padding-right below, driven together, not one at a time
       — the same sweep leaves that set empty at all three widths.

       padding-right buys the buttons room to clear .ed-toolbar-status, whose
       fixed box occupies the viewport's right 124px (right: 12px plus
       min-width: 7rem at this font size). The claim it supports is narrow:
       SOME scroll position exists at which a given button is clear of the
       slot. It is NOT "a button can never sit under the slot" — driven at
       scrollLeft 0 after this change, [outline, preview] still overlap the
       slot at a 900px viewport and [check, bold, italic, strike] do at 420px.
       What makes that harmless is the slot's own pointer-events: none, in
       the rule further down — the load-bearing line, not this one. */
    padding-right: 124px;
    background: rgba(16, 18, 21, 0.92); color: #e6edf3;
    /* F12, second half: before this the bar scrolled but nothing on screen
       said so, so a button parked outside the viewport looked absent rather
       than off to one side. Two edge gradients, painted only on the side
       that still has content hidden behind it; the client runtime writes
       data-ed-tb-overflow from scrollLeft against the scroll maximum.

       Deliberately a background layer on the bar itself rather than a new
       overlay node. An overlay would have to be taken out of flow to avoid
       becoming a flex item in this row, and an out-of-flow child of this bar
       anchors to the scrolled content box and drifts with it (the
       .ed-toolbar-status note below records that measurement) — while
       anchoring it to the viewport instead would add a floating layer to the
       set test/editor-journey.test.js enumerates and demands a scroll answer
       for. A background layer needs neither: background-attachment defaults
       to scroll, which pins the gradient to this element's padding box while
       its contents move underneath.

       These gradients sit behind the buttons, because that is where the
       painting order puts a background layer: above the background colour,
       below the element's content. That is the reason they are dark rather
       than light — darkening the ground a translucent button sits on is what
       reads as content passing under an edge, where a light wash behind the
       same button would not. */
    --ed-tb-edge-l: rgba(0, 0, 0, 0);
    --ed-tb-edge-r: rgba(0, 0, 0, 0);
    background-image:
      linear-gradient(to right, var(--ed-tb-edge-l), rgba(0, 0, 0, 0)),
      linear-gradient(to left, var(--ed-tb-edge-r), rgba(0, 0, 0, 0));
  ...
  .ed-toolbar[data-ed-tb-overflow~="left"] { --ed-tb-edge-l: rgba(0, 0, 0, 0.55); }
  .ed-toolbar[data-ed-tb-overflow~="right"] { --ed-tb-edge-r: rgba(0, 0, 0, 0.55); }
  .ed-toolbar-sep {
  ...
  .ed-toolbar-btn:hover:not(:disabled) { background: rgba(255, 255, 255, 0.18); }
  /* F12: where the toolbar's keyboard cursor is. An outline rather than a
     background so it stays legible on a button that is already pressed-in
     (aria-pressed paints that one nearly white). Claim no more for it than a
     keyboard route: the buttons keep their tabindex="-1" and the caret stays
     in the document, so no focus move happens for assistive technology to
     follow, and what this cue does for a screen reader has not been measured
     here. What it does do is let the row be walked without a mouse. */
  .ed-toolbar-btn[data-ed-tb-cursor] {
    outline: 2px solid #6ea8fe; outline-offset: 1px;
  }
  .ed-toolbar-btn:disabled { opacity: 0.38; cursor: default; }
  .ed-toolbar-btn[aria-pressed="true"] {
    background: #e6edf3; color: #101215; border-color: #e6edf3;
  }
  /* v3.3.0 (F2): the middle state, matching .ed-seltb-btn[aria-pressed="mixed"]
     above so the two bars answer the same question the same way. */
  .ed-toolbar-btn[aria-pressed="mixed"] {
    background: rgba(230, 237, 243, 0.3);
    border-style: dashed; border-color: rgba(230, 237, 243, 0.8);
  }
  .ed-toolbar-b { font-weight: bold; }
  .ed-toolbar-i { font-style: italic; }
  .ed-toolbar-s { text-decoration: line-through; }
  /* H1 fix — and its reason has since moved, so both readings are kept here
     rather than one silently standing in for the other. The brief's literal
     margin-left: auto (a flex-flow item) was measured on v3.2.0 to break
     .ed-toolbar's justify-content: center — an auto margin on any flex item
     absorbs ALL positive free space before justify-content sees any, so the
     ~22-button group snapped flush left on every viewport wide enough not to
     overflow. F12 replaced that declaration with justify-content: flex-start
     (the .ed-toolbar rule above), so flush left IS the shipped layout now and
     that particular failure mode no longer exists. What the rule guards today
     is the slot's out-of-flow-ness, and the assertion beside it in
     test/editmode-render.test.js carries the re-driven numbers: against the
     slot as it ships the margin changes nothing at all, while putting the slot
     back in flow with it turns the slot into a scrolling flex item
     (scrollWidth 975 -> 1093 at 420px) and moves its rect off the strip
     .ed-toolbar's padding-right reserves for it ([1276, 1388] -> [1164, 1276]
     at 1400px). position: fixed removes
     the slot from the flex layout entirely (buttons measured byte-identical
     to a build with the slot absent, at both a 1400px and a 420px viewport),
     and — unlike position: absolute against .ed-toolbar's own
     position: fixed box — it anchors to the browser viewport, not to
     .ed-toolbar's scrolled content, so it does not drift as the button
     row's overflow-x: auto scrolls (measured identical rect at scrollLeft
     0 / mid / max, where the absolute alternative moved by the exact scroll
     delta). References no gutter-alignment custom property.
     H2 fix: pointer-events: none below is LOAD-BEARING, not decorative.
     v3.2.1 filled this slot with the current mode's name ('編輯' / '原始碼'),
     so it no longer ships empty — but that content is inert TEXT, never a
     control, which is exactly what keeps the rule below sufficient. The
     overlap it keeps harmless is real and still present after F12 moved this
     bar's scroll geometry: at scrollLeft 0, this box's footprint covers
     outline and preview at a 900px-wide viewport, and check / bold / italic /
     strike at 420px. Re-driven on this build, hit-testing the middle of each
     of those overlaps: with the rule as it ships,
     document.elementFromPoint() answers the BUTTON at every one of them; flip
     this span's pointer-events to auto and leave everything else alone, and
     every one of them answers this span instead. That reading is the same hit
     test a press resolves through, and the rule below is what stands between
     the two readings — not the span's emptiness. WHOEVER PUTS A CONTROL HERE (or
     wires up two-way sync, §3) MUST remove or scope this
     rule AND independently solve the overlap (e.g. reflow the slot in-flow
     at narrow widths, or shrink/hide it below some width) — simply
     deleting this line without doing that reopens the swallowed-click
     regression, this time on content someone can actually see and try to
     click. */
  .ed-toolbar-status { position: fixed; top: 0; right: 12px;
    height: var(--ed-toolbar-h); line-height: var(--ed-toolbar-h);
    min-width: 7rem; text-align: right; font-size: 12px; opacity: .8;
    pointer-events: none; }
  /* The H▾ dropdown: same singleton-panel idiom as .ed-insert-menu, but
     anchored to a fixed bar rather than moved into a block, so it is
     positioned in viewport coordinates by the client runtime.
     max-height/overflow-y: same backstop role as .ed-handle-submenu above
     (Task 9 fix round 1, Ruling T9-5) — toggleToolbarHeadingMenu()
     (lib/editor/client.js) does the shift-to-fit; this covers a viewport
     shorter than the panel, where the shift alone cannot bring the whole
     list on-screen. */
  .ed-toolbar-menu {
    position: fixed; z-index: 102;
    display: flex; flex-direction: column; gap: 2px;
    padding: 4px; border-radius: 8px;
    background: rgba(16, 18, 21, 0.96); color: #e6edf3;
    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.35);
    max-height: calc(100vh - 8px); overflow-y: auto;
  }
  .ed-toolbar-menu[hidden] { display: none; }
  .ed-toolbar-menu-btn {
    min-width: 120px; padding: 4px 10px; text-align: left;
  ...
  .ed-toolbar-menu-btn:hover { background: rgba(255, 255, 255, 0.18); }
  /* backlog #7 (Task 9 review, M1): the panel's OWN keyboard cursor
     (lib/editor/client.js's toolbarMenuIndex / data-ed-tb-menu-cursor) had
     no matching rule at all — the fix made the six items operable by
     keyboard but left the cursor invisible, the exact backlog #10 class of
     defect ("can act, but cannot see where"). Same outline idiom as
     .ed-toolbar-btn[data-ed-tb-cursor] above, for the same reason (legible
     over :hover's own background, and over nothing at all when neither
     applies). */
  .ed-toolbar-menu-btn[data-ed-tb-menu-cursor] {
    outline: 2px solid #6ea8fe; outline-offset: -2px;
    background: rgba(255, 255, 255, 0.18);
  }
  /* 追加 4: the whole-document source escape hatch. Replaces the rendered
     content column for the duration of source mode; the document's own
     lines[] is untouched until the mode is left (see docsource.js). */
  .ed-source {
    display: block; width: 100%; box-sizing: border-box;
    min-height: 70vh; padding: 12px;
  ...
  /* backlog #8: 'top: 0' used to put this banner directly OVER .ed-toolbar
     (same top-of-viewport band, and z-index 999 against the bar's 101 wins
     the paint) — MEASURED (task-10-toolbar-probe.js): all 23 toolbar
     buttons' own centre points hit-tested to the banner, not the button,
     for every .ed-conflict banner showBanner() raises (conflict /
     render-failed / save-failed / dropped-gesture / structural-refusal /
     swallow all share this one class — there is exactly one producer, see
     showBanner() in lib/editor/client.js). Floors at the SAME
     --ed-toolbar-h token placeTeMenuAboveCell()'s .ed-te-menu already
     clamps to (if top < topInset, top = r.bottom + …, both in
     lib/editor/client.js) — rather than inventing a new offset.
     Final-review I1 correction: an earlier draft of this comment also named
     positionSelToolbar()'s .ed-seltb here, as if it clamped to this same
     token. Retracted — MEASURED: positionSelToolbar() reads --ed-toolbar-h
     only to decide whether a selection scrolled fully out of view should
     hide the bar entirely (rect.bottom <= topInset); the vertical floor it
     actually clamps 'top' to is a bare margin = 4 local, independent
     of the toolbar's height. Swept 66 scroll positions: at 17 of them
     .ed-seltb's own top measures as low as 14.75px — inside this banner's
     band — with no --ed-toolbar-h involved (task10-seltb-check-probe.js).
     Not reproduced with a LIVE banner open at the same time (two attempts:
     a burst's bold toggle never hits /api/render, and Ctrl+S's
     switchAwayFrom() tears the bar down before a banner could show) — left
     as a geometry note in CHANGELOG.md's Known issues rather than a
     behaviour change.
     Deliberately NOT a z-index change: the toolbar comment above
     ("z-index sits ... below .ed-conflict (999): a save conflict must
     always be reachable") still holds — the banner stays the topmost
     thing, it just no longer shares the toolbar's own band to be on top
     OF. showBanner() additionally closes .ed-te-grip / .ed-tb-insert /
     .ed-te-menu / .ed-toolbar-menu — four dynamic fixed overlays that can
     each land in that band, though (per the correction above) only
     .ed-te-menu's own code floors there via --ed-toolbar-h; the others
     land nearby as a side effect of their own trigger's position, not a
     shared clamp — see showBanner()'s parallel comment in
     lib/editor/client.js. Closed regardless, so a banner can never land on
     top of one of those instead. */
  .ed-conflict {
    position: fixed; top: var(--ed-toolbar-h); left: 0; right: 0; padding: 10px;
...
(function () {
  ...
  // ── drawio sheet bar ─────────────────────────────────────────────────────
  // v3.4.0 §4: switching pages is a pure hidden-attribute toggle. Every page was
  // rendered to inert SVG at build time (lib/drawio.js's bake pass), so there
  // is no engine, no diagrams.net script, and no network request here — this
  // works exactly the same with the network unplugged. Only injected when
  // usesDrawioMultiPage: a single-page .drawio file must not pay for this UI
  // at all (matching CSS gate is in the <style> block above).
${usesDrawioMultiPage ? `  // v3.4.0 batch2 Task 6: wired through a NAMED, reusable hook instead of a
  // bare one-shot forEach, so the edit-mode client runtime can re-invoke it
  // — scoped to just one freshly swapped-in block — after a background
  // refresh re-bakes a \`.drawio\`/\`.xml\` source that changed on disk (see
  // lib/editor/client.js's stale-drawio handler). Still gated on
  // usesDrawioMultiPage exactly as before: a single-page document must never
  // pay for (or even mention) this UI at all — a guard test asserts the
  // whole feature's class name is absent byte-for-byte from that output, so
  // nothing below this line may ever run, or exist as source text, outside
  // this branch.
  window.__md2docInitDrawioSheetBar = function (rootEl) {
    var scope = rootEl || document;
    scope.querySelectorAll('.drawio[data-drawio-pages]').forEach(function (box) {
      if (box.querySelector('.drawio-sheetbar')) return;   // already wired
      var names;
      ...
      names.forEach(function (name, i) {
        ...
        b.addEventListener('click', function (event) {
          // The bar lives inside .drawio, which is also a lightbox click
          // target below (LIGHTBOX_TARGETS) — without stopPropagation a page
          // switch would also bubble up and pop the lightbox open.
  ...
  window.__md2docInitDrawioSheetBar(document);
` : ''}
  // ── Diagram / image lightbox ──────────────────────────────────────────────
  // Specs are read at 100% but their block diagrams and waveforms are drawn far
  // wider than the column, so the inline copy is shrunk to fit. Clicking one
  // pops it into a modal stage that zooms and scrolls. Past a point shrinking
  // stops being an acceptable inline copy at all -- that floor, and what the
  // inline copy does instead below it, is the block right after this one, and
  // the two share LIGHTBOX_TARGETS so they can never disagree about what
  // counts as a diagram.
  var LIGHTBOX_TARGETS = 'img, .mermaid, .graphviz, .drawio, [id^="WaveDrom_Display_"], .wavedrom-diagram';

  // ── Readability floor ─────────────────────────────────────────────────────
  //
  // The lightbox above is the escape hatch for a diagram drawn wider than the
  // column. This is the other half: the inline copy has to be worth reading in
  // the first place. ".content svg { max-width: 100% }" shrinks to fit and
  // never looks at how small it is making the text, and the damage grows with
  // the aspect ratio, so past some width the reader is handed a picture of a
  // waveform rather than a waveform.
  //
  // MEASURED on this branch, 1280x900 viewport, .content clientWidth 918, the
  // waveform skin's own labels at 11pt = 14.667px: a 60-cycle waveform is
  // 2480px natural, fits at scale 0.370, and its labels land at 5.43px, where
  // "clk" and "data" are smudges. The identical expression leaves an 8-node
  // flowchart alone (1287px natural, fits at 0.714, labels at 11.41px), which
  // is the whole point -- the trigger is the measured glyph size, not which
  // engine drew the diagram.
  //
  // This block ships in EVERY document, including diagram-free ones, and
  // test/md2doc.test.js asserts that such a document carries no diagram-engine
  // runtime. So engines may be named here in prose only, never by the global,
  // skin or probe identifiers the engines themselves define -- that guard
  // greps for those, and it cannot tell a comment from code.
  //
  // The floor was measured, not estimated. Eight widths were rendered and
  // screenshotted one per row at deviceScaleFactor 1 and read: 9.35px was the
  // last comfortable row, 8.86px was where lowercase labels start breaking up.
  // The full table lives in test/reader-panels.test.js section 5, beside the
  // assertions that bracket this number from both sides.
  var DIAGRAM_FONT_FLOOR = 9;

  // The width the diagram was drawn at, in the SAME units diagramMinFontPx
  // reports -- the svg's own user units. The pairing is the whole point: the
  // rendered size of a glyph is its computed font-size times (rendered width /
  // this), so the two must not be measured in different coordinate systems.
  //
  // That is why the viewBox comes first and the width attribute is only the
  // fallback for an svg that has none (where user units are CSS px anyway).
  // MEASURED, three engines, three different width attributes for the same
  // idea: the waveform engine writes width="2480" (equal to its viewBox, so
  // the order does not matter), mermaid writes width="100%" and keeps the real
  // number in the viewBox ("0 0 6838.25 70" for a 40-node flowchart), and the
  // graph engine writes width="4069pt" against a 4069-unit viewBox. Reading
  // the attribute first gives 100 for every mermaid diagram -- which reads as
  // "already fits", so the floor silently never applies to any of them -- and
  // mixes points into a ratio of pixels for the graph one.
  function diagramNaturalWidth(svg) {
    var viewBox = svg.getAttribute('viewBox');
    if (viewBox) {
      // The doubled backslashes are not a typo and node --check will not miss
      // them for you: this whole runtime is emitted from a JS template
      // literal, where "\\s" is an unrecognised escape that collapses to a
      // bare "s". This split really did ship as /[s,]+/ once, which parsed
      // "0 0 6838.25 70" as a single token, returned 0, and turned the
      // non-waveform half of this feature into a silent no-op. Precedent: the
      // normaliseText helper above spells the same thing /\\s+/g.
      var vw = parseFloat(viewBox.split(/[\\s,]+/)[2]);
      if (vw > 0) return vw;
    }
    var attr = svg.getAttribute('width');
    if (attr && /^\\s*[0-9.]+(px)?\\s*$/.test(attr)) {
      var px = parseFloat(attr);
      if (px > 0) return px;
    }
    return 0;
  }

  // The smallest glyph the diagram draws, in those same user units -- an svg's
  // computed font-size does not move when its viewBox is scaled, so the size
  // on screen is this times the fit scale.
  //
  // Also MEASURED rather than assumed: mermaid's default label is not an svg
  // <text> at all, it is an html <span> inside a <foreignObject> (79 of them
  // in the flowchart above, and zero <text> nodes), so a text-only query
  // returns Infinity for a mermaid diagram and hands it the same free pass.
  // Only a node that really carries a text node is counted; the font-size of
  // an empty wrapper is a number nobody can see.
  function diagramMinFontPx(svg) {
    var min = Infinity;
    var nodes = svg.querySelectorAll('text, tspan, foreignObject *');
    for (var i = 0; i < nodes.length; i++) {
      var node = nodes[i];
      var carriesText = false;
      for (var child = node.firstChild; child; child = child.nextSibling) {
        // .trim() rather than a /\\S/ test: one less escape to get wrong in a
        // template literal, and it says the same thing.
        if (child.nodeType === 3 && child.nodeValue.trim()) { carriesText = true; break; }
      }
      if (!carriesText) continue;
      var size = parseFloat(window.getComputedStyle(node).fontSize);
      if (isFinite(size) && size > 0 && size < min) min = size;
    }
    return min;
  }

  function applyDiagramScale() {
    var nodes = document.querySelectorAll(LIGHTBOX_TARGETS);
    for (var i = 0; i < nodes.length; i++) {
      var host = nodes[i];
      // ":scope > svg" is the same relationship ".diagram-scroll > svg" in the
      // stylesheet asks for -- one rule, written twice, deliberately
      // identically. An <img> target has no svg and no text to measure, and
      // neither do the empty duplicate display divs the waveform engine's
      // repeated render passes leave behind.
      var svg = host.querySelector ? host.querySelector(':scope > svg') : null;
      if (!svg) continue;
      var natural = diagramNaturalWidth(svg);
      if (!natural) continue;
      // Always measure the FITTED geometry, never geometry a previous pass
      // produced. Clearing the inline width is half of that and matters on its
      // own: a mermaid svg is width="100%", so lifting max-width would leave it
      // exactly as wide as the column it is supposed to be escaping.
      host.classList.remove('diagram-scroll');
      svg.style.width = '';
      var avail = host.clientWidth;
      if (!avail) continue;
      var scale = avail / natural;
      if (scale >= 1) continue;
      var minFont = diagramMinFontPx(svg);
      if (!isFinite(minFont)) continue;
      if (minFont * scale >= DIAGRAM_FONT_FLOOR) continue;
      host.classList.add('diagram-scroll');
      // max-width alone is enough for a diagram that states its width in px
      // and not enough for one that states it in percent, so both get the
      // number explicitly. @media print is unaffected: a max-width always
      // beats a width, which is what lets the print rule hand these back to
      // shrink-to-fit without this line having to know about it.
      svg.style.width = natural + 'px';
    }
  }

  // When to re-run. Two inputs move after this script executes: whether the
  // diagram exists yet, and how wide the column is.
  //
  // The waveform engine's own render pass is scheduled four times
  // (DOMContentLoaded, load, +250ms, +1000ms) and mermaid renders on its own
  // clock, so an svg can appear long after this line. The MutationObserver is
  // deliberately not a copy of those timings -- a copy goes stale the first
  // time either engine changes its schedule, and silently: a pass that runs
  // before the svg exists looks exactly like a pass that decided to do
  // nothing. It only wakes for an
  // insertion that actually brought an svg, so edit-mode typing does not pay
  // for it, and it cannot self-trigger because adding the class is an
  // attribute mutation, not a childList one.
  //
  // The column width moves on a window resize, on crossing the 1080px drawer
  // breakpoint and on a sidebar-splitter drag. A ResizeObserver on .content
  // sees all three; a window resize listener sees only the first, which is why
  // there is no hand-placed call at the end of the sidebar handlers. Toggling
  // the class changes the host's HEIGHT (a scrollbar appears) and that feeds
  // back into the same observer, so a pass only runs when the width moved.
  var diagramScaleFrame = 0;
  function scheduleDiagramScale() {
    if (diagramScaleFrame) return;
    diagramScaleFrame = window.requestAnimationFrame(function () {
      diagramScaleFrame = 0;
      applyDiagramScale();
    });
  }
  applyDiagramScale();
  window.addEventListener('DOMContentLoaded', scheduleDiagramScale);
  window.addEventListener('load', scheduleDiagramScale);
  ...
  if (contentEl && typeof ResizeObserver === 'function') {
    // KNOWN RISK, and honestly not measurable in this environment.
    //
    // On a platform with CLASSIC (space-taking) scrollbars rather than overlay
    // ones, adding the class makes the page taller -- the svg goes to natural
    // size and keeps height: auto -- and a page that gains a vertical
    // scrollbar loses about 15px of column width to it. That re-runs the
    // verdict at a narrower width, which can flip a diagram sitting close to
    // the floor back the other way, which makes the page shorter again. The
    // guard below is "the width changed", and the width is exactly what moves
    // in that loop, so the guard does not damp it.
    //
    // Not reproducible here: this environment only ever produces overlay
    // scrollbars (clientHeight === offsetHeight on a scrolling host, and
    // launching with the overlay-scrollbar features disabled changes nothing),
    // so neither the oscillation nor a fix for it can be observed -- and an
    // unmeasurable fix is how a worse bug gets shipped. Left alone on purpose.
    // The worst case is a busy loop rather than a hang, because
    // scheduleDiagramScale coalesces onto one animation frame. If it is ever
    // reproduced, the damping to reach for is hysteresis on the decision (two
    // thresholds instead of one), not a looser equality test on the width.
    var lastDiagramColW = -1;
    new ResizeObserver(function () {
      var w = contentEl.clientWidth;
      if (w === lastDiagramColW) return;
      lastDiagramColW = w;
      scheduleDiagramScale();
    }).observe(contentEl);
  } else {
    window.addEventListener('resize', scheduleDiagramScale);
  }
  ...
  function lightboxSourceOf(node) {
    ...
    // A multi-page .drawio bakes every page's <svg> into the DOM (hidden
    // ones included, so page switching needs no engine) — a plain
    // querySelector('svg') returns the FIRST one in document order, which is
    // wrong whenever page 1 isn't the one currently shown. Pick the visible
    // page's svg instead.
    if (node.classList && node.classList.contains('drawio')) {
      var visiblePage = node.querySelector('.drawio-page:not([hidden])');
      if (visiblePage) return visiblePage.querySelector('svg') || node;
    }
  ...
  const gvHtml = await bakeGraphviz(html);
  // editMode's bodyHtml is already baked per-part above; re-baking here is a
  // short-circuit no-op (bakeGraphviz bails out when there's no
  // data-graphviz-src left). Non-editMode bodyHtml is baked here as before.
  const gvBody = await bakeGraphviz(bodyHtml);
  // v3.4.0 §4: same short-circuit story for drawio. editMode already baked
  // every part above, so both calls below are no-ops there (no browser is
  // opened) — this is only load-bearing for the non-editMode
  // `bodyHtml = marked.parse(mdPre)` path, which never went through the
  // per-part loop. `html` and `bodyHtml` share the same placeholders (html
  // embeds bodyHtml), so both bakes share one lazily-opened browser instead
  // of paying for two.
  const drawioBrowser = makeLazyBrowserRef();
  let bakedHtml, bakedBody;
  try {
    bakedHtml = await bakeDrawio(gvHtml, drawioBrowser);
    bakedBody = await bakeDrawio(gvBody, drawioBrowser);
  } finally {
    await drawioBrowser.closeIfOpen();
  }
  return {
    html: bakedHtml, bodyHtml: bakedBody, parts, blocks,
    // v3.4.0 batch2 Task 6: an added field, not a shape change to the four
    // existing ones — every call site destructures the specific keys it
    // wants (checked across lib/editor/server.js and test/*.js), none reads
    // the object as a whole, so an addition here is backward compatible.
    drawioRefs: Array.from(drawioRefs, ([p, stamp]) => ({ path: p, stamp })),
  };
}
...
async function bakeGraphviz(htmlStr) {
  if (!GRAPHVIZ_PLACEHOLDER.probe.test(htmlStr)) return htmlStr;
  let Graphviz;
  try {
  ...
  return htmlStr.replace(GRAPHVIZ_PLACEHOLDER.re, (m, b64) => {
...
// v3.4.0 §4. A document with no drawio reference must not pay for opening
// Chromium. `get()` launches on its first call only (and every later caller
// awaits the same promise, so a document with several drawio placeholders
// still opens exactly one browser); `closeIfOpen()` closes it only if `get()`
// was ever actually called.
function makeLazyBrowserRef() {
...
// Render every deferred drawio placeholder to inline SVG using a headless
// Chromium running the vendored drawio viewer (lib/drawio.js). Every page is
// baked, not just the one selected by the markdown fragment (`#PageName`),
// so switching pages client-side is a pure `hidden`-attribute toggle with no
// engine involved — see lib/drawio.js's own comment on why a Node-side XML
// parser can't substitute for the real layout engine.
async function bakeDrawio(htmlStr, browserRef) {
  if (!DRAWIO_PLACEHOLDER.probe.test(htmlStr)) return htmlStr;

  // replace()'s callback can't be async, so collect the jobs first and apply
  // them (each with its own possible browser-page await) afterwards.
  const jobs = [];
  htmlStr.replace(DRAWIO_PLACEHOLDER.re, (m, b64, pageIdx, altB64) => {
    jobs.push({
      m,
      b64,
      pageIdx: Number(pageIdx),
      alt: Buffer.from(altB64, 'base64').toString('utf8'),
    });
    return m;
  });
  ...
  for (const job of jobs) {
    ...
    let svgs;
    try {
      // One document shares one browser: browserRef.get() only really opens
      // Chromium on its first call.
      svgs = await drawio.bakeDrawioSvg(xml, { browser: await browserRef.get() });
    } catch (e) {
      // page.evaluate() can reject with a plain string rather than an Error
      // (observed: puppeteer surfacing a serialization failure as "buffer
      // error") — e.message would silently read as "undefined" then.
      const reason = (e && e.message) ? e.message : String(e);
      console.error('[WARN] drawio render failed:', reason);
      // A silent blank is the most expensive failure this project has learned
      // from (v3.3.0) — leave a visible placeholder naming the error instead.
      const msg = ('drawio 圖無法渲染：' + reason)
        .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      // The callback form, NOT a replacement string: `msg` is generated
      // content (a renderer error message), and String.replace() interprets
      // $$ / $& / $` / $' inside a replacement STRING even when the pattern
      // is a plain string. A callback's return value is never re-scanned.
      // See the same note on the success path below.
    ...
    // Page names may contain any character (commas and quotes included), so
    // they travel as base64-encoded JSON rather than a delimited string — a
    // delimiter breaks on both "two names glued together" and "a name that
    // happens to contain the delimiter", and a literal control character
    // living in the document is invisible to boot.
    const attr = Buffer.from(JSON.stringify(names), 'utf8').toString('base64');
    // v3.4.0 batch2 final review R5 — the author already wrote a name in
    // `![Architecture overview](arch.drawio)`; give the baked box that name.
    // The role differs by page count on purpose: a multi-page box has the
    // sheet bar's <button>s appended INSIDE it at runtime, and role="img"
    // would take them off the accessibility tree entirely, so those boxes get
    // role="group" — also nameable, children still exposed. No alt text means
    // no attributes at all: an empty accessible name is worse than none.
    const named = job.alt
      ? ` role="${names.length >= 2 ? 'group' : 'img'}" aria-label="${job.alt}"`
      : '';
    // v3.4.0 batch2 final review B2 — the callback form is load-bearing.
    // `inner` is the baked SVG, i.e. arbitrary diagram text, and a
    // replacement STRING has $$ / $& / $` / $' interpreted in it even when
    // the pattern is a plain string: a `$$` label silently collapsed to a
    // single `$`, and a `$&` label (the byte sequence any label with `$`
    // before `&` produces once the SVG escapes the `&`) spliced this very
    // placeholder — the whole .drawio file's base64 — into the middle of a
    // <text> element. bakeGraphviz() above was always a callback; this is the
    // same rule. Keep it a callback.
    htmlStr = htmlStr.replace(job.m,
      () => `<div class="drawio" data-drawio-pages="${attr}"${named}>${inner}</div>`);
  }
  return htmlStr;
}
