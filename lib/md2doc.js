...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  /* v3.5.0 Task 11 fix round 4 audit: FOUND, not fixed here — see this
     task's own report for why. This text (drawn by renderEdges, AFTER
     .ed-wave-edge-hit) is the same shape of hazard as .ed-wave-selection
     and .ed-wave-gap above: purely decorative, sits on top of the fat
     invisible click target a press aimed at SELECTING the edge is meant to
     land on. The reason it is NOT touched in this round: ed-wave-edge-label
     is not this text's own class alone — wave-ui.js's sidebar
     edgeLabelInput (the real, keyboard-and-mouse text field for editing an
     edge's label) uses the IDENTICAL class name. A blanket pointer-events:
     none here would also land on that input field and block clicking into
     it with the mouse. Fixing this properly needs the two split onto
     different class names first — out of scope for a round whose own item
     is the bus-label selection rect. */
