...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  /* The 大綱 button's DESKTOP half. body[data-sidebar-open] is toggled by
     the reader runtime's existing setSidebarOpen() (md2doc.js's own
     #sidebar-toggle handler) and, before this rule, was read by NOTHING
     outside the max-width:1080px drawer block — which is why the toolbar's
     outline button was a no-op on a desktop viewport. No new JS: this rule
     gives the attribute a desktop meaning.
     ⚠ The attribute's sense is INVERTED between the two viewports, and that
     is deliberate rather than an oversight. On mobile the sidebar is a
     drawer that is CLOSED by default, so "open" = show it. On desktop it is
     a sticky column that is SHOWN by default, so the only thing a pure
     attribute TOGGLE can mean there is "hide it" — a JS-free desktop rule
     and a non-inverted attribute cannot both be had, and the plan chose
     no-new-JS. The visible behaviour is the one the user asks for (press
     大綱, the outline goes away; press it again, it comes back);
     #sidebar-toggle's aria-expanded, which is written by that same shared
     handler, reads inverted on desktop as the cost. */
  @media (min-width: 1081px) {
    body[data-sidebar-open] .reader-sidebar,
    body[data-sidebar-open] .sidebar-splitter { display: none; }
  }`;
