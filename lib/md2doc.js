...
<style>
  ...
  /* Scroll lock for the mobile drawer, same two-rule shape the lightbox uses
     just below (body[data-lightbox-open] / html[data-lightbox-open]) and for
     the identical reason: 'html, body { overflow-x: clip }' keeps
     documentElement's own overflow-y at the visible default, so it stays the
     scrolling element regardless of what 'body' says — a body-only rule
     leaves the page scrollable behind the scrim. Was missing entirely here
     (this is a missing lock, not a defeated one): test/editor-journey.test.js
     used to measure and PIN the resulting scroll — 'V3 .sidebar-scrim /
     .reader-sidebar stay live across a scroll' scrolled the page to
     scrollY 1938 with the drawer open and asserted only that the overlays
     stayed put on screen, not that the page held still. That row is migrated
     alongside this fix. JS mirrors data-sidebar-open onto documentElement in
     setSidebarOpen() below, matching openLightbox/closeLightbox. */
