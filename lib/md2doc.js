...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  .ed-wave-panel {
    background: #fff; border-radius: 6px; width: min(1200px, 94vw);
  ...
  .ed-wave-metafield input { width: 110px; font-size: 12px; padding: 2px 4px; }
  /* v3.5.0 Task 6 fix round 2: with no rule at all this is a flex item inside
     .ed-wave-toolbar (display:flex; flex-wrap:wrap) that sizes to content, so
     its three controls squeeze against whichever control precedes them
     instead of forming their own row. The two property groups below are not
     new values invented for this rule: display/flex-wrap/gap/align-items are
     copied from .ed-wave-brushes, .ed-wave-meta above (the
     shared flex-row rule its own children need), and flex:1 1 100% is copied
     from .ed-wave-unmodelled below (what makes THAT sibling claim its own row
     instead of sitting beside the control before it) — this row needs both at
     once: it lays out its own children AND has to stop sharing a line with
     whatever toolbar control precedes it. */
  .ed-wave-edgebar { display: flex; flex-wrap: wrap; gap: 4px; align-items: center; flex: 1 1 100%; }
  ...
  /* Widths chosen so the DRAWING still gets room at the narrowest viewport this
     repo's browser tests use (800x600 -> panel 752): 752 - 200 - 160 = 392px,
     which is 8 cycles at .ed-wave-canvas's 48px pitch. The canvas is a scroll
     container, so a wider diagram scrolls rather than pushing the preview off —
     but a canvas column narrower than a couple of cycles would make the drag
     gesture unusable, and that was the first thing measured to go wrong here
     (a press landed on the preview column, 116px past the canvas's clip). */
  .ed-wave-lanes {
  ...
  .ed-wave-preview {
    flex: 0 1 340px; min-width: 160px; border-left: 1px solid #e2e2e2;
