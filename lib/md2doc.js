...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  //
  // Depths beyond MAX are not given a counter of their own; they fall back to
  // the depth-agnostic rule below and may show a wrong number. Real markdown
  // does not nest that far, and the failure is cosmetic and self-correcting on
  // the next re-render.
  ...
  .ed-block[data-list-type="ul"][data-task="1"] > .ed-li-marker { min-width: 0; }
  ...
  /* D6: ＋ immediately left of ⠿, on the same row, no gap — replacing the
     ...
     [contentLeft-54, contentLeft-18], whose outer 6px hangs into
     .page-layout's own 24px padding, so ＋ never reaches a negative viewport
     x. The 20px table row grip still straddles contentLeft at
