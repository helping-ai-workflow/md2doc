...
<div class="sidebar-splitter" id="sidebar-splitter" role="separator" aria-orientation="vertical" aria-label="Resize sidebar"></div>`;
  ...
  // WaveDrom has no such race (ProcessAll() is a synchronous scan-and-replace,
  // and the non-edit path already calls it up to 4 times as a matter of
  // course — see the WaveDrom block below), so it is left in place and this
  // hook's call is simply an additional, harmless invocation.
  ...
  window.__md2docInitDiagrams = function (rootEl) {
    ...
    if (typeof WaveDrom !== 'undefined') {
      ...
      // whole document. Known limitation of this hook's "scoped to rootEl"
      // contract for WaveDrom specifically; harmless (idempotent) either way.
      WaveDrom.ProcessAll();
    }
  };
