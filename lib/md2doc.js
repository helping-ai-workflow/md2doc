...
<style>
  ...
  /* H1 fix: the brief's literal margin-left: auto (a flex-flow item) was
     ...
     control, which is exactly what keeps the rule below sufficient. At narrow viewports .ed-toolbar's own
     overflow-x: auto lets the button row scroll to a rest position where
     this fixed 7rem box's footprint physically covers three real buttons
     (image / outline / preview, measured at 420px width, scrollLeft at its
     max 221) — document.elementFromPoint() over any of them resolves to
     this span, not the button underneath, and a real click on it never
     reaches the button's listener. Harmless today only because the span
     carries nothing to interact with. WHOEVER PUTS A CONTROL HERE (or
