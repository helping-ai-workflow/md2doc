...
// v3.4.0 batch2 Task 6 fix round 1 (review F7) — the identity of a referenced
// `.drawio`/`.xml` file's bytes AS THIS PROCESS LAST SAW THEM.
//
// (mtimeMs, size) rather than mtimeMs alone: this project's own environment
// notes document a filesystem (WSL DrvFs over /mnt/c) that degrades and
// behaves oddly around timestamps, and coarse-granularity mtimes are also the
// norm on SMB shares — a rewrite landing inside the same tick as the recorded
// one compares equal and is never reported. Size costs nothing extra (the
// Stats object is already in hand) and closes that hole for every edit that
// changes the file's length.
//
// Returns null when the file cannot be stat'd at all. That deliberately
// collapses "deleted" and "unreadable" (EACCES, an editor's atomic-replace
// window) into one value: for staleness they are the same answer — "the bytes
// you baked are no longer the bytes on disk" — and the caller has no repair
// action that would differ between them. The cost is diagnostic only.
function drawioStampOf(absPath) {
  try {
...
// 不是新依賴。
//
// 遠端路徑預設值必須在 viewer 載入之前就改寫掉。viewer 自己的寫法是
// `window.PROXY_URL = window.PROXY_URL || "https://viewer.diagrams.net/proxy"`
// 這種形式 —— `||` 代表先設好的值會勝出，所以 addScriptTag 之前先 evaluate。
//
// 數字要講清楚，因為下面那份清單看起來很像「全部」：viewer 裡這種形狀的
// 遠端預設值共有 14 個（11 個指向 diagrams.net、3 個指向 github/gitlab），
// 下面只中和了靜態渲染路徑真的會讀到的 8 個，外加 2 個載入旗標。沒中和的
// 6 個（EXPORT_URL、VSS_CONVERT_URL、DRAWIO_LIGHTBOX_URL、DRAWIO_GITHUB_URL、
// DRAWIO_GITHUB_API_URL、DRAWIO_GITLAB_URL）屬於匯出／格式轉換／git 整合功能，
// GraphViewer.createViewerForElement() 這條路不會叫到它們。
//
// 所以「輸出離線可用」這個保證**不是**靠這份清單完整，而是靠 test/drawio.test.js
// 對產出斷言沒有任何遠端 URL。要加功能而動到 viewer 的其他能力時，先重新量，
// 不要假設這 8 個就是全部。
...
module.exports = { isDrawioXml, pageNamesOf, resolvePageIndex, bakeDrawioSvg, drawioStampOf };
