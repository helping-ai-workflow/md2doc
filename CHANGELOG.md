...
這一版處理編輯器裡四個「要一直伸手拿滑鼠」的地方：清單項目的 MD 原始碼窄到不能用、
block 之間不能用方向鍵移動、表格選不到一塊格子、清單跟其他區塊不能一起刪。

### Added — 方向鍵走得出 block

- **↑ 在第一行、↓ 在最後一行，游標會跨進相鄰的 block**，落在最接近原本水平位置的
  字元上。不在邊界時照舊是瀏覽器自己的移動；有反白、按著修飾鍵、IME 組字中都不攔。
- **程式碼、引用、分隔線、圖表這類不能直接打字的 block 不會被跳過**：走到它身上會
  變成整塊選取，Enter 開 MD 原始碼（wavedrom 照舊開波形編輯器）、Delete 刪除、再按
  ↑/↓ 離開。
- **表格裡四個方向鍵都能換格**，但一定要先走到那一格文字的邊界；表頭按 ↑、最後一列
  按 ↓ 會離開表格。
- 目的地在畫面外、被上方工具列蓋住、或比整個畫面還高時，會先把**要落點的那一行**
  捲進來再放游標。

### Added — 表格跨格選取

- **選取一塊格子**：從一格拖進另一格、Shift+點擊，或 Shift+方向鍵在文字邊界時往外
  擴。一般點擊、一般方向鍵、Esc、Tab、Enter 取消。
- **對那一塊操作**，每個操作都是一步 undo：Ctrl+C（TSV 加 HTML 表格，貼進 Excel
  直接對齊）、Ctrl+X、Delete 清空、直接打字（清空後字落在最後那一格，跟 Excel 一樣）、
  工具列 B / I / S / U / code 與 Ctrl+B / I / U（每格整格套用；全部都已經有這個格式
  時就取消）。
- **貼上一塊格子**：剪貼簿有 TAB 分隔的內容或 HTML `<table>` 時，從範圍（或游標那格）
  的左上角開始填，表格不夠大就自動長出列和欄。兩者都有時用 TSV（Excel、Sheets 的
  HTML 是排版過的）。沒有 TAB 的多行文字照舊塞進同一格。超過 500 列或 50 欄拒絕並
  顯示提示。

### Changed

- **清單加上其他區塊的多選可以一起刪了**，條件是選取碰到的每個清單都整個被選到
  （含子項）。只選到清單的一部分時照樣拒絕，提示改成
  「刪除時清單只選到一部分：請把整個清單選進來，或只選清單項目」。搬移、轉換、
  建立副本、Tab 的規則不變。
- 同一張表裡的 Shift+點擊與跨格拖曳，以前是格子內的文字選取，現在是格子範圍。
- 有 block 選取時按一般的 ↑/↓，會取消選取並從選取的焦點 block 繼續走。

### Fixed

- **清單項目的 MD 原始碼 textarea 不再只有 189px 寬**（任何視窗寬度都是），現在跟
  其他 block 一樣撐滿文件欄，巢狀清單也一樣。

### Known limitations

- 固定工具列的 B / I / S / code 對格子範圍沒作用，按下去還會取消範圍；請用浮動的
  選取工具列或 Ctrl+B / I / U。
- 格子範圍套粗體／斜體時不認瀏覽器原生產生的 `<b>` / `<i>` / `<s>`；在同一次編輯裡
  先按原生 Ctrl+B 再對範圍按 B，會疊成兩層。
- 對格子範圍套 code 時，格子裡的換行與粗體在存檔時會消失（既有的 code 序列化行為）。
- Tab 在 block 之間移動時，如果離開的那格表格剛插入過列，目的地仍可能找不到；方向鍵
  已處理這個情況，Tab 還沒有。
