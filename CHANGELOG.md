...
- **工具列的存檔狀態槽位。** 工具列右側先前刻意留空的那個位置，現在有了一個佔位
  的存檔狀態指示器——本身還不接雙向同步（見下方 Known issues），純粹是把介面的
  位置先佔住，等真正的同步邏輯到位時不必再挪一次版面。

### Fixed

- **反應慢。** v3.1.0 的 Known issues 記過這件事：「編輯器仍然慢，這一版沒有改」。
  上面的增量 render 就是那個修法——一次提交的重繪成本現在只跟「變動了多少」成正比，
  不再跟「文件有多長」成正比。
- **沒有選取內容時，行內格式鈕維持可點但沒有作用對象。** 工具列 inline 組的幾顆按鈕
  （粗體／斜體／刪除線／底線／連結）現在會在游標沒有任何選取範圍時變灰，按下去才不會
  是一個看起來能點、實際上什麼都不會發生的按鈕。
