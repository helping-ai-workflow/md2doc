...
- **`CLAUDE.md` 的 `lib/md2doc.js` 區段行號表全部重新量過。** 六列裡有五列是錯的，
  而且錯了上千行。每一列現在都附一個可以 `grep -n` 的錨點字串，下一個人請從錨點
  重新推導，不要相信數字。

### Known limitations

- **PDF 裡的寬圖仍然是縮小的。** 列印路徑的選擇是「縮到欄寬」而不是「裁掉」——裁掉
  更糟——但一張 60 cycle 的波形印進 PDF 之後還是讀不了。**這正是這一版起因的那份回報
  的原始抱怨，對 PDF 而言仍然成立。** 網頁上的閱讀路徑已經修好；PDF 需要的是另一種
  作法（換頁切片、或橫式頁面），不是把同一條規則再調一次參數。
