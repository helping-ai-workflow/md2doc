...
兩個使用者看得到的結果：**編輯器現在用得到它面前的整個螢幕**，而且**閱讀頁上的寬圖
不再被縮成一張示意圖**。第二件事有一個仍然成立的例外，寫在最下面的「Known
limitations」——**PDF 裡的圖還是縮的**，那正是原始抱怨的一半。

### Added — 畫布畫得出更多東西了

- **lane 名稱欄、cycle 尺規、`head` / `foot` 兩條帶**都畫在畫布上了，不用再靠右邊的
  預覽才知道自己在編第幾格。群組逐層縮排。
- **`period`、`phase` 與 `config.hscale` 進了幾何模型**——畫布跟引擎現在畫的是同一個
  幾何，不再是「左邊一個樣、右邊一個樣」。逐值對過釘住的 wavedrom 3.5.0：`period: 4`
  要畫 **17** 條刻度（引擎量到 17，不是 `cycles + 1` 的 5），`hscale: 2` 的刻度與格線
  共用同一個乘過 hscale 的間距（48 × 2 = 96）。這一條也順帶讓 v3.5.0 那個「這三個
  屬性畫不出來」的告示從畫面上消失——它現在是假的了。
- **電位轉態畫出斜坡（slew）。** 斜坡的起訖比例是從引擎自己的 `waveSkin` 量的
  （`0m1` 的 `3/40`→`9/40`，用 `1m0` 交叉核對過）。**適用範圍是真的電位轉態，不是
  全部 22 個筆刷**：`wave-draw.js` 的 `isRampPair` 是一份 allow-list，只認 `000` 與
  `111` 兩種 brick，所以 22 個筆刷裡只有 `0`、`1`、`h`、`l` 這 **4** 個畫得出斜坡，
  **另外 18 個永遠不會**（`x`、`z`、`u`、`d`、`=`、`2`–`9`、`p`、`n`、`P`、`N`、`|`）。
  那是刻意的：bus（六角形）與 `x`（方框）本來就自帶斜邊或框線，clock brick 自帶尖銳
  的垂直邊而且根本走不到那個判斷。這些筆刷之間的轉態畫的仍然是既有的垂直邊。
- **工具列分成八個帶標題的分區**；每個電位按鈕帶一個圖示，而且那個圖示是由**畫布
  自己那批繪製函式**產生的，不是第二套實作——按鈕上長什麼樣，塗下去就是什麼樣。

### Added — 對話框用得到整個螢幕

- **面板寬度上限從 `min(1200px, 94vw)` 放寬成 `min(1920px, 94vw)`。** 舊的 1200px 在
  任何 ≥1277px 寬的視窗上都是**實際的**限制，於是 1280／1600／1920 三種螢幕拿到的畫布
  一樣寬——換句話說，買一台更大的螢幕不會讓這個編輯器多給你任何東西。
- **右欄五個面板**：關聯線、文件、Lane、WaveDrom 預覽、原始碼——這五個是對話框裡
  真正印出來的標題（`wave-panels.js` 的 section 表），在畫面上找「這條 lane」或單獨的
  「預覽」是找不到的。WaveDrom 預覽與原始碼預設摺疊。
  **原始碼面板顯示的是「保留並關閉」真的會寫回檔案的那串 bytes 本身**，不是為了顯示
  另外序列化一份——所以它永遠不可能跟實際會存下去的東西不一致。
- **側欄在窄視窗（<1100px）收成一條 28px 的軌**，需要時展開。
- **匯出 SVG / PNG，以及把 WaveJSON 複製到剪貼簿。**
- **`Alt+↑` / `Alt+↓` 用鍵盤搬動選取的 lane**；**連線武裝時 `Alt+←` / `Alt+→` 在該條
  訊號的轉態點之間跳**。**單純的 `←` / `→` 在任何狀態下都仍然是逐格移動**——武裝與否
  不改變方向鍵的意思。沒武裝就按 `Alt+←/→` 會出聲拒絕，不是靜靜地什麼都不做。

### Fixed

- **關聯線的端點原本畫在格子中心，比引擎的位置右移了 `(0.5 − 0.15)` 個 cycle**（編輯器
  畫布的 48px 格寬下是 16.8px），所以
  一條用來標注上升沿的線並不貼著上升沿。端點改成落在**錨點**——引擎放註解的那一點，
  也就是轉態斜坡的**中點**（50% 交越），比例 0.15 是從 `waveSkin` 的 socket 量出來的。
  （這個數字中途修正過一次：曾經以為錨點就是斜坡的**結束**點，那是拿 `0m0`
  這種「上去又下來」的 brick 量出來的巧合，改用真正的轉態 brick `0m1` / `1m0` 重量
  之後，斜坡結束是 0.225，錨點是 0.15。）
- **連線手勢的命中從 `floor` 改成最近邊界**，往左偏一個像素不再跳到前一格。
- **lane rail 每列原本塞七個控制項**，200px 放不下，中文按鈕被瀏覽器壓成直排，名字欄
  只剩 45px。操作全部移到工具列，rail 每列只留拖曳把手、名字與標籤，名字欄至少 120px。
- **鍵盤游標會掉到 `<body>` 上。** 一個動作做完之後要把焦點交還給某顆按鈕，而那段程式
  把「找到了但 `disabled`」當成「找到了」——`.focus()` 對 disabled 元素是空操作，於是
  焦點無聲地離開整個對話框，下一個按鍵不知道去了哪裡。
- **跟選取有關的按鈕狀態只有 `render()` 會重畫**，但方向鍵與拖曳走的是
  `repaintDrawing()`。結果是：用方向鍵改變選取之後，「建立群組」這類按鈕的啟用狀態
  停在上一拍。
- **匯出的 SVG 離開這一頁會變成半張圖。** 預覽自己帶 **0 個 `<defs>`** 與 **51 個
  `<use>`**，全部指向頁面上**另一個** SVG 裡的 symbol；直接把預覽存下來，那 51 個參照
  在檔案裡一個都解析不到。匯出時會把頁面 skin 的 `<style>` 與 `<defs>` 一起複製進去。
- **匯出的檔名不再帶「未存檔」記號**——那是編輯器的狀態，不是這張圖的名字。
- **閱讀頁的寬圖原本一律縮放貼合。** 實測（1280×900 視窗、內文欄 918px）：一張 60 cycle
  的波形原生 2480px，貼合後縮到 **0.370**，它自己的標籤字剩 **5.43px**——「clk」跟
  「data」在那個尺寸下是一團墨。現在量的是**縮放後最小字級**，低於實測的 **9px** 地板
  才改成原生尺寸加水平捲動。判準不是「這是不是波形圖」：同一條式子讓一張 8 節點的
  流程圖（貼合 0.714、標籤 11.41px）原封不動貼合，因為它本來就讀得懂。
- **PDF／列印的圖會被裁掉。** 上一條的列印規則第一版壓不過 mermaid **自己寫在元素上的
  `style="max-width: …"`**（inline 宣告贏過樣式表裡任何規則，跟權重無關），第一份用它
  產出的 PDF 有一張 16 段的流程圖在 Stage 3 就被切斷。改用 `!important` 之後，寬圖在
  PDF 裡是**完整但縮小**的——完整優先，理由見下方 Known limitations。

### Changed

- **`lib/editor/wave-ui.js` 拆成三支**：`wave-ui.js`（外殼／狀態機／事件）、
  `wave-draw.js`（畫布繪製）與 `wave-panels.js`（工具列／面板／rail），另外幾何、
  編解碼與 undo 各自獨立成 `wave-geometry.js` / `wave-codec.js` / `wave-store.js`。
  拆檔與改行為刻意分開進行，拆的那幾步不帶任何行為改動。
  **但「拆成三支」講的是機制，不是結果**：`wave-ui.js` 在 v3.5.0 是 **4053** 行，兩個
  抽取 commit 之後是 **2711** 行，而這一版做完之後是 **4018** 行——淨 **−35 行
  （−0.9%）**。後面那些 task 把一千多行新的外殼／狀態機／事件程式碼放回了同一個檔。
  這沒有錯（抽取是真的，新碼也本來就該落在那一層），但一個照著這句話去讀的人，打開的
  會是 `lib/editor/` 裡第二大的模組（僅次於 `client.js`），並且會在裡面找到
  `moveSelectedLane`、`carryMarks`、`commit`、`clearSelectedEdgeOfSide`、
  `handleDrawingKey`、整個訊號／群組工具列，以及 `renderCanvas` 的呼叫端。
- **選取合併成一套**：`laneMultiSelect` 從原始碼中完全移除，rail 與畫布共用 `selection`。
  工具列的「訊號」「群組」分區作用在同一個 `selection` 上。

### Known limitations

- **PDF 裡的寬圖仍然是縮小的，而且小得很嚴重。** 列印路徑的選擇是「縮到欄寬」而不是
  「裁掉」——裁掉更糟，整段內容會不見——但縮完之後也讀不了。實測（A4 寬度的
  794×1123 列印模擬，內文欄 774px）：一張 40 段的流程圖原生 6838px，40 個標籤
  **一個不少、全部落在圖框內**，但最後一個標籤「Stage 39」只佔 772–780 這 8 個
  像素，有效字級 **1.81px**。一張 60 cycle 的波形同理。**這正是這一版起因的那份
  回報的原始抱怨，對 PDF 而言仍然成立。** 網頁上的閱讀路徑已經修好；PDF 需要的是
  另一種作法（換頁切片、或橫式頁面），不是把同一條規則再調一次參數。
- **drawio 的圖不在可讀性地板的涵蓋範圍內。** 它的 svg 位置是
  `.drawio > .drawio-page > svg`，不是直接子元素，CSS 與 JS 用的是同一條「直接子元素」
  規則，所以兩邊一致地跳過它。要涵蓋它需要一併處理多分頁與隱藏分頁，這一版沒做。
- **`Alt+←` / `Alt+→` 與瀏覽器的「上一頁／下一頁」會不會打架，在這個環境量不出來。**
  本專案的 headless Chrome 對 CDP 合成的 `Alt+ArrowLeft` **根本不觸發返回**——在一個
  完全沒有按鍵處理程式的空白頁上也一樣——所以測試環境沒辦法在任何方向上回答這個問題。
  需要在真正的桌面 Chrome 上確認。寫「它可以」會是一個沒量過的宣稱。
- **在 lane 名稱欄裡打字時，`Alt+←/→` 吃掉了 macOS 的 Option+←/→ 逐字移動。** 那是
  上一條那個保險做法的代價：快捷鍵在 capture 階段就攔下來。名字欄是一個放訊號名稱的
  單行輸入框，損失不大，但它是真的損失。
- **一個寫不回去的改動，狀態列上可能讀起來像成功。** 對話框的狀態列只有**一條**
  `role="status"` 字串。`afterStoreMoved` 會先說
  「這個改動沒辦法只改幾個位元組寫回去：<原因>」，但呼叫端緊接著會用自己那句愉快的
  話蓋過去——`moveSelectedLane` 的「搬動的 lane 落在…」（這一版新增）、`新增 lane` 的、
  複製的，以及刪除的「已刪掉 n 條 lane —— Ctrl+Z 可以拿回來」。**於是你讀到的那一行
  說它落好了，而它其實沒有寫回去。** 緩解是真的存在：`client.js` 的
  `waveRefuseGesture` 仍然會升起一條需要你確認的橫幅，指名它寫不進去的行，所以改動
  不會靜靜地消失。把這幾句 `say` 重新排序是行為改動，刻意留到下一版。
- **`Alt+↑` / `Alt+↓` 會丟掉打到一半的 lane 名稱。** 這個鍵沒有子面板閘門：在 rail 的
  改名欄或 bus 的 data 標籤欄開著的時候按下去，lane 會照搬，重畫順手把那個欄位收掉，
  裡面還沒提交的字就沒了（跟在同一個欄位裡按 `Ctrl+Z` 一樣）。它的兄弟代價
  （macOS 的 Option+←/→ 逐字移動）已經列在上面，而這一條破壞性更大。
- **lane 名稱欄沒有 `aria-label` 也沒有 `title`。** `.ed-wave-lane-name` 只帶 class 與
  `data-focus-key`，螢幕閱讀器讀到的是一個沒有名字的文字框——而那正是 rail 重排之後
  `focusOverride` 把鍵盤使用者送過去的地方。
- **匯出的 PNG 是 1:1。** canvas 用的就是預覽 SVG 自己的 `width` / `height`，沒有乘
  devicePixelRatio 或任何放大倍率，所以一張寬波形匯成 PNG 之後放大來看會糊。要更高
  解析度請用 SVG。
- v3.5.0 記載的兩條仍然成立：關聯線線身經過 bus 標籤字形時那幾個像素屬於標籤；
  在輸入框裡打字時按 `Ctrl+Z`，還沒提交的按鍵會被丟掉。

## v3.5.0 — 2026-09-15

v3.4.0 交付了 wavedrom 的 GUI 波形編輯之後，使用者問「有沒有把參考對象（一個內部的
VS Code 擴充套件）的功能全做出來」。答案是沒有：對照它那份 14 條的 feature scope 逐條
讀我們的程式碼，**完整 1 條、部分 9 條、缺 4 條**。這一版補的是被判為重要的那一批。

判準是使用者定的：**比參考對象好沒問題、冗餘功能沒做沒問題、重要功能缺少是極大缺陷**。
據此三條被判為冗餘、明確不做：匯出 SVG／PNG（我們的圖本來就在產出的 HTML／PDF 裡，
而且 lightbox 可放大）、唯讀 source pane（我們的「⠿ → MD 原始碼」是同一份源碼而且
**可寫**）、以及 `config.hscale`（它與 `period`／`phase` 同根，整批留給下一版）。

### Added — 關聯線（edges）

- **可以拉關聯線了。** 按工具列的「關聯線」武裝，然後從一格拖到另一格；或用鍵盤：
  游標停在起點按 Enter、移到終點再按 Enter。選中一條線之後，工具列會多出一列
  inspector（20 種形狀的下拉、標籤、刪除），**兩端各自可以再拖到別的格子**。
  武裝是 one-shot，但**沒有產出的拖曳不會解除武裝**——使用者按了按鈕卻沒拿到東西，
  解除等於懲罰他瞄不準。拖曳期間完全不碰文件，放開才是一次 commit ＝ **一步 undo**。
- **拖動一端時，共用的錨點字母會分岔。** wavedrom 的 `edge` 用單一字母指涉錨點，
  而同一個字母可以被多條線引用。把其中一條的一端拖走時，如果那個字母**別人也在用**，
  我們在目標格配一個新字母、只改這一條的 entry——**另一條的 entry 逐位元組不變**；
  如果是獨佔的，就原地搬移、**保住名字**（使用者可能已經把那個字母寫進標籤或註解）。
  直接搬會把別人的線一起拖走，那是資料損壞不是不方便。
- **三種落點什麼都不做，而且不留 undo 記錄**：拖回原格、拖到沒有波形的列（群組標題／
  空白列）、拖到自己另一端（會變成 `a~>a`）。實作上不是在 UI 寫三次特判，而是讓
  codec 回傳**同一份文件**，由既有的 commit 路徑判定「沒有改變」。
- **edge 的形狀清單（20 種）是從釘住的 wavedrom 3.5.0 原始碼量出來的**，不是憑記憶
  或文件：`arc-shape.js` 的 case 清單。切字串的方式同樣照著引擎的 `render-arcs.js`。
  引擎自己有一個瑕疵——它用未 trim 的字串配 trim 後算出的長度取標籤位移，所以
  `"  a~>b hello"` 的標籤會被讀成 `"b hello"`——我們的寫入端因此**絕不產生前導空白**。

### Added — 八條編輯缺口

- **筆刷從 11 顆增加到 22 顆**：補上 `P`／`N`（帶邊緣箭頭的時脈）、`|`（gap）與
  `2`–`9`（多色 bus）。`P`／`N` 與 `p`／`n` 的差別**實測過**：差異不在第二個半 brick
  （既有的 `brickOf` 量的是那裡，它量對了但只量了一半），而在**第一個半 brick 的
  symbol 本身**（`pclk`→`Pclk`），多出一個實心小三角形。畫布現在照量到的座標補上它。
- **鍵盤現在按得到全部 22 顆。** 舊的守衛寫著擋 Shift+方向鍵，但方向鍵早就由別處以
  鍵的身分擋掉了——那道守衛實際唯一的效果，是把**所有需要 Shift 才打得出來的字元**
  （`P`、`N`，以及美式鍵盤上的 `|`）擋在鍵盤路徑外。
- **bus 的 data 標籤可以直接在框上編輯**：點標籤文字（或鍵盤游標停在該格按 Enter）
  就地開一個輸入框。沒有標籤的 bus 格會畫一個安靜的佔位記號當作按壓目標——否則
  「先塗一個 `=` 造出 bus、再替它命名」這條最常見的流程在滑鼠上是斷的。
  **按框的其他地方仍然是塗色**，兩個動作有各自不相交的命中區。
  第 n 個標籤屬於第 n 個**消耗資料槽**的格子（只有 `=` 與 `2`–`9` 消耗，`.` 與 `|`
  不消耗），使用者永遠不需要自己數。
- **群組可以建立與解散。** 在 lane 列表上 Shift+點多選一段**連續**的 lane 再按建立；
  不連續的選取會被**拒絕**而不是把中間那些一起吞進去——WaveJSON 的群組是一段連續的
  陣列元素，跳著選表達不出來。解散會把 lane 原位放回去，**巢狀子群組原樣保留**。
- **head／foot 的尺規欄位補齊成八個**：`{head, foot} × {text, tick, tock, every}`。
  引擎實際讀取的就是這八個（量自 `parse-config.js`／`render-lanes.js`／
  `render-marks.js`），先前只開放其中五個，是使用者猜不出規律的任意子集。
  **清空一個欄位現在會真的移除那個鍵**，而不是寫入空字串：對 `text` 兩者等價
  （引擎用 truthy 判斷），但 `tick`／`tock` 的引擎判斷是 `=== undefined`，所以
  空字串會讓它畫出**一整條預設的 0 起算刻度尺**——使用者看著空欄位以為沒設定，
  圖上卻多了一條他沒要的編號尺。
- **複製 lane、插入空白列**各一顆按鈕。複製會帶走 `data` 與其他所有欄位，但
  **不帶 `node`**：複本是一條全新的 lane，沒有任何 edge 指著它，繼承別人的錨點字母
  會讓那些線靜默搬到複本上。空白列沒有專屬的移除鍵——它就是一條 lane，既有的刪除
  會處理它。
- **undo 歷史上限 200 步。** 每一步存的是整份文件的凍結拷貝，無上限會隨 session
  長度線性成長。

### Fixed

- **畫布不再對「畫不出來的關聯線」保持沉默。** `edgeLayout` 有三種跳過一條線的理由，
  先前只有其中一種會在畫面上說出來——而且剛好是引擎也不畫的那一種。另外兩種
  （錨點索引超出該 lane 的長度、形狀 parse 不了）**正是左邊畫布與右邊預覽真的不一致
  的情況**，卻一個字都沒有。其中第一種**從編輯器自己的按鈕就走得到**：插入／刪除
  cycle 不會重排 `node` 字串，所以刪掉幾格就可能把錨點推到 lane 的長度之外。
- **`|-` 與 `|->` 的轉角方向畫反了**，與 `-|`／`-|>` 逐位元組相同。這兩個形狀存在的
  唯一理由就是區分轉角順序，而形狀下拉選單把 20 種全開放——使用者在兩者之間切換，
  左邊毫無變化、右邊預覽卻變了。

### Known limitations

- **一條關聯線的線身經過 bus 標籤的字形時，那幾個像素屬於標籤。** 線的其餘部分仍然
  選得到。這是兩個命中目標之間的取捨：看得見的標籤 vs 看不見的 10px 命中線，我們
  選了看得見的那個。
- **在 lane 名稱或尺規欄位裡打字時按 `Ctrl+Z`，還沒提交的按鍵會被丟掉。** 這條路徑
  在 v3.4.0 就存在（全域快捷鍵同步重繪，輸入框被用儲存的舊值重建），這一版沒有讓
  它更容易踩到，但**把受影響的欄位從 3 個擴大到 10 個**。誠實的修法是一個「重繪前
  先 flush 目前聚焦欄位」的通用機制，那是下一版的事，不是在這一批尾端硬掛上去的
  東西。
...
## v3.4.1 — 2026-09-14（未單獨上架，內容隨 v3.4.2 一起發布）

**npm 上沒有 3.4.1 這個版本。** 它做完的時候 v3.4.2 已經在同一天做完，而 npm 的
`latest` 是照發布順序決定的——兩個 publish workflow 平行跑會競態，3.4.1 晚一步
落地就會把 `latest` 拉回去。要安全就得等第一顆發完（約 35 分鐘）再發第二顆，沒有
意義。所以下面這一段的修正是**隨 3.4.2 一起上架**的，段落保留是因為它是一件獨立的
事、有自己的根因與測試。
